...
<Note>
Kỹ năng này hướng dẫn coding agent của bạn cách tích hợp **300+ AI APIs** của LemonData chỉ bằng cách mô tả những gì bạn cần. Nó được đóng gói thành một kỹ năng dùng chung agent-first và hoạt động với bất kỳ coding agent nào hỗ trợ file skill hoặc rules.
...
Agent tạo code với endpoint `/v1/audio/transcriptions` chính xác và xử lý file âm thanh.
...
Agent tự động chuyển sang native endpoint để có hiệu năng tốt hơn (không chuyển đổi định dạng, extended thinking, prompt caching).
...
| Tạo ảnh | Midjourney, Flux, Stable Diffusion |
