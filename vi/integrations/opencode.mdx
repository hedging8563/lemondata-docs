---
title: "OpenCode"
description: "Cấu hình OpenCode để sử dụng API LemonData"
---
...
Tài liệu hiện tại của OpenCode khuyến nghị cấu hình các provider thông qua `opencode.json` hoặc `opencode.jsonc`. Với LemonData, hãy sử dụng một provider tùy chỉnh thay vì ghi đè provider `openai` tích hợp sẵn:
...
<Note>
Sử dụng `@ai-sdk/openai-compatible` để tương thích theo kiểu chat-completions. Nếu bạn cần cụ thể một đường dẫn provider được xây dựng trên `/v1/responses`, tài liệu provider của OpenCode khuyến nghị chuyển package sang `@ai-sdk/openai`.
</Note>
