...
Google Gemini CLI là một công cụ dòng lệnh để tương tác với các mô hình Gemini. TokenLab cung cấp một endpoint tương thích cho phép bạn sử dụng Gemini CLI với quyền truy cập vào hàng trăm mô hình.


<Warning>
Hãy coi đây là tích hợp thử nghiệm: Gemini CLI hiện chưa tài liệu hóa một quy trình Base URL tùy chỉnh kiểu TokenLab theo cách ổn định. Nếu bạn cần một luồng TokenLab trên terminal được hỗ trợ rõ ràng, hãy ưu tiên Codex CLI, Claude Code hoặc OpenCode.
</Warning>
...
2. Đi tới [API Keys](https://tokenlab.sh/dashboard/api)
...
<AccordionGroup>
  <Accordion title="Lỗi kết nối">
    - Xác minh `GOOGLE_GEMINI_BASE_URL` đã được đặt thành `https://api.tokenlab.sh`
  ...
  <Accordion title="Xác thực thất bại">
    ...
    - Đảm bảo key đang hoạt động trong TokenLab dashboard
  </Accordion>

  <Accordion title="Mô hình không khả dụng">
    - Kiểm tra tính khả dụng của các mô hình Gemini tại [tokenlab.sh/en/models](https://tokenlab.sh/vi/models)
