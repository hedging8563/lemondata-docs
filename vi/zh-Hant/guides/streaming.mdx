...
Streaming cho phép bạn nhận từng phần nội dung ngay khi phản hồi đang được tạo, mang lại trải nghiệm người dùng tốt hơn cho các ứng dụng trò chuyện.

## Kích hoạt Streaming

Thiết lập `stream: true` trong yêu cầu của bạn:
...
Mỗi khối (chunk) trong luồng streaming tuân theo định dạng sau:
...
## Xử lý kết thúc Streaming

Streaming kết thúc khi:
- `finish_reason: "stop"` - Hoàn thành bình thường
- `finish_reason: "length"` - Đạt đến giới hạn `max_tokens`
- `finish_reason: "tool_calls"` - Mô hình muốn gọi công cụ
- `data: [DONE]` - Thông báo cuối cùng
...
## Streaming bất đồng bộ (Async Streaming)
...
Đối với giao diện trò chuyện trên web:
