...
OpenAI Codex là một công cụ dòng lệnh (CLI) mã nguồn mở, hoạt động như một coding agent nhẹ, có khả năng đọc, sửa đổi và thực thi mã trong terminal. Nó được xây dựng dựa trên mô hình GPT và được tối ưu hóa cho việc tạo mã.
...
<Note>
Nếu tệp cấu hình không tồn tại, hãy chạy `codex` một lần để tạo tệp, sau đó tiến hành chỉnh sửa.
</Note>
...
|-------|----------|
| `gpt-4o` | Các tác vụ lập trình phức tạp, thiết kế kiến trúc |
...
| `/tokens` | Xem lượng token sử dụng |
...
<AccordionGroup>
  <Accordion title="Lỗi kết nối">
    - Xác minh `base_url` trong config.toml có chính xác là `https://api.lemondata.cc/v1` hay không
    - Kiểm tra trạng thái kết nối mạng
    - Đảm bảo không có sự can thiệp của máy chủ proxy
  </Accordion>
  ...
  <Accordion title="Không tìm thấy mô hình">
    - Kiểm tra xem tên mô hình có khớp hoàn toàn hay không
