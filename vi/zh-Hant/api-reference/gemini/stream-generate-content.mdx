...
description: "Sử dụng định dạng Google Gemini API để tạo nội dung dạng stream"
---

Phiên bản stream của endpoint Gemini `generateContent`. Trả về Server-Sent Events.
...
<ParamField query="key" type="string">
  API key (phương pháp thay thế cho xác thực qua header).
</ParamField>

## Body yêu cầu

Giống với [Tạo nội dung](/api-reference/gemini/generate-content).

## Phản hồi

Trả về một stream các đối tượng JSON, mỗi đối tượng chứa một phần của phản hồi.
