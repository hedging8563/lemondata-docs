...
<ParamField body="model" type="string" required>
  ID của mô hình để sử dụng. Xem [Models](https://lemondata.cc/vi/models) để biết các tùy chọn có sẵn.
</ParamField>
...
<ParamField body="messages" type="array" required>
  ...
  Khi `content` là một mảng, LemonData hỗ trợ các khối đa phương tiện có cấu trúc cho các mô hình tương thích:
  - text: `{ "type": "text", "text": "..." }`
  ...
  Đối với lưu lượng sản xuất đa phương tiện, ưu tiên các URL công khai `https`. LemonData sẽ chuyển đổi các khối media này thành dạng yêu cầu cụ thể của nhà cung cấp cần thiết cho mô hình vật lý được chuyển hướng.
</ParamField>
...
<ParamField body="top_k" type="integer">
  Tham số Top-K sampling (dành cho mô hình Anthropic/Gemini).
</ParamField>
...
<ParamField body="cache_control" type="object">
  Tùy chọn điều khiển cache của LemonData.

  - `type` (string): Chiến lược cache - `default`, `no_cache`, `no_store`, `response_only`, `semantic_only`
  - `max_age` (integer): TTL cache tính bằng giây (tối đa 86400)
</ParamField>
