...
Biến thể SSE streaming gốc của Gemini `generateContent`. TokenLab giữ nguyên sự kiện, thứ tự và trường upstream; sự kiện chỉ có metadata, chunk trung gian không có `finishReason` và EOF tự nhiên đều hợp lệ. Không thêm dấu Chat Completions `[DONE]` và không fallback sang Chat.
## Tham số đường dẫn
...
Tương tự như [Tạo nội dung](/api-reference/gemini/generate-content).

`generationConfig.candidateCount`, tổ hợp công cụ và trường tương lai được chuyển tiếp không thay đổi đến upstream; TokenLab không áp giới hạn streaming riêng.
