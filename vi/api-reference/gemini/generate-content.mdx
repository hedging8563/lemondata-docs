...
TokenLab chỉ cung cấp tuyến này cho mô hình công bố hợp đồng Gemini gốc và có tuyến upstream cùng giao thức; không fallback sang Chat Completions. Tên ProtoJSON lowerCamelCase và tên proto gốc snake_case đều là chính thức và được giữ nguyên, kể cả trong yêu cầu trộn lẫn. Trường chưa biết được chuyển tiếp theo best-effort và upstream quyết định khả năng hỗ trợ.
## Tham số đường dẫn
...
<ParamField body="contents" type="array" required>
  Nội dung hội thoại. Giá trị `role` và mọi `parts` được giữ nguyên như đã gửi; TokenLab không trim/lowercase, viết lại Base64 hay suy luận MIME. `inlineData` / `inline_data` và `fileData` / `file_data` đều là cách viết chính thức, và TokenLab không bắt buộc `fileData.mimeType` ở cục bộ. ID Files/Cache do TokenLab tạo được kiểm tra quyền sở hữu và giữ binding upstream ban đầu; nhà cung cấp quyết định các quy tắc media và tool khác.
</ParamField>
...
<ParamField body="generationConfig" type="object">
  Cấu hình tạo nội dung gốc. Tên lowerCamelCase như `maxOutputTokens`, `candidateCount`, `thinkingConfig` và tên snake_case tương ứng được chấp nhận không thay đổi. TokenLab không xóa `thinkingConfig` sau lỗi hoặc áp giới hạn cục bộ cho `candidateCount` khi streaming; upstream quyết định phạm vi và tổ hợp.
</ParamField>
