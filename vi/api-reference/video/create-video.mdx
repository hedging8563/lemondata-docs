...
<Info>
Với các mô hình Seedance 2.0, TokenLab tự động chuẩn bị các trường hình ảnh (`image`, `image_url`, `image_urls`, `reference_images`, `start_image`, `end_image`) thành material avatar ảo có thể tái sử dụng trước khi tạo video. Nếu chưa sẵn sàng trong 60 giây, API trả về `409 seedance_material_preparing` kèm `auto_material_asset_ids`; hãy thử lại sau khi các material đó chuyển sang `ACTIVE`.
</Info>
...
<ParamField body="draft" type="boolean">
  Cờ workflow Draft của Seedance 1.5 Pro. Dùng `draft=true` để tạo tác vụ draft trên các route Seedance hỗ trợ Draft. Không gửi cùng `draft_task_id`.
</ParamField>
