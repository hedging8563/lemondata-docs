...
<ParamField body="model" type="string" required>
  ID của mô hình để sử dụng. Xem [Models](https://lemondata.cc/vi/models) để biết các lựa chọn có sẵn.
</ParamField>
...
<ParamField body="tools" type="array">
  ...
  Với tool hosted `image_generation` dùng model tool ảnh mặc định hoặc đặt rõ `model: "gpt-image-2"`, LemonData sẽ xóa `input_fidelity` không được hỗ trợ trước khi chuyển tiếp request, vì GPT Image 2 đã xử lý input ảnh ở high fidelity. Đừng gửi `background: "transparent"` cho tool này; LemonData không âm thầm xóa trường đó vì nó làm thay đổi ngữ nghĩa output.
</ParamField>
