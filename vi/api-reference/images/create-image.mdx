...
Ghi chu tuong thich: neu yeu cau `gpt-image-2` gui `input_fidelity`, LemonData se xoa truong nay truoc khi chuyen tiep vi GPT Image 2 tu dong xu ly dau vao hinh anh voi do trung thuc cao.

### Ghi chú hành vi mô hình

Google Gemini không dùng cùng một hợp đồng selector:

- `gemini-3.1-flash-image-preview`, `gemini-3-pro-image-preview`, `nano-banana-2`, và `nano-banana-pro` hỗ trợ `aspect_ratio` cùng `resolution` (`1k`, `2k`, `4k`) cho text-to-image.
...
- Với request ảnh tham chiếu cho `nano-banana`, `nano-banana-2`, và `nano-banana-pro`, hãy dùng endpoint này (`/v1/images/generations`) với `operation: "image-to-image"` và `image_urls`. Đừng gửi request ảnh tham chiếu Nano Banana tới `/v1/images/edits`.
- Với request image-to-image Nano Banana, hãy bỏ qua `resolution`. `nano-banana-2` và `nano-banana-pro` chỉ công khai `resolution` cho text-to-image, còn `nano-banana` không công khai trường này.
...
<ParamField body="image_url" type="string">
  URL HTTPS công khai của ảnh tham chiếu cho image-to-image. Với họ Nano Banana, đặt `operation` là `image-to-image` và bỏ qua `resolution` trừ khi model hỗ trợ rõ trường này cho operation đó.
</ParamField>
...
Ví dụ Nano Banana dùng ảnh tham chiếu: gửi request tới `/v1/images/generations`, không gửi tới `/v1/images/edits`, và bỏ qua `resolution`:
