...
| `/v1/tasks/{id}` | GET | Lấy trạng thái bất đồng bộ của tác vụ video |
| `/v1/videos/generations/{id}` | GET | Đường dẫn trạng thái video tương thích ngược |

<Info>
Với client mới, hãy ưu tiên `/v1/tasks/{id}` và đi theo `poll_url` do phản hồi tạo trả về. Chỉ giữ `/v1/videos/generations/{id}` cho mục đích tương thích ngược.
</Info>
