...
TokenLab tự động quản lý việc lưu bộ nhớ đệm (caching) để tối ưu hóa hiệu suất và giảm chi phí. Mặc dù không có endpoint công khai để xóa các mục cache, bạn có toàn quyền kiểm soát hành vi caching thông qua các điều khiển ở cấp độ yêu cầu (request-level).

Đây là hướng dẫn cache ở phía request.

Nếu bạn cần các trang strict API reference, hãy xem:

- [Gửi phản hồi về bộ nhớ đệm](/api-reference/cache/submit-cache-feedback)
- [Lấy thống kê phản hồi cache](/api-reference/cache/get-cache-feedback-stats)
...
| Loại | Tác động |
|------|--------|
| `no_cache` | Bỏ qua tra cứu và ghi cache; luôn nhận phản hồi mới |
...
Hãy dùng định danh request/cache entry đã cache hit trong tổ chức của bạn; ID tùy ý sẽ bị từ chối.
