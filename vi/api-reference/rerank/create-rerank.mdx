...
<ParamField body="top_n" type="integer">
  Số lượng kết quả hàng đầu cần trả về. Mặc định là tất cả tài liệu. Phải tối thiểu là `1` và không được lớn hơn `documents.length`. Hiện LemonData chưa có hard cap thấp hơn theo từng provider đã được quản trị; nếu provider công bố giới hạn đó sau này, giới hạn phải được thêm vào truth request-shape của rerank trước khi được tài liệu hóa hoặc áp dụng.
</ParamField>
