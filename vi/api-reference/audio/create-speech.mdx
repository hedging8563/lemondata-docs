...
Các field tuỳ chọn được hỗ trợ khác nhau theo từng họ model. TokenLab kiểm tra ma trận field này trước khi routing: OpenAI TTS nhận `voice`, `instructions`, `response_format`, `stream_format`, và `speed`; MiniMax speech nhận `voice`, `voice_id`, `response_format`, `stream_format`, và `speed`; Gemini TTS nhận `prompt`, `language_code`, `voice`, `response_format`, `stream_format`, `speed`, và `temperature`.
