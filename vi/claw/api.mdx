<think>**Prioritizing the Translation**
**Evaluating Current Strategy**
**Analyzing Target Elements**
**Focusing on the Details**
**Finalizing the Translations**
**Executing the Translations**
**Detailing the Translations**
**Perfecting the Vietnamese Translation**
</think>
...
Trả về tất cả các instance của tổ chức đã được xác thực.
...
| Trạng thái | Mô tả |
|------------|-------------|
| `PENDING` | Vừa được tạo, đang chờ cung cấp tài nguyên |
| `PROVISIONING` | Container đang được triển khai |
| `RUNNING` | Instance đang hoạt động và ổn định |
| `STOPPED` | Đã dừng do hết hạn (dữ liệu được giữ lại trong 7 ngày) |
| `FAILED` | Triển khai thất bại (được tự động hoàn tiền) |
| `DELETED` | Đã bị xóa vĩnh viễn |
...
Tạo một instance mới và tính phí **$20** cho 30 ngày từ số dư tài khoản tổ chức của bạn.

<ParamField body="subdomain" type="string" required>
  Subdomain (3-32 ký tự, chữ cái viết thường và số + dấu gạch ngang). URL của Instance: `https://claw-{subdomain}.lemondata.cc`
</ParamField>

<ParamField body="defaultModel" type="string">
  Mô hình AI mặc định (ví dụ: `claude-opus-4-6`, `gpt-5.2`, `gemini-3-pro-preview`, `minimax-m2.1`, `kimi-k2.5`)
</ParamField>
...
<Warning>
Số dư không đủ sẽ trả về **402 Payment Required**. Hãy đảm bảo tổ chức của bạn có ít nhất $20 trước khi tạo instance.
</Warning>
...
Trả về chi tiết của một instance cụ thể.
...
Cập nhật cấu hình của instance.
...
## Xây dựng lại (Rebuild) Instance

```
POST /v1/claw/instances/:id/rebuild
```

Hủy và tạo lại container trong khi vẫn giữ lại dữ liệu (PVC). Giới hạn **10 lần rebuild mỗi ngày**.
...
Kéo dài chu kỳ thanh toán thêm 30 ngày ($20). Nếu instance đã bị dừng do hết hạn, nó sẽ tự động khởi động lại.
...
Trả về trạng thái sức khỏe của dịch vụ Claw. Không yêu cầu xác thực.
