...
Các trường cơ bản bắt buộc (`message`, `type`) luôn tồn tại. `code` và `param` là tùy chọn và chỉ xuất hiện khi phù hợp. Các trường gợi ý (`did_you_mean`, `suggestions`, `hint`, `retryable`, `retry_after`, `balance_usd`, `estimated_cost_usd`) là phần mở rộng tùy chọn để AI agent tự sửa. Xem [Hướng dẫn Agent-First API](/guides/agent-first-api) để biết chi tiết.

Các endpoint tương thích OpenAI sử dụng các loại lỗi cổng ổn định của TokenLab. Các endpoint tương thích Anthropic và Gemini sử dụng họ lỗi và cấu trúc phản hồi gốc của chúng.
