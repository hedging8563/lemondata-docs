...
Agent-First API của LemonData làm giàu phản hồi lỗi với các gợi ý có cấu trúc mà các agent AI có thể phân tích và hành động ngay lập tức — không cần tìm kiếm web, không cần tra cứu tài liệu, không phải đoán mò.
...
| `cache_pricing` | object or `null` | Giá upstream cho prompt cache + giảm giá semantic cache của nền tảng |
| `agent_preferences.<scene>` | object | Snapshot khuyến nghị phi-chat cho cảnh sử dụng đó |

`agent_preferences` được dẫn xuất từ một snapshot tỷ lệ thành công trong 24 giờ được cache:
