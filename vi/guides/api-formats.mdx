...
**Gemini Files và Cache:** Tuyến Gemini gốc hỗ trợ `/upload/v1beta/files`, `/v1beta/files`, `/v1beta/files:register` và `/v1beta/cachedContents`. Files dùng các kênh upstream tương thích Gemini File API; tài nguyên Cache tường minh cũng có thể đi qua kênh Vertex AI. Tài nguyên tạo qua LemonData được gắn với cùng channel/key upstream cho các lần gọi `generateContent` sau đó.
...
Nếu LemonData không thể định tuyến yêu cầu có công cụ native tới một đường upstream hỗ trợ định dạng native, hệ thống sẽ trả lỗi unsupported-field rõ ràng thay vì âm thầm bỏ công cụ hoặc giả vờ đó là hàm Chat Completions. Công cụ hàm do người dùng định nghĩa vẫn là đường công cụ dễ di chuyển nhất.
