...
Các ví dụ công khai hiện tại sử dụng `suno_music` cho việc tạo nhạc và bao gồm `mv` với phiên bản mô hình Suno chính thức như `chirp-v4`. Đối với các quy trình chỉ lời bài hát, gửi `action: "LYRICS"` với một mô hình mà chi tiết model của nó tài liệu hóa việc tạo lời bài hát và bỏ qua `mv`. Xem ID mô hình là ID TokenLab công khai, không phải là đảm bảo rằng các trường cụ thể của nhà cung cấp là trường chi tiết model.
...
Phản hồi tạo là một bản ghi tác vụ có thể polling, không phải âm thanh cuối cùng:
...
Phản hồi polling đã hoàn tất có thể bao gồm các trường media cuối cùng:
...
Các trường media cuối cùng sẽ vắng mặt cho đến khi `status` là `completed`. Tác vụ thất bại trả về `status: "failed"` cùng với `error`.
