...
Các ví dụ công khai hiện tại sử dụng `suno_music` cho việc tạo nhạc và bao gồm `mv` với phiên bản mô hình Suno chính thức như `chirp-v4`. Đối với các quy trình chỉ lời bài hát, gửi `action: "LYRICS"` với một mô hình mà hợp đồng công khai của nó tài liệu hóa việc tạo lời bài hát và bỏ qua `mv`. Xem ID mô hình là ID TokenLab công khai, không phải là đảm bảo rằng các trường cụ thể của nhà cung cấp là trường hợp đồng công khai.
