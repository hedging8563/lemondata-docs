---
title: "Khả năng tương thích IDE & SDK"
description: "Hướng dẫn về khả năng tương thích dựa trên bằng chứng cho các công cụ lập trình AI, SDK và framework"
---
...
- **Lộ trình native mạnh mẽ (Strong native path)** nghĩa là kho lưu trữ cũng có bằng chứng về adapter trực tiếp hoặc định dạng yêu cầu cho họ giao thức đó.
...
<Note>
  Các trường không được hỗ trợ sẽ không được xử lý đồng nhất. Trên các tuyến đường tương thích, một số trường bị bỏ qua hoặc được chuẩn hóa. Trên `/v1/responses`, các trường không được hỗ trợ có thể trả về lỗi `400` hoặc `503` rõ ràng khi tuyến đường đó không thể đảm bảo hành vi được yêu cầu.
</Note>
...
## Khả năng tương thích IDE & CLI

### Các công cụ đã được ghi nhận

| Công cụ | Mức độ hỗ trợ | Định dạng | Ghi chú |
|------|---------------|--------|-------|
| **Cursor** | Được hỗ trợ có giới hạn | Tương thích OpenAI | Hoạt động cho các luồng chat/editor tiêu chuẩn BYOK, không thay thế cho các tính năng do Cursor quản lý như Tab Completion |
| **Claude Code CLI** | Lộ trình native mạnh mẽ | Anthropic | Tuyến đường `/v1/messages` native với phạm vi adapter cho thinking và `tool_choice` |
| **Codex CLI** | Được hỗ trợ với giới hạn model/đường dẫn | OpenAI Responses | Coi `/v1/responses` là lộ trình nâng cao cho các quy trình làm việc cụ thể của Codex; một số trường chỉ dành cho Responses không được đảm bảo trên mọi model và đường dẫn |
| **Gemini CLI** | Nỗ lực tốt nhất / thử nghiệm | Gemini | Luồng URL cơ sở tùy chỉnh của TokenLab không phải là tích hợp chính thức của Gemini CLI |
...
### Các SDK & Framework đã được ghi nhận

| SDK / Framework | Ngôn ngữ | Mức độ hỗ trợ | Ghi chú |
|-----------------|----------|---------------|-------|
| **OpenAI SDK** | Python/JS/Go | Lộ trình cốt lõi được hỗ trợ | Lộ trình tương thích cho `/v1` Chat Completions và Embeddings; sử dụng các tuyến đường nhà cung cấp native cho hành vi cụ thể của nhà cung cấp, và đừng giả định các trường chỉ dành cho Responses hoạt động trên mọi model |
| **Anthropic SDK** | Python/JS | Lộ trình native mạnh mẽ | Tuyến đường Messages native với bằng chứng trực tiếp cho tools, thinking và prompt caching |
...
| **OpenAI Agents SDK** | Python | Lộ trình chat-completions được hỗ trợ | Sử dụng `AsyncOpenAI(base_url="https://api.tokenlab.sh/v1")` với `OpenAIChatCompletionsModel`; hành vi Responses native phụ thuộc vào model và tuyến đường được chọn |
...
| **Flowise** | Web / Node | Được hỗ trợ với giới hạn phạm vi | Sử dụng node chat của TokenLab khi có sẵn, hoặc node tương thích ChatOpenAI trỏ đến `https://api.tokenlab.sh/v1` |
...
| **Agno** | Python | Lộ trình được hỗ trợ | Sử dụng trình bao bọc model của TokenLab khi có sẵn, hoặc lộ trình URL cơ sở tương thích OpenAI |
...
| **Helicone** | Gateway / Quan sát | Lộ trình quan sát được hỗ trợ | Định tuyến các yêu cầu tương thích OpenAI qua Helicone Gateway với `Helicone-Target-Url: https://api.tokenlab.sh` |
| **Phoenix** | Quan sát | Lộ trình theo dõi được hỗ trợ | Tự động đo lường client SDK OpenAI được cấu hình với URL cơ sở `/v1` của TokenLab |
| **Opik** | Quan sát | Lộ trình theo dõi được hỗ trợ | Bao bọc client SDK OpenAI được cấu hình với URL cơ sở `/v1` của TokenLab |
| **LangBot** | Nền tảng Bot | Lộ trình chat được hỗ trợ | Sử dụng trình yêu cầu của TokenLab khi có sẵn, hoặc cấu hình trình yêu cầu tương thích OpenAI với endpoint `/v1` của TokenLab |
| **Open WebUI** | Web Chat | Lộ trình ứng dụng chat được hỗ trợ | Thêm TokenLab dưới dạng kết nối API tương thích OpenAI và làm mới danh sách model |
...
| Tham số | Kiểu | Mô tả |
|-----------|------|-------------|
| `stream_options` | object | Chỉ dành cho Chat Completions: `{ include_usage: true }` để đếm token |
| `reasoning_effort` | string | `"low"`, `"medium"`, `"high"` cho các model GPT-5 có khả năng suy luận |
...
| `store` | boolean | Lưu trữ để truy xuất sau này |
...
| `reasoning_effort` | string | Cho các model suy luận |
...
| Tham số | Kiểu | Mô tả |
|-----------|------|-------------|
| `cachedContent` | string | Tham chiếu nội dung đã lưu vào bộ nhớ đệm |
...
# Theo dõi mức sử dụng Chat Completions
...
<AccordionGroup>
  <Accordion title="Sử dụng passthrough cho các tham số không xác định">
    Các tham số không xác định chỉ được chuyển tiếp khi tuyến đường công khai và model được chọn hỗ trợ chúng.
  </Accordion>

  <Accordion title="Chỉ sử dụng stream_options.include_usage cho Chat Completions">
    Đối với streaming Chat Completions, hãy bật `stream_options.include_usage` để có số lượng token chính xác. Responses có hợp đồng streaming riêng và không hỗ trợ tùy chọn chỉ dành cho Chat này.
  </Accordion>

  <Accordion title="Sử dụng định dạng tool_choice phù hợp">
    Hãy khớp với định dạng mà SDK của bạn mong đợi. TokenLab chấp nhận cả định dạng của OpenAI và Anthropic.
  </Accordion>
</AccordionGroup>