...
Trang này được thiết kế có chủ đích hẹp hơn so với một ma trận tiếp thị:
...
|------|---------------|--------|-------|
| **Cursor** | Được hỗ trợ có giới hạn | Tương thích OpenAI | Hoạt động với các luồng chat/editor tiêu chuẩn BYOK, không thay thế cho các tính năng do Cursor quản lý như Tab Completion |
| **Claude Code CLI** | Lộ trình native mạnh mẽ | Anthropic | Lộ trình `/v1/messages` native với phạm vi bộ điều hợp cho thinking và `tool_choice` |
| **Codex CLI** | Được hỗ trợ với giới hạn model/đường dẫn | OpenAI Responses | Coi `/v1/responses` là lộ trình nâng cao cho các quy trình làm việc cụ thể của Codex; một số trường chỉ dành cho Responses không được đảm bảo trên mọi model và lộ trình |
...
<Note>
  Các trình soạn thảo và công cụ agent tương thích OpenAI khác thường hoạt động với cùng một mẫu URL cơ sở; hãy kiểm tra hỗ trợ nhà cung cấp tùy chỉnh của chính công cụ đó trước khi sử dụng trong môi trường production.
</Note>
...
|-----------------|----------|---------------|-------|
| **OpenAI SDK** | Python/JS/Go | Lộ trình cốt lõi được hỗ trợ | Lộ trình tương thích cho `/v1` Chat Completions và Embeddings; sử dụng các lộ trình nhà cung cấp native cho hành vi cụ thể của nhà cung cấp, và đừng giả định các trường chỉ dành cho Responses hoạt động trên mọi model |
...
| **LangChain** | Python/JS | Các bề mặt tiêu chuẩn được hỗ trợ | `ChatOpenAI` và `OpenAIEmbeddings` là phạm vi dự kiến; các phần bổ sung native của nhà cung cấp nằm ngoài phạm vi |
...
| **OpenAI Agents SDK** | Python | Lộ trình chat-completions được hỗ trợ | Sử dụng `AsyncOpenAI(base_url="https://api.tokenlab.sh/v1")` với `OpenAIChatCompletionsModel`; hành vi Responses native phụ thuộc vào model và lộ trình đã chọn |
...
| **Dify** | - | Được hỗ trợ với giới hạn phạm vi | Nhà cung cấp OpenAI và các luồng hướng chat-completions là lộ trình dự kiến; không phù hợp cho Responses hoặc hành vi WebSocket cụ thể của Codex |
...
| **Agno** | Python | Lộ trình được hỗ trợ | Sử dụng trình bao bọc model TokenLab khi có sẵn, hoặc lộ trình URL cơ sở tương thích OpenAI |
...
| **LangBot** | Bot Platform | Lộ trình chat được hỗ trợ | Sử dụng trình yêu cầu TokenLab khi có sẵn, hoặc cấu hình trình yêu cầu tương thích OpenAI với endpoint `/v1` của TokenLab |
...
| `n` | integer | Số lượng hoàn thành (1-128) |
...
TokenLab trả về các phản hồi lỗi tương thích OpenAI:
...
<AccordionGroup>
  <Accordion title="Sử dụng passthrough cho các tham số không xác định">
    Các tham số không xác định chỉ được chuyển tiếp khi lộ trình công khai và model được chọn hỗ trợ chúng.
  </Accordion>

  <Accordion title="Chỉ sử dụng stream_options.include_usage cho Chat Completions">
    Đối với streaming Chat Completions, hãy bật `stream_options.include_usage` để đếm token chính xác. Responses có hợp đồng streaming riêng và không cung cấp tùy chọn chỉ dành cho Chat này.
  </Accordion>

  <Accordion title="Sử dụng định dạng tool_choice phù hợp">
    Hãy khớp với định dạng mà SDK của bạn mong đợi. TokenLab chấp nhận cả định dạng OpenAI và Anthropic.
  </Accordion>
</AccordionGroup>