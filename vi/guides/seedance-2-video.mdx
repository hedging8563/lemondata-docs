...
1. Gọi [Tạo phiên xác thực người thật](/vi/api-reference/video/create-real-person-verification-session) với `callback_url` của bạn.
...
4. Khi `verification_status` là `succeeded`, hãy gọi [Liên kết kết quả xác thực người thật](/vi/api-reference/video/bind-real-person-verification-result).
5. Lưu trữ `group_id` được trả về; truyền nó khi tạo tài sản tài nguyên `liveness_face`.
6. Sử dụng [Liệt kê lịch sử xác minh người thật](/vi/api-reference/video/list-real-person-verification-history) để xem lại các phiên trước đó. Lịch sử được lưu trữ lâu dài, nhưng không kéo dài thời hạn liên kết 120 giây.
...
Đối với `aigc_avatar`, `group_id` là tùy chọn; TokenLab sẽ sử dụng hoặc tạo nhóm avatar ảo mặc định của tổ chức. Đối với `liveness_face`, `group_id` là bắt buộc và phải là nhóm được trả về bởi [Liên kết kết quả xác thực người thật](/vi/api-reference/video/bind-real-person-verification-result).
