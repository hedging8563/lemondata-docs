---
title: "Streaming (Akış)"
description: "Gerçek zamanlı akış yanıtlarını uygulama"
---

## Genel Bakış

Streaming, yanıt oluşturulurken içeriğin parçalarını gerçek zamanlı olarak almanıza olanak tanıyarak sohbet uygulamaları için daha iyi bir kullanıcı deneyimi sağlar.

## Streaming'i Etkinleştirme

İsteğinizde `stream: true` ayarını yapın:
...
## Streaming Yanıt Formatı

Akıştaki her bir parça (chunk) aşağıdaki formatı izler:
...
Akış şu durumlarda sona erer:
- `finish_reason: "stop"` - Normal tamamlanma
- `finish_reason: "length"` - `max_tokens` sınırına ulaşıldı
- `finish_reason: "tool_calls"` - Model araç çağırmak istiyor
- `data: [DONE]` - Son mesaj

## Tam Yanıtı Toplama

Streaming sırasında tam yanıtı toplamak için:
...
## Asenkron Streaming (Async Streaming)
