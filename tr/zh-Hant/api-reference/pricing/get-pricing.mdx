---
title: "Fiyatlandırmayı Al"
openapi: "GET /v1/pricing"
description: "Modellerin fiyatlandırma bilgilerini alın"
---

## Sorgu Parametreleri

<ParamField query="model" type="string">
  Fiyatlandırması alınacak model ID'si. Belirtilmezse tüm modellerin fiyatlandırması döndürülür.
</ParamField>

## Yanıt

<ResponseField name="object" type="string">
  Her zaman `list`.
</ResponseField>

<ResponseField name="data" type="array">
  Model fiyatlandırma nesnelerinden oluşan bir dizi.

  Her nesne şunları içerir:
  - `model` (string): Model tanımlayıcı
  - `pricing` (object): Fiyatlandırma bilgisi
    - `input_per_1m` (string | null): 1M `input tokens` başına ABD doları cinsinden fiyat
    - `output_per_1m` (string | null): 1M `output tokens` başına ABD doları cinsinden fiyat
    - `per_request` (string | null): İstek başına ABD doları cinsinden fiyat (sabit fiyatlı modeller için geçerlidir)
    - `is_lock_price` (boolean): Fiyatlandırmanın istek başına sabit olup olmadığı
    - `currency` (string): Her zaman `USD`
  - `provider` (string): Model sağlayıcısı
</ResponseField>

<RequestExample>
```bash cURL
curl "https://api.lemondata.cc/v1/pricing?model=gpt-4o" \
  -H "Authorization: Bearer sk-your-api-key"
```

```python Python
import requests

response = requests.get(
    "https://api.lemondata.cc/v1/pricing",
    headers={"Authorization": "Bearer sk-your-api-key"},
    params={"model": "gpt-4o"}
)

print(response.json())
```

```javascript JavaScript
const response = await fetch(
  'https://api.lemondata.cc/v1/pricing?model=gpt-4o',
  {
    headers: { 'Authorization': 'Bearer sk-your-api-key' }
  }
);

const data = await response.json();
console.log(data);
```

```go Go
package main

import (
    "encoding/json"
    "fmt"
    "net/http"
)

func main() {
    req, _ := http.NewRequest("GET", "https://api.lemondata.cc/v1/pricing?model=gpt-4o", nil)
    req.Header.Set("Authorization", "Bearer sk-your-api-key")

    client := &http.Client{}
    resp, _ := client.Do(req)
    defer resp.Body.Close()

    var result map[string]interface{}
    json.NewDecoder(resp.Body).Decode(&result)
    fmt.Println(result)
}
```

```php PHP
<?php
$ch = curl_init('https://api.lemondata.cc/v1/pricing?model=gpt-4o');

curl_setopt_array($ch, [
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_HTTPHEADER => [
        'Authorization: Bearer sk-your-api-key'
    ]
]);

$response = curl_exec($ch);
curl_close($ch);

$data = json_decode($response, true);
print_r($data);
```
</RequestExample>

<ResponseExample>
```json Response
{
  "object": "list",
  "data": [
    {
      "model": "gpt-4o",
      "pricing": {
        "input_per_1m": "2.50",
        "output_per_1m": "10.00",
        "per_request": null,
        "is_lock_price": false,
        "currency": "USD"
      },
      "provider": "openai"
    }
  ]
}
```
</ResponseExample>

## Fiyatlandırma Türleri

| Tür | Açıklama | Örnek Modeller |
|------|-------------|----------------|
| **Token tabanlı** | Giriş/çıkış token'larına göre ücretlendirilir | GPT-4o, Claude, Gemini |
| **İstek başına** | İstek başına sabit fiyat | DALL-E 3, Sora, Suno |