...
description: "Giriş metnini temsil eden bir embedding vektörü oluşturur"
---

## İstek Gövdesi

<ParamField body="model" type="string" required>
  Kullanılacak embedding model kimliği (örneğin: `text-embedding-3-small`).
</ParamField>

<ParamField body="input" type="string | array" required>
  Embedding işlemi yapılacak giriş metni. Bir dize veya dize dizisi olabilir.
</ParamField>

<ParamField body="encoding_format" type="string" default="float">
  Embedding formatı: `float` veya `base64`.
</ParamField>

<ParamField body="dimensions" type="integer">
  Çıktı boyutlarının sayısı (belirli modele bağlıdır).
</ParamField>

<ParamField body="user" type="string">
  Kötüye kullanım izleme amacıyla son kullanıcınızı temsil eden benzersiz bir tanımlayıcı.
</ParamField>
...
<ResponseField name="data" type="array">
  ...
  - `index` (integer): Giriş dizisindeki dizin
...
<ResponseField name="usage" type="object">
  `prompt_tokens` ve `total_tokens` değerlerini içeren Token kullanımı.
</ResponseField>
