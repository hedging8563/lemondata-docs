---
title: "OpenAI SDK"
description: "Resmi OpenAI SDK aracılığıyla LemonData kullanın"
---
...
OpenAI API ile tam olarak aynı şekilde çalışır:

```python
response = client.chat.completions.create(
    model="gpt-4o",  # Veya herhangi bir LemonData modeli
    messages=[
...
```python
response = client.chat.completions.create(
    model="gpt-4o",  # Veya claude-sonnet-4-5
    messages=[{
        "role": "user",
        "content": [
...
print(response.data[0].embedding[:5])  # İlk 5 boyut
```

## Ses - Metinden Konuşmaya (Text to Speech)
...
## Ses - Konuşmadan Metne (Transcription)
...
LemonData'nın temel avantajı, birden fazla sağlayıcıya erişim sunmasıdır:
