...
<Note>
Chat-completions uyumluluğu için `@ai-sdk/openai-compatible` kullanın. Özellikle `/v1/responses` gerekiyorsa sağlayıcı paketini `@ai-sdk/openai` olarak değiştirin.
</Note>
...
<AccordionGroup>
  <Accordion title="Bağlantı Hatası">
    - `options.baseURL`'in `https://api.lemondata.cc/v1` olarak ayarlandığını doğrulayın
