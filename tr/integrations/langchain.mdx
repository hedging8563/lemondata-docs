...
<AccordionGroup>
  <Accordion title="`base_url` değerini açıkça iletin">
    En güvenilir LemonData kurulumu, eski environment variable alias'larına bağlı kalmak yerine `base_url="https://api.tokenlab.sh/v1"` değerini doğrudan `ChatOpenAI` ve `OpenAIEmbeddings`'e iletmektir.
  </Accordion>
