...
Google Gemini CLI, Gemini modelleriyle etkileşim kurmak için kullanılan bir komut satırı aracıdır. TokenLab, Gemini CLI'yı 300'den fazla modele erişimle kullanmanıza olanak tanıyan uyumlu bir uç nokta (endpoint) sağlar.


<Warning>
Bu entegrasyonu deneysel kabul edin: Gemini CLI şu anda TokenLab benzeri özel Base URL iş akışını kararlı bir resmi yapılandırma olarak belgelemiyor. Desteklenen bir TokenLab terminal kurulumu istiyorsanız Codex CLI, Claude Code veya OpenCode kullanın.
</Warning>
...
1. [TokenLab Dashboard](https://tokenlab.sh/dashboard) sayfasına giriş yapın
2. [API Keys](https://tokenlab.sh/dashboard/api) bölümüne gidin
...
<AccordionGroup>
  <Accordion title="Bağlantı Hatası">
    - `GOOGLE_GEMINI_BASE_URL` değişkeninin `https://api.tokenlab.sh` olarak ayarlandığını doğrulayın
  ...
  <Accordion title="Kimlik Doğrulama Başarısız">
    ...
    - Anahtarın TokenLab panelinde aktif olduğundan emin olun
  </Accordion>

  <Accordion title="Model Mevcut Değil">
    - Gemini modellerinin uygunluğunu [tokenlab.sh/en/models](https://tokenlab.sh/tr/models) adresinden kontrol edin
