...
**Birincil yol**: OpenAI-compatible via OpenAILike

**Destek seviyesi**: OpenAILike üzerinden desteklenir
</Note>

LemonData için daha sağlam LlamaIndex kurulumu, yerleşik OpenAI sınıfları yerine **OpenAI-compatible integrations** kullanmaktır.

Mevcut LlamaIndex dokümantasyonu, yerleşik OpenAI sınıflarının resmi model adlarından metadata çıkarması nedeniyle, üçüncü taraf OpenAI-compatible endpoint'leri için açıkça `OpenAILike` kullanılmasını önerir.

Başka bir deyişle: burada `OpenAILike` yolunu LemonData için desteklenen yol olarak görün; yerleşik OpenAI sınıflarını değil.
