...
Desteklenen isteğe bağlı alanlar model ailesine göre değişir. TokenLab bu alan matrisini yönlendirmeden önce doğrular: OpenAI TTS `voice`, `instructions`, `response_format`, `stream_format` ve `speed` kabul eder; MiniMax speech `voice`, `voice_id`, `response_format`, `stream_format` ve `speed` kabul eder; Gemini TTS `prompt`, `language_code`, `voice`, `response_format`, `stream_format`, `speed` ve `temperature` kabul eder; daha dar Vidu/Kling tarzı TTS rotaları yalnızca `input` ve `stream_format` kabul edebilir. `user` dahil bilinmeyen üst seviye alanlar yok sayılmaz, `400 unsupported_parameter` döndürür.
