...
<ResponseField name="lemondata" type="object">
  LemonData'ya özgü herkese açık metadata. Detay route'u `category`, `pricing_unit`, `capability_flags`, `supported_operations`, `pricing_provenance`, `public_contract_summary`, tam `public_contract` ve chat dışı öneri snapshot'ları için sahne düzeyinde `agent_preferences` içerebilir.
</ResponseField>
...
Chat dışı modeller için detay route'u ayrıca `lemondata.agent_preferences.<scene>` döndürür.
