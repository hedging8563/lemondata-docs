...
Gemini `generateContent` işleminin yerel SSE streaming sürümü. TokenLab upstream olaylarını, sırasını ve alanlarını korur; yalnızca metadata içeren olaylar, `finishReason` bulunmayan ara chunk’lar ve doğal EOF geçerlidir. Chat Completions `[DONE]` işareti eklenmez ve Chat’e fallback yapılmaz.
## Yol Parametreleri
...
`generationConfig.candidateCount`, araç kombinasyonları ve gelecekteki alanlar değiştirilmeden upstream’e iletilir; TokenLab kendi streaming sınırını uygulamaz.
