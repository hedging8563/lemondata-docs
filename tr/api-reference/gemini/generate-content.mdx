...
TokenLab, Gemini modelleri için yerel Google Gemini API formatını destekler. Bu, Google AI SDK'ları ile doğrudan uyumluluk sağlar.
...
<ParamField path="model" type="string" required>
  ...
  TokenLab, mümkün olduğunda desteklenen Gemini yerel kanallarını yerel yol üzerinden yönlendirecek ve yerel olarak kullanıma hazır bir rota bu çok modlu istek için mevcut olmadığında otomatik olarak uyumlu dahili dönüştürme yoluna geri dönecektir.
...
<ParamField body="contents" type="array" required>
  ...
  - `parts` (array): içerik part'ları. TokenLab şu anda şunları destekler:
    - metin part'ları: `{ "text": "..." }`
    - satır içi medya part'ları: `inlineData` / `inline_data`
    - URL tabanlı dosya part'ları: `fileData` / `file_data`

  Medya part'ları için TokenLab şu anda image, audio ve video MIME type'larını kabul eder ve bunları Gemini uyumlu genel sözleşme üzerinden iletir.

  `user` ve `model` role değerleri büyük/küçük harfe duyarsız biçimde normalize edilir. `application/octet-stream` içeren `inlineData` / `inline_data`, TokenLab desteklenen görsel veya video baytlarını tanıyabildiğinde kabul edilir; aksi halde istek routing öncesinde başarısız olur. Yerel image-output isteklerinde yalnızca Google search/maps araç ailesi kabul edilir ve desteklenmeyen araç kombinasyonları upstream retry başlamadan önce başarısız olur.
</ParamField>
...
<ParamField body="generationConfig" type="object">
  ...
  - `candidateCount` (integer): Stream olmayan üretim için aday sayısı. Streaming istekleri bu alanı kaldırmalı veya `1` olarak tutmalıdır.
  - `responseModalities` (array): Uyumlu native rotalar için istenen çıktı modaliteleri.
  - `responseMimeType` (string): `text/plain` veya `application/json` gibi çıktı MIME tipi.
  - `responseSchema` (object): `responseMimeType` JSON istediğinde yapılandırılmış çıktı için JSON schema.
  - `thinkingConfig` / `thinking_config` (object): Uyumlu modeller için düşünme bütçesi seçenekleri.
</ParamField>
