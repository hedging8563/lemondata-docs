...
`gpt-image-2` görüntü oluşturma için desteklenen herkese açık parametreler `prompt`, `n`, `size`, `quality`, `response_format`, `async`, `background`, `output_format`, `output_compression` veya `compression`, `moderation` ve `user` alanlarıdır. `size` veya `quality` gönderilmezse TokenLab `auto` kullanır; özel `size` değerleri aşağıda belgelenen esnek `WIDTHxHEIGHT` sözleşmesini izlemelidir.
