...
<Warning>
Bu endpoint şu anda DALL-E tarzı multipart düzenleme akışını belgelemektedir. LemonData üzerindeki diğer desteklenen image-to-image aileleri, desteklendiği durumlarda `POST /v1/images/generations` ile `operation: "image-to-image"` ve `image_url` gibi JSON input'ları kullanır.
</Warning>

## İstek Gövdesi

<ParamField body="image" type="file" required>
  Düzenlenecek görsel. Geçerli bir PNG dosyası olmalı, 4MB'tan küçük olmalı ve kare olmalıdır. Görsel, düzenleme istenen yerde transparan alanlara sahip olmalıdır.
</ParamField>
...
<ParamField body="mask" type="file">
  Görselin nerede düzenleneceğini gösteren tamamen transparan alanlara sahip ek bir görsel. Geçerli bir PNG dosyası olmalı, 4MB'tan küçük olmalı ve `image` ile aynı boyutta olmalıdır.
</ParamField>

<ParamField body="model" type="string" default="dall-e-2">
  Görsel düzenleme için kullanılacak model. Şu anda yalnızca `dall-e-2` desteklenir.
</ParamField>
...
<ParamField body="size" type="string">
  Üretilen görsellerin boyutu. `256x256`, `512x512` veya `1024x1024` değerlerinden biri olmalıdır.
</ParamField>
