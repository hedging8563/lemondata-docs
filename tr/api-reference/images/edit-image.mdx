...
<Note>
`gpt-image-2` bu uç noktada desteklenir. Send multipart `image` uploads, JSON `image_url`, JSON `image_urls`, or the official JSON `images` array with `image_url` references; GPT Image edits accept up to 16 source images. Optional parameters are `mask`, `n`, `size`, `quality`, `response_format`, `output_format`, `output_compression` or `compression`, `moderation`, `partial_images`, and `user`. `background` yalnızca görüntü oluşturma yolunda kalır ve mevcut `gpt-image-2` görüntü düzenleme genel sözleşmesinin parçası değildir. Billing follows the upstream usage token details, including image input tokens and any image output tokens added for partial images.
</Note>
