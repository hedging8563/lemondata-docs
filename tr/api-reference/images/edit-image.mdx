...
<Note>
`gpt-image-2` bu uç noktada desteklenir. Multipart `image` yüklemeleri, JSON `image_url`, JSON `image_urls` veya resmi JSON `images` dizisinde `image_url` ya da `file_id` referansları gönderebilirsiniz; GPT Image edits en fazla 16 kaynak görüntü kabul eder. `file_id` değerleri LemonData `/v1/files` üzerinden `purpose=user_data` veya `purpose=vision` ile oluşturulmalıdır; böylece ağ geçidi bunları aynı upstream görüntü düzenleme kanalına bağlayabilir. İsteğe bağlı parametreler `mask`, `n`, `size`, `quality`, `response_format`, `output_format`, `output_compression` veya `compression`, `moderation`, `partial_images` ve `user` şeklindedir. `background` yalnızca görüntü oluşturma yolunda kalır ve mevcut `gpt-image-2` görüntü düzenleme genel sözleşmesinin parçası değildir. Faturalama, görüntü input tokenları ve partial images ile eklenen görüntü output tokenları dahil upstream usage token ayrıntılarını izler.
</Note>
