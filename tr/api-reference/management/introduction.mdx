...
description: "Kuruluş API anahtarlarını yönetin ve bir yönetim token'ıyla key düzeyinde kullanım ve faturalamayı alın."
---

## Genel Bakış

Management API, kuruluş API anahtarlarını yönetmenizi ve normal bir inference API anahtarı kullanmadan belirli bir anahtar için kullanım ve faturalama bilgilerini almanızı sağlar.
...
- Varsayılan olarak, request tarafındaki `limitAmount` USD olarak yorumlanır. RMB limit göndermek için `limitCurrency: "CNY"` ayarlayın; sunucu bunu kaydetmeden önce güncel kurla USD'ye çevirir.
- Response'taki parasal alanlar özgün USD alanını korur ve `_cny` ekli RMB karşılığını ekler.
- `exchange_rate`, API anahtarı metadata'sıyla birlikte döner ve bu response'taki her `*_cny` değeri için kullanılan USD→CNY kurudur.
...
Önce mevcut yönetim token'ı için erişilebilir API anahtarlarını listeleyin:
