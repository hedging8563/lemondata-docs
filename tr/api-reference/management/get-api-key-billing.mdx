...
Bu uç nokta, seçilen API anahtarı için tüketime dayalı faturalandırma toplamlarını döndürür. Buna toplam maliyet, toplam istek sayısı, faturalandırma birimine göre kullanım ile istekte gönderdiğiniz model adı (`model`), sağlayıcı, `scene`, ``, cache sınıfı ve günlük maliyet eğilimine göre kırılımlar dahildir.
