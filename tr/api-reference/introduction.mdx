...
| `/v1/tasks/{id}` | GET | Video işleri için asenkron durum al |
| `/v1/videos/generations/{id}` | GET | Video görev durumu için geriye dönük uyumlu yol |

<Info>
Yeni istemciler için `/v1/tasks/{id}` yolunu tercih edin ve create yanıtlarının döndürdüğü `poll_url` değerini izleyin. `/v1/videos/generations/{id}` yolunu yalnızca geriye dönük uyumluluk için saklayın.
</Info>
