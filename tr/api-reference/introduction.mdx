...
<Warning>
**İnteraktif Playground Hakkında**: Bu dokümantasyon sitesindeki playground yalnızca gösterim amaçlıdır ve API anahtarı girişini desteklemez. API'yi test etmek için lütfen şunları kullanın:
- **cURL** - Örnek komutları kopyalayın ve `sk-your-api-key` yerine gerçek anahtarınızı yazın
- **Postman** - [OpenAPI spesifikasyonumuzu](/openapi.json) içe aktarın
- **SDK** - OpenAI/Anthropic SDK'sını base URL'imizle kullanın
</Warning>
...
Native Google Gemini API format desteği:

| Uç Nokta | Metot | Açıklama |
|----------|--------|-------------|
| `/v1beta/models/{model}:generateContent` | POST | İçerik oluştur (Gemini formatı) |
| `/v1beta/models/{model}:streamGenerateContent` | POST | Streaming içerik oluştur (Gemini formatı) |
