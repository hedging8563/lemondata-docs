...
Ses çıkışı modele bağlıdır. TokenLab’da `output_audio` gönderilmezse Veo 3 ailesi varsayılan olarak sesi açık kabul eder. Bir model ses kontrolünü destekliyorsa bunu açıkça yönetmek için `output_audio` kullanın. CamelCase takma adı `outputAudio` da uyumluluk için kabul edilir.
...
<ParamField body="output_audio" type="boolean">
  Modele bağlı ses çıktısı anahtarıdır. TokenLab’da Veo 3 ailesi istekleri bu alan yoksa varsayılan olarak `true` kabul edilir. `kling-3.0-video`, öğe referansı kullanmayan isteklerde bu selector’ı kabul eder ve bu model için uyumlu ses kontrolünü kullanır; alan yoksa Kling istekleri varsayılan olarak sessizdir. `output_audio=true` ile `kling_elements` birlikte kullanılamaz. Diğer herkese açık video modelleri yönetilen varsayılan davranışlarını izler. Uyumluluk için camelCase `outputAudio` takma adı da kabul edilir.
</ParamField>
...
- Uyumluluk amacıyla TokenLab ayrıca camelCase takma adları `referenceImages`, `referenceImageType` ve `outputAudio` değerlerini de kabul eder.
- `operation` gönderilmezse TokenLab bunu girdilerden çıkarır. Yine de üretim için açıkça göndermeniz önerilir.
