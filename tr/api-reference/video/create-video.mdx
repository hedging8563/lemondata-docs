...
<ParamField body="kling_elements" type="array">
  `kling-3.0-video` için element referansı tanımları. Yalnızca görüntü koşullu isteklerde desteklenir. 1-3 element tanımlayın; her element `name`, isteğe bağlı `description` ve 2-4 görüntü URL içeren `element_input_urls` alanlarını içerir. `prompt` içinde elementi `@name` olarak referanslayın.
</ParamField>
...
<ParamField body="video_urls" type="array">
  Çok modlu referans koşullandırmasını destekleyen modeller için ek referans video girdileri. Desteklenen adet modele bağlıdır. `seedance-2.0` ve `seedance-2.0-fast` için LemonData şu anda en fazla 3 referans videoyu destekler.
</ParamField>
...
<ParamField body="audio_urls" type="array">
  Çok modlu referans koşullandırmasını destekleyen modeller için ek referans ses girdileri. Desteklenen adet modele bağlıdır. `seedance-2.0` ve `seedance-2.0-fast` için LemonData şu anda en fazla 3 referans sesi destekler.
</ParamField>
...
<ParamField body="output_audio" type="boolean">
  Modele bağlı ses çıkışı anahtarıdır. LemonData’da bu alan gönderilmezse Veo 3 ailesi varsayılan olarak `true` kabul edilir. `kling-3.0-video` da bu seçiciyi kabul eder ve uyumlu upstream `sound` kontrolüne eşler; Kling isteklerinde alan atlanırsa varsayılan çıktı sessizdir. CamelCase takma adı `outputAudio` da uyumluluk için kabul edilir.
</ParamField>
...
<ResponseField name="billing_transaction_id" type="string">
  Mutabakat tamamlandığında dönen LemonData faturalama işlem kimliği. Dashboard / mutabakat için kullanılan işlem tanımlayıcısıdır ve asenkron `id` / `task_id` alanlarından ayrıdır.
</ResponseField>
...
Model özel referans koşullandırmasını destekliyorsa `operation=reference-to-video` kullanın. LemonData'nın herkese açık sözleşmesinde görsel referanslar `reference_images`, çok modlu referans video ve sesler ise `video_urls` ve `audio_urls` ile iletilir. `seedance-2.0` ve `seedance-2.0-fast` için LemonData şu anda en fazla 9 referans görseli, ayrıca en fazla 3 referans video ve 3 referans sesi destekler. `duration` yalnızca üretilen çıktının süresini kontrol eder; referans video girdisi için ayrı bir süre sınırı belirlemez.
