...
Desteklenen modeller `marble-1.0`, `marble-1.1` ve `marble-1.1-plus` modelleridir. World Labs `marble-1.0-draft` modelini belgeler, ancak TokenLab bu endpoint içinde draft generation özelliğini henüz sunmaz.
...
Bu endpoint dünya üretimi ve polling kapsar. TokenLab şu anda World Labs media asset upload, world listing, world deletion veya bağımsız pano tools için proxy sağlamaz.
