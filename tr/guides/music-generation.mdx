...
Mevcut kamu örnekleri müzik üretimi için `suno_music` kullanır ve `chirp-v4` gibi resmi bir Suno model sürümünü `mv` ile gönderir. Sadece söz akışları için, söz üretimini belgeleyen bir model ile `action: "LYRICS"` gönderin ve `mv` alanını atlayın. Model kimliklerini kamu TokenLab kimlikleri olarak değerlendirin, sağlayıcıya özgü alanların kamu sözleşme alanları olduğu garantisi olarak değil.
...
Oluşturma yanıtı nihai ses değil, sorgulanabilir bir görev kaydıdır:
...
Tamamlanmış bir sorgulama yanıtı nihai medya alanlarını içerebilir:
...
Nihai medya alanları `status` `completed` olana kadar bulunmaz. Başarısız görevler `status: "failed"` ve `error` döndürür.
