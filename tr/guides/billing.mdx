...
### Asenkron Görev Faturalandırması (Video/Müzik/3D)

<Warning>
  **Önemli**: Video, müzik ve 3D oluşturma görevleri tamamlandığında değil, **gönderildiğinde faturalandırılır**.
</Warning>

Asenkron oluşturma görevleri (video, müzik, 3D modeller) için:

1. **Faturalandırma anında gerçekleşir** görev gönderildiğinde
2. Durumu sorgulamak için bir `task_id` alırsınız
3. **Görev başarısız olursa otomatik iade yoktur**

Bunun nedeni, yukarı akış sağlayıcılarının sonuçtan bağımsız olarak görev gönderildiğinde bizi ücretlendirmesidir. Görev hatalarıyla karşılaşırsanız, lütfen support@lemondata.cc ile iletişime geçin.

```python
# Örnek: Video oluşturma faturalandırması
response = client.post("/v1/video/generations", json={
    "model": "kling-v1",
    "prompt": "Okyanus üzerinde gün batımı"
})
