...
TokenLab, yaygın kodlama araçlarının, SDK'ların ve çerçevelerin minimum bağlantı kodu ile entegre olabilmesi için birden fazla API formatı sunar.

Bu sayfa kasıtlı olarak bir pazarlama matrisinden daha dar kapsamlıdır:

- **Supported** belgelenmiş somut bir kurulum yolunu ve TokenLab'nın bu yolun beklediği protokol şekline uygunluğu sunduğu anlamına gelir.
...
| **Gemini CLI** | En iyi çaba / deneysel | Gemini | Özel TokenLab base URL akışı istikrarlı bir yukarı akış sözleşmesi değildir |
...
<Tabs>
  <Tab title="Cursor">
    ...
    ```
    Cursor dahili olarak Anthropic tarzı araç formatını kullanır. TokenLab her iki formatı da destekler:
...
| **LlamaIndex** | Python | `OpenAILike` ile desteklenir | TokenLab gibi üçüncü taraf gateway'ler için dahili OpenAI sınıfları yerine `OpenAILike` kullanın |
...
| Parametre | Tip | Açıklama |
|-----------|-----|----------|
| `stream_options` | object | Yalnızca Chat Completions: token sayımları için `{ include_usage: true }` |
...
Chat Completions ve Responses dahil `stream: true` sunan üretim endpoint'leri Server-Sent Events (SSE) akışını kullanır:
...
<AccordionGroup>
  <Accordion title="Bilinmeyen parametreler için passthrough kullanın">
    Bilinmeyen parametreler yalnızca seçilen public route ve model desteklediğinde iletilir.
  </Accordion>

  <Accordion title="stream_options.include_usage yalnızca Chat Completions için kullanılır">
    Chat Completions akışında doğru token sayımları için `stream_options.include_usage`'ı etkinleştirin. Responses kendi akış sözleşmesine sahiptir ve bu Chat'e özel seçeneği herkese açık sunmaz.
  </Accordion>

  <Accordion title="Uygun tool_choice formatını kullanın">
    SDK'nızın beklediği formata uyun. TokenLab hem OpenAI hem de Anthropic formatlarını kabul eder.
  </Accordion>
</AccordionGroup>
