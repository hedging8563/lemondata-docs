...
| Argumente | zuerst `-y`, dann `@tokenlabai/mcp-server` | Verwenden Sie `@tokenlabai/mcp-server@0.6.10`, um die aktuell geprüfte Version festzuschreiben. |
...
| `full` | 77 | 75 generierte Endpunkt-Tools plus die zwei zusammengesetzten Discovery-Tools. Fügt Response-Lebenszyklus, Batches, Welten, native Modell-Discovery und andere zugelassene Entwickleroperationen hinzu. |
...
MCP-Tool-Aufrufe geben ein finales Ergebnis zurück. Streaming-only- und Echtzeit-Operationen sind ausgeschlossen, und generierte Request-Schemas beschränken unterstützte `stream`-Felder auf `false`. Kleine Bild- und Audio-Antworten geben nativen MCP-Content zurück; größere binäre Antworten werden in `TOKENLAB_ARTIFACT_DIR` geschrieben und mit ihrem lokalen Pfad, MIME-Typ und Byte-Anzahl zurückgegeben.
...
<CardGroup cols={2}>
  <Card title="TokenLab MCP" icon="plug" href="https://tokenlab.sh/mcp">
    Produktübersicht und Verbindungsoptionen für die Toolsets Core 31 und Full 78
  </Card>
