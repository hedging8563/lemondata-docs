...
description: "Installieren Sie das gemeinsame LemonData API-Integrations-Skill für jeden Coding-Agenten, mit Agent-First-Fehlerbehebung."
---

<Note>
Dieses Skill zeigt Ihrem Coding-Agenten, wie er die **300+ AI APIs** von LemonData integriert, indem Sie einfach beschreiben, was Sie benötigen. Es ist als gemeinsames agent-first Skill verpackt und funktioniert mit jedem Coding-Agenten, der Skill- oder Rules-Dateien unterstützt.
...
2. **Der Agent ruft die API auf** — er kann dabei einen Modellnamen oder Endpoint erraten
...
Dadurch wird das gemeinsame `lemondata-api-integration` Skill aus dem LemonData-Skills-Repository installiert.

Falls Ihr Tool den Installer nicht unterstützt, kopieren Sie den Ordner `skills/lemondata-api-integration/` aus dem Repository in das Verzeichnis für gemeinsame Skills oder Rules Ihres Tools.
</Note>
...
Der Agent generiert Code mit dem korrekten Endpoint `/v1/audio/transcriptions` und der Handhabung von Audiodateien.
...
Der Agent wechselt automatisch zum nativen Endpoint für bessere Performance (keine Formatkonvertierung, erweitertes Denken, Prompt-Caching).
...
<CardGroup cols={2}>
  ...
  <Card title="Kostenmanagement" icon="wallet">
    Legen Sie Nutzungslimits in Ihrem [Dashboard](https://lemondata.cc/dashboard) fest. Prüfen Sie `error.balance_usd`, um Kosten programmatisch zu überwachen.
  </Card>
...
<AccordionGroup>
  ...
  <Accordion title="Wie hilft die API dem Agenten, das richtige Modell zu finden?">
    Die [Agent-First API](/guides/agent-first-api) von LemonData gibt strukturierte Fehlerhinweise zurück. Wenn der Agent einen falschen Modellnamen errät, enthält der Fehler `did_you_mean` mit dem korrekten Namen und `suggestions` mit Alternativen. Der Agent liest diese und korrigiert sich beim nächsten Versuch automatisch.
  </Accordion>
...
<Info>
**Haben Sie Fragen?** Schauen Sie in unsere [GitHub Issues](https://github.com/hedging8563/lemondata-skills/issues) oder kontaktieren Sie [support@lemondata.cc](mailto:support@lemondata.cc)
</Info>