...
description: "Installieren Sie das gemeinsame TokenLab API-Integrations-Skill für jeden Coding-Agenten, mit Agent-First-Fehlerbehebung."
---

<Note>
Dieses Skill zeigt Ihrem Coding-Agenten, wie er die **Hunderte von KI-APIs** von TokenLab integriert, indem Sie einfach beschreiben, was Sie benötigen. Es ist als gemeinsames agent-first Skill verpackt und funktioniert mit jedem Coding-Agenten, der Skill- oder Rules-Dateien unterstützt.
...
Dadurch wird das gemeinsame `tokenlab-api-integration` Skill aus dem TokenLab-Skills-Repository installiert.

Falls Ihr Tool den Installer nicht unterstützt, kopieren Sie den Ordner `skills/tokenlab-api-integration/` aus dem Repository in das Verzeichnis für gemeinsame Skills oder Rules Ihres Tools.
</Note>
...
Wenn Sie `tokenlab-api-integration` sehen, war die Installation erfolgreich.

## API Key abrufen

<Steps>
  <Step title="TokenLab besuchen">
    Gehen Sie zu [tokenlab.sh](https://tokenlab.sh)
  </Step>
  <Step title="Anmelden">
    Erstellen Sie ein Konto oder melden Sie sich an
  </Step>
  <Step title="API Key abrufen">
    Navigieren Sie zu [Dashboard → API Keys](https://tokenlab.sh/dashboard/api) und erstellen Sie einen neuen Key
  </Step>
...
Behandeln Sie diese Header als Routing-Hinweise, nicht als erzwungenen automatischen Wechsel. Native Claude-Aufrufe verwenden `/v1/messages`; native Gemini-Aufrufe sollten dem von `X-TokenLab-Native-Endpoint` angegebenen Endpoint folgen, häufig `/v1beta/gemini`.
...
| Videogenerierung | Sora, Kling, Veo, Seedance |
...
<CardGroup cols={2}>
  ...
  <Card title="Kostenmanagement" icon="wallet">
    Legen Sie Nutzungslimits in Ihrem [Dashboard](https://tokenlab.sh/dashboard) fest. Prüfen Sie `error.balance_usd`, um Kosten programmatisch zu überwachen.
  </Card>
  <Card title="Seien Sie direkt" icon="bullseye">
    Sagen Sie "Ich möchte Bilder in Python generieren" statt "Hat TokenLab eine Bild-API?"
  </Card>
...
<AccordionGroup>
  <Accordion title="Skill wird nicht automatisch ausgelöst?">
    Versuchen Sie, in Ihrer Anfrage "TokenLab" oder "TokenLab API" zu erwähnen:
    ```
    Ich möchte GPT-4 mit TokenLab in mein Projekt integrieren
    ```
  </Accordion>

  <Accordion title="Wie hilft die API dem Agenten, das richtige Modell zu finden?">
    Die [Agent-First API](/guides/agent-first-api) von TokenLab gibt strukturierte Fehlerhinweise zurück. Wenn der Agent einen falschen Modellnamen errät, enthält der Fehler `did_you_mean` mit dem korrekten Namen und `suggestions` mit Alternativen. Der Agent liest diese und korrigiert sich beim nächsten Versuch automatisch.
  </Accordion>

  <Accordion title="Welche Coding-Agenten werden unterstützt?">
    Jeder Coding-Agent, der gemeinsame Skill- oder Rules-Verzeichnisse unterstützt. Wenn der Installer unterstützt wird, platziert `npx skills add` das gemeinsame `tokenlab-api-integration` Skill automatisch am erkannten Speicherort.
  </Accordion>
...
<Info>
**Haben Sie Fragen?** Schauen Sie in unsere [GitHub Issues](https://github.com/hedging8563/tokenlab-skills/issues) oder kontaktieren Sie [support@tokenlab.sh](mailto:support@tokenlab.sh)
</Info>
