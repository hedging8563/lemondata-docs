...
Das Google Gemini CLI ist ein Befehlszeilen-Tool zur Interaktion mit Gemini-Modellen. TokenLab bietet einen kompatiblen Endpunkt, der es Ihnen ermöglicht, das Gemini CLI mit Zugriff auf über 300 Modelle zu nutzen.


<Warning>
Dies ist nur eine experimentelle Kompatibilitätslösung: Gemini CLI dokumentiert derzeit keinen stabilen TokenLab-ähnlichen Custom-Base-URL-Workflow. Wenn Sie eine unterstützte TokenLab-Terminalintegration benötigen, verwenden Sie bevorzugt Codex CLI, Claude Code oder OpenCode.
</Warning>
...
2. Navigieren Sie zu [API Keys](https://tokenlab.sh/dashboard/api)
...
<AccordionGroup>
  <Accordion title="Verbindungsfehler">
    - Überprüfen Sie, ob `GOOGLE_GEMINI_BASE_URL` auf `https://api.tokenlab.sh` gesetzt ist
  ...
  <Accordion title="Authentifizierung fehlgeschlagen">
    ...
    - Stellen Sie sicher, dass der Key im TokenLab-Dashboard aktiv ist
  </Accordion>

  <Accordion title="Modell nicht verfügbar">
    - Prüfen Sie die Verfügbarkeit der Gemini-Modelle unter [tokenlab.sh/en/models](https://tokenlab.sh/de/models)
