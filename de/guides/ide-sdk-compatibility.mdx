...
- **Unterstützt** bedeutet, dass wir einen konkreten Einrichtungspfad dokumentieren und TokenLab die Protokollform bereitstellt, die dieser Pfad erwartet.
- **Starker nativer Pfad** bedeutet, dass das Repository auch direkte Adapter oder Belege für das Anforderungsformat dieser Protokollfamilie enthält.
...
<Note>
  Nicht unterstützte Felder werden nicht einheitlich behandelt. Auf Kompatibilitätsrouten werden einige Felder ignoriert oder normalisiert. Unter `/v1/responses` können nicht unterstützte Felder explizite `400` oder `503` Fehler zurückgeben, wenn die Route das angeforderte Verhalten nicht garantieren kann.
</Note>
...
|-----------------|----------|---------------|-------|
| **OpenAI SDK** | Python/JS/Go | Unterstützter Kernpfad | Kompatibilitätspfad für `/v1` Chat Completions und Embeddings; verwenden Sie native Provider-Routen für provider-spezifisches Verhalten und gehen Sie nicht davon aus, dass Responses-only Felder bei jedem Modell funktionieren |
| **Anthropic SDK** | Python/JS | Starker nativer Pfad | Native Messages-Route mit direkter Unterstützung für Tools, Thinking und Prompt Caching |
| **Vercel AI SDK** | TypeScript | Empfohlenes Integrationsmuster | Bevorzugen Sie `@ai-sdk/openai-compatible`; verwenden Sie `@ai-sdk/openai` nur, wenn Sie explizit Responses-natives Verhalten benötigen |
| **LangChain** | Python/JS | Unterstützte Standardoberflächen | `ChatOpenAI` und `OpenAIEmbeddings` sind der beabsichtigte Umfang; vendor-native Extras sind außerhalb des Umfangs |
| **LlamaIndex** | Python | Unterstützt via `OpenAILike` | Verwenden Sie `OpenAILike`, nicht die integrierten OpenAI-Klassen, für Drittanbieter-Gateways wie TokenLab |
...
| **Dify** | - | Unterstützt mit Einschränkungen | OpenAI-Provider und Chat-Completions-orientierte Flows sind der beabsichtigte Pfad; nicht geeignet für Codex-spezifische Responses oder WebSocket-Verhalten |
| **Flowise** | Web / Node | Unterstützt mit Einschränkungen | Verwenden Sie den TokenLab-Chat-Node, falls verfügbar, oder einen ChatOpenAI-kompatiblen Node, der auf `https://api.tokenlab.sh/v1` zeigt |
...
| **Helicone** | Gateway / Observability | Unterstützter Observability-Pfad | Leiten Sie OpenAI-kompatible Anfragen durch das Helicone Gateway mit `Helicone-Target-Url: https://api.tokenlab.sh` |
| **LangBot** | Bot-Plattform | Unterstützter Chat-Pfad | Verwenden Sie den TokenLab-Requester, falls verfügbar, oder konfigurieren Sie einen OpenAI-kompatiblen Requester mit dem `/v1`-Endpunkt von TokenLab |
...
| `reasoning_effort` | string | `"low"`, `"medium"`, `"high"` für GPT-5 Modelle mit Reasoning-Unterstützung |
...
| `store` | boolean | Speichern für späteren Abruf |
...
| `service_tier` | string | Prioritäts-Tier |
...
<AccordionGroup>
  ...
  <Accordion title="Verwenden Sie stream_options.include_usage nur für Chat Completions">
    Aktivieren Sie für Chat Completions-Streaming `stream_options.include_usage` für genaue Token-Zählungen. Responses hat einen eigenen Streaming-Vertrag und bietet diese Chat-only Option nicht an.
  </Accordion>

  <Accordion title="Verwenden Sie das passende tool_choice-Format">
    Passen Sie das Format an das erwartete Format Ihres SDKs an. TokenLab akzeptiert sowohl OpenAI- als auch Anthropic-Formate.
  </Accordion>
</AccordionGroup>