...
- **Starker nativer Pfad (Strong native path)** bedeutet, dass das Repository auch direkte Adapter oder Anforderungsformat-Belege für diese Protokollfamilie enthält.
...
| **Codex CLI** | Unterstützt mit Modell/Pfad-Einschränkungen | OpenAI Responses | Behandeln Sie `/v1/responses` als fortgeschrittenen Pfad für Codex-spezifische Workflows; einige Responses-only Felder sind nicht über jedes Modell und jeden Pfad garantiert |
| **Gemini CLI** | Best-Effort / experimentell | Gemini | Der benutzerdefinierte TokenLab Base-URL-Flow ist keine offizielle Gemini CLI-Integration |
...
| **OpenHands** | Unterstützt | OpenAI-kompatibel | Verwenden Sie den LiteLLM/OpenAI-kompatiblen Pfad mit `openai/<model>` und der `/v1` Base-URL von TokenLab |

<Note>
  Andere OpenAI-kompatible Editoren und Agent-Tools funktionieren oft mit demselben Base-URL-Muster; prüfen Sie die Unterstützung für benutzerdefinierte Provider des jeweiligen Tools, bevor Sie es in der Produktion einsetzen.
</Note>
...
| **Agno** | Python | Unterstützter Pfad | Verwenden Sie den TokenLab Modell-Wrapper, falls verfügbar, oder den OpenAI-kompatiblen Base-URL-Pfad |
...
| **Phoenix** | Observability | Unterstützter Tracing-Pfad | Auto-instrumentieren Sie den OpenAI SDK-Client, der mit der `/v1` Base-URL von TokenLab konfiguriert ist |
| **Opik** | Observability | Unterstützter Tracing-Pfad | Umschließen Sie einen OpenAI SDK-Client, der mit der `/v1` Base-URL von TokenLab konfiguriert ist |
...
### OpenAI Fortgeschrittene Funktionen

| Parameter | Typ | Beschreibung |
|-----------|------|-------------|
| `modalities` | array | `["text", "audio"]` für multimodal |
...
Generierungs-Endpunkte, die `stream: true` bereitstellen, einschließlich Chat Completions und Responses, verwenden Server-Sent Events (SSE):
...
# Chat Completions Nutzungstracking
...
<AccordionGroup>
  ...
  <Accordion title="Verwenden Sie stream_options.include_usage nur für Chat Completions">
    Aktivieren Sie für Chat Completions-Streaming `stream_options.include_usage` für genaue Token-Zählungen. Responses hat einen eigenen Streaming-Vertrag und stellt diese Chat-exklusive Option nicht bereit.
  </Accordion>

  <Accordion title="Verwenden Sie das passende tool_choice-Format">
    Passen Sie das Format an die Erwartungen Ihres SDKs an. TokenLab akzeptiert sowohl OpenAI- als auch Anthropic-Formate.
  </Accordion>
</AccordionGroup>