...
Den aktuellen Bestand öffentlicher Videomodelle finden Sie über die [Models API](/de/api-reference/models/list-models) oder auf der [Modellseite](https://lemondata.cc/de/models).
...
sequenceDiagram
    ...
    LemonData->>Provider: Normalisierte Anfrage weiterleiten
...
| Modell | Öffentliche Operationen |
|-------|----------|
| `grok-imagine-video` | Text-zu-Video, Bild-zu-Video |
...
Für `seedance-2.0` und `seedance-2.0-fast` unterstützt LemonData derzeit bis zu 9 Referenzbilder sowie zusätzlich bis zu 3 Referenzvideos und 3 Referenzaudios. `duration` steuert nur die Länge des generierten Outputs und definiert kein separates Dauerlimit für Referenzvideo-Eingaben.
...
Die Abrechnung ist modellabhängig. Einige öffentliche Videomodelle verhalten sich effektiv wie requestbasierte Modelle, andere eher wie sekundenbasierte Modelle. Verlassen Sie sich für die aktuelle öffentliche Preisfläche auf die [Modellseite](https://lemondata.cc/de/models) oder die [Pricing API](/de/api-reference/pricing/get-pricing).
