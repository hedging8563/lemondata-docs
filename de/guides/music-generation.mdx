...
Die Create-Antwort ist ein abfragbarer Aufgabendatensatz, nicht die finale Audiodatei:
...
Eine abgeschlossene Polling-Antwort kann die finalen Medienfelder enthalten:
...
Finale Medienfelder fehlen, bis `status` `completed` ist. Fehlgeschlagene Aufgaben geben `status: "failed"` mit `error` zurück.
