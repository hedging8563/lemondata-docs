...
Native Gemini-SSE-Streamingvariante von `generateContent`. TokenLab erhält Upstream-Ereignisse, Reihenfolge und Felder; reine Metadatenereignisse, Zwischen-Chunks ohne `finishReason` und ein natürliches EOF sind gültig. Es wird kein Chat-Completions-Marker `[DONE]` angehängt und kein Fallback auf Chat durchgeführt.
## Pfadparameter
...
`generationConfig.candidateCount`, Tool-Kombinationen und zukünftige Felder werden unverändert an den Upstream weitergeleitet; TokenLab erzwingt kein eigenes Streaming-Limit.
