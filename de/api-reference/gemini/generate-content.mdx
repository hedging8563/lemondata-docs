...
TokenLab bietet diese Route nur für Modelle mit veröffentlichtem nativen Gemini-Vertrag und einer gleichartigen Upstream-Route an; ein Fallback auf Chat Completions findet nicht statt. ProtoJSON-lowerCamelCase und ursprüngliche proto-snake_case-Namen sind beide offiziell und werden auch in gemischten Anfragen erhalten. Unbekannte Felder werden bestmöglich weitergeleitet, und der Upstream entscheidet über ihre Unterstützung.
## Pfadparameter
...
<ParamField body="contents" type="array" required>
  Konversationsinhalte. `role`-Werte und alle `parts` bleiben unverändert; TokenLab führt weder trim/lowercase noch Base64-Umschreibung oder MIME-Erkennung durch. `inlineData` / `inline_data` und `fileData` / `file_data` sind offizielle Schreibweisen, und `fileData.mimeType` ist lokal nicht vorgeschrieben. Bei TokenLab Files- oder Cache-IDs werden Eigentum und ursprüngliche Upstream-Bindung geprüft; andere Medien- und Toolregeln bleiben beim Anbieter.
</ParamField>
...
<ParamField body="generationConfig" type="object">
  Native Generierungskonfiguration. lowerCamelCase-Namen wie `maxOutputTokens`, `candidateCount` und `thinkingConfig` sowie die entsprechenden snake_case-Namen werden unverändert akzeptiert. TokenLab entfernt `thinkingConfig` nicht nach einem Fehler und erzwingt kein lokales Streaming-Limit für `candidateCount`; der Upstream entscheidet über Bereiche und Kombinationen.
</ParamField>
