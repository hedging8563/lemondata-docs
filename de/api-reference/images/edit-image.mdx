...
<Note>
`gpt-image-2` wird hier unterstützt. Akzeptiert werden multipart-`image`-Uploads, JSON `image_url` / `image_urls` und offizielle `images[]`-Referenzen (`image_url` oder `file_id`) mit bis zu 16 Quellbildern. `file_id`-Werte zuerst über `/v1/files` erstellen. Häufige optionale Parameter sind `mask`, `n`, `size`, `quality`, `response_format`, `output_format` / `output_compression`, `moderation`, `partial_images` und `user`.
</Note>
