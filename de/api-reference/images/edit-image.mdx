...
<Note>
`gpt-image-2` wird auf diesem Endpunkt unterstützt. Sende multipart-`image`-Uploads, JSON-`image_url`, JSON-`image_urls` oder das offizielle JSON-`images`-Array mit `image_url`- oder `file_id`-Referenzen; GPT Image Edits akzeptieren bis zu 16 Quellbilder. `file_id`-Werte müssen über LemonData `/v1/files` mit `purpose=user_data` oder `purpose=vision` erstellt werden, damit das Gateway sie an denselben Upstream-Image-Edit-Channel binden kann. Optionale Parameter sind `mask`, `n`, `size`, `quality`, `response_format`, `output_format`, `output_compression` oder `compression`, `moderation`, `partial_images` und `user`. `background` bleibt auf den Bildgenerierungs-Pfad beschränkt und gehört nicht zum aktuellen öffentlichen `gpt-image-2`-Image-Edit-Vertrag. Die Abrechnung folgt den Upstream-Usage-Token-Details, einschließlich Image-Input-Tokens und zusätzlicher Image-Output-Tokens durch partial images.
</Note>
