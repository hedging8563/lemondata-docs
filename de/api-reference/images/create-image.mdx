...
<Info>
Wenn `poll_url` zurückgegeben wird, verwenden Sie genau diese URL. Behandeln Sie `task_id` als die asynchrone Task-ID für nachfolgende Polling-Abfragen.
</Info>
...
<ResponseField name="poll_url" type="string">
  Relative URL zum Abrufen der Ergebnisse per Polling (z. B. `/v1/images/generations/{task_id}`).
</ResponseField>
