...
Unterstützte optionale Felder unterscheiden sich je nach Modellfamilie. TokenLab validiert diese Feldmatrix vor dem Routing: OpenAI TTS akzeptiert `voice`, `instructions`, `response_format`, `stream_format` und `speed`; MiniMax Speech akzeptiert `voice`, `voice_id`, `response_format`, `stream_format` und `speed`; Gemini TTS akzeptiert `prompt`, `language_code`, `voice`, `response_format`, `stream_format`, `speed` und `temperature`; engere Vidu/Kling-artige TTS-Routen akzeptieren möglicherweise nur `input` und `stream_format`. Unbekannte Top-Level-Felder, einschließlich `user`, werden nicht ignoriert, sondern mit `400 unsupported_parameter` abgelehnt.
