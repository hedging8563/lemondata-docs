...
<Info>
Für Seedance-2.0-Modelle bereitet TokenLab Bildfelder (`image`, `image_url`, `image_urls`, `reference_images`, `start_image`, `end_image`) vor der Generierung automatisch als wiederverwendbare Virtual-Avatar-Materialien vor. Wenn die Vorbereitung nicht innerhalb von 60 Sekunden abgeschlossen ist, gibt die API `409 seedance_material_preparing` mit `auto_material_asset_ids` zurück; wiederholen Sie die Anfrage, nachdem diese Materialien `ACTIVE` sind.
</Info>
...
<ParamField body="draft" type="boolean">
  Seedance 1.5 Pro Draft-Workflow-Schalter. Verwenden Sie `draft=true`, um auf draft-fähigen Seedance-Routen eine Draft-Aufgabe zu erstellen. Nicht zusammen mit `draft_task_id` senden.
</ParamField>
