...
<ParamField body="negative_tags" type="string">
  Zu vermeidende Stile. Erforderlich, wenn `audio_operation` `add-instrumental` ist; wird als `negativeTags` an KIE weitergegeben.
</ParamField>
...
<ParamField body="make_instrumental" type="boolean">
  Auf `true` setzen, um ein instrumentales Ergebnis ohne Gesang zu erzeugen. TokenLab ordnet dieses öffentliche Feld dem KIE-Flag `instrumental` zu. Verwenden Sie TokenLab-Feldnamen wie `make_instrumental`, `audio_url`, `negative_tags` und `continue_at`; KIE-Upstream-Namen wie `instrumental`, `uploadUrl`, `negativeTags` und `continueAt` werden abgelehnt.
</ParamField>
