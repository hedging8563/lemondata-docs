...
Mit der Management API können Sie Organisations-API-Keys verwalten und Nutzung sowie Billing für einen bestimmten Key abrufen, ohne einen normalen Inference-API-Key zu verwenden.
...
- Standardmäßig wird `limitAmount` auf der Request-Seite als USD interpretiert. Mit `limitCurrency: "CNY"` können Sie ein RMB-Limit senden; der Server rechnet es vor dem Speichern mit dem aktuellen Kurs in USD um.
