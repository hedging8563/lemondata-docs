...
<Warning>
**Über den interaktiven Playground**: Der Playground auf dieser Dokumentationsseite dient nur zu Demonstrationszwecken und unterstützt keine API-Key-Eingabe. Um die API zu testen, verwenden Sie bitte:
...
| `/v1beta/models/{model}:streamGenerateContent` | POST | Streaming-Inhalt generieren (Gemini-Format) |

<Note>
Gemini-Endpunkte unterstützen neben dem Standard-Bearer-Token auch die `?key=`-Query-Parameter-Authentifizierung.
</Note>
