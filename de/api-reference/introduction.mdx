...
| `/v1/tasks/{id}` | GET | Asynchronen Status für Video-Jobs abrufen |
| `/v1/videos/generations/{id}` | GET | Abwärtskompatibler Statuspfad für Video-Tasks |

<Info>
Für neue Clients bevorzugen Sie `/v1/tasks/{id}` und folgen Sie dem `poll_url`, das von Create-Responses zurückgegeben wird. Lassen Sie `/v1/videos/generations/{id}` nur für Abwärtskompatibilität bestehen.
</Info>
