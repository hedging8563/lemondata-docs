...
TokenLab expose plusieurs formats d'API afin que les outils de codage courants, les SDK et les frameworks puissent s'intégrer avec un minimum de code de liaison.
...
| Paramètre | Type | Description |
|-----------|------|-------------|
| `stream_options` | object | Chat Completions uniquement : `{ include_usage: true }` pour les comptes de tokens |
...
Les endpoints de génération qui exposent `stream: true`, notamment Chat Completions et Responses, utilisent le streaming Server-Sent Events (SSE) :
...
<AccordionGroup>
  <Accordion title="Utiliser le passthrough pour les paramètres inconnus">
    Les paramètres inconnus ne sont transmis que lorsque la route publique et le modèle sélectionnés les prennent en charge.
  </Accordion>

  <Accordion title="Utiliser stream_options.include_usage uniquement avec Chat Completions">
    Pour le streaming Chat Completions, activez `stream_options.include_usage` afin d'obtenir des comptes de tokens précis. Responses possède son propre contrat de streaming et n'expose pas cette option propre à Chat.
  </Accordion>
