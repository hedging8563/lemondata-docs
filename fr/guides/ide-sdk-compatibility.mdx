...
description: "Guide de compatibilité basé sur des preuves pour les outils de codage IA, les SDK et les frameworks"
---
...
- **Supporté** signifie que nous documentons un chemin d'installation concret et que TokenLab expose la structure de protocole attendue par ce chemin.
...
- **Meilleur effort** signifie que l'intégration peut fonctionner, mais que le client en amont ne traite pas ce flux de travail de passerelle personnalisé comme un contrat stable.
...
| **Gemini CLI** | Meilleur effort / expérimental | Gemini | Le flux d'URL de base personnalisé TokenLab n'est pas une intégration officielle de Gemini CLI |
...
<Note>
  D'autres éditeurs et outils d'agent compatibles OpenAI fonctionnent souvent avec le même modèle d'URL de base ; vérifiez le support de fournisseur personnalisé de l'outil avant de l'utiliser en production.
</Note>
...
| **Anthropic SDK** | Python/JS | Chemin natif robuste | Route Messages native avec preuves directes pour les outils, le raisonnement et la mise en cache de prompts |
...
| **Langflow** | Python / Web | Supporté avec limites de périmètre | Utilisez le champ **OpenAI API Base** du composant OpenAI avec le point de terminaison `/v1` de TokenLab |
...
| **Portkey Gateway** | Passerelle | Chemin supporté | Utilisez TokenLab comme fournisseur en amont pour le trafic de chat et Responses compatible OpenAI |
| **Helicone** | Passerelle / Observabilité | Chemin d'observabilité supporté | Routez les requêtes compatibles OpenAI via Helicone Gateway avec `Helicone-Target-Url: https://api.tokenlab.sh` |
| **LangBot** | Plateforme de bot | Chemin de chat supporté | Utilisez le demandeur TokenLab lorsqu'il est disponible, ou configurez un demandeur compatible OpenAI avec le point de terminaison `/v1` de TokenLab |
| **Open WebUI** | Chat Web | Chemin d'application de chat supporté | Ajoutez TokenLab comme connexion API compatible OpenAI et rafraîchissez la liste des modèles |
| **Chatbox / DeepChat / Jan / LibreChat / Cherry Studio** | Bureau / Web | Chemin d'application de chat supporté | Utilisez le flux de fournisseur personnalisé compatible OpenAI de chaque application ; ces applications sont généralement orientées chat-completions |
...
| `max_tokens` | integer | Nombre maximal de jetons de sortie |
| `temperature` | number | Température d'échantillonnage (0-2) |
| `top_p` | number | Échantillonnage nucléus (0-1) |
...
|--------|---------|-------------|
| Chaîne | `"auto"`, `"none"`, `"required"` | Sélection simple |
...
| Paramètre | Type | Description |
|-----------|------|-------------|
| `stream_options` | object | Chat Completions uniquement : `{ include_usage: true }` pour le comptage des jetons |
...
| `logprobs` | boolean | Renvoyer les log-probabilités |
| `top_logprobs` | integer | Nombre de log-probabilités principales (0-20) |
| `logit_bias` | object | Carte de biais des jetons (-100 à 100) |
...
| `max_output_tokens` | integer | Nombre maximal de jetons de sortie |
...
Supporte les formats d'outil OpenAI et Anthropic :
...
Les points de terminaison de génération qui exposent `stream: true`, y compris Chat Completions et Responses, utilisent les Server-Sent Events (SSE) :
...
<AccordionGroup>
  <Accordion title="Utilisez le passthrough pour les paramètres inconnus">
    Les paramètres inconnus ne sont transmis que lorsque la route publique et le modèle sélectionnés les supportent.
  </Accordion>

  <Accordion title="Utilisez stream_options.include_usage uniquement pour Chat Completions">
    Pour le streaming Chat Completions, activez `stream_options.include_usage` pour un comptage précis des jetons. Responses possède son propre contrat de streaming et n'expose pas cette option réservée à Chat.
  </Accordion>

  <Accordion title="Utilisez le format tool_choice approprié">
