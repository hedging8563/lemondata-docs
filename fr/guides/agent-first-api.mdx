...
L'API Agent-First de LemonData enrichit les réponses d'erreur avec des indices structurés que les agents d'IA peuvent analyser et appliquer immédiatement — pas de recherches web, pas de consultation de docs, pas de conjectures.
...
| `cache_pricing` | object or `null` | Prix du cache de prompt en amont + remise du cache sémantique de la plateforme |
| `agent_preferences.<scene>` | object | Instantané de recommandation non-chat pour cette scène |

`agent_preferences` est dérivé d'un instantané de taux de succès sur 24 heures mis en cache :
