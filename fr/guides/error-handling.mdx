...
Les champs de base requis (`message`, `type`) sont toujours présents. `code` et `param` sont facultatifs et ne sont renvoyés que lorsqu'ils sont pertinents. Les champs d'indice (`did_you_mean`, `suggestions`, `hint`, `retryable`, `retry_after`, `balance_usd`, `estimated_cost_usd`) sont des extensions optionnelles pour l'auto-correction des agents IA. Voir le [Agent-First API guide](/guides/agent-first-api) pour plus de détails.

Les endpoints compatibles OpenAI utilisent les types d'erreur stables du gateway de TokenLab. Les endpoints compatibles Anthropic et Gemini utilisent leurs propres familles d'erreurs natives et formats de réponse.
