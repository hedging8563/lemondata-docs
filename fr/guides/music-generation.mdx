...
Les exemples publics actuels utilisent `suno_music` pour la génération de musique et incluent `mv` avec une version officielle du modèle Suno, comme `chirp-v4`. Pour les flux uniquement de paroles, envoyez `action: "LYRICS"` avec un modèle dont le détails du modèle documente la génération de paroles et omettez `mv`. Traitez les ID de modèle comme des ID publics TokenLab, et non comme une garantie que les champs spécifiques au fournisseur sont des champs de détails du modèle.
...
La réponse de création est un enregistrement de tâche à interroger, pas l'audio final :
...
Une réponse de polling terminée peut inclure les champs média finaux :
...
Les champs média finaux sont absents jusqu'à ce que `status` soit `completed`. Les tâches échouées renvoient `status: "failed"` avec `error`.
