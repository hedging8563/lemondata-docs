...
Les exemples publics actuels utilisent `suno_music` pour la génération de musique et incluent `mv` avec une version officielle du modèle Suno, comme `chirp-v4`. Pour les flux uniquement de paroles, envoyez `action: "LYRICS"` avec un modèle dont le contrat public documente la génération de paroles et omettez `mv`. Traitez les ID de modèle comme des ID publics TokenLab, et non comme une garantie que les champs spécifiques au fournisseur sont des champs de contrat public.
