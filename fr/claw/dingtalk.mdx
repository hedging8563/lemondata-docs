...
Configurez le comportement des discussions de groupe dans l'onglet **Channels** de votre WebUI :

| Paramètre | Options | Description |
|---------|---------|-------------|
| **Group Policy** | Open (par défaut) / Allowlist / Disabled | Contrôle qui peut déclencher le bot dans les groupes |
| **Group Allowed Users** | Liste d'ID utilisateurs | Seuls ces utilisateurs peuvent déclencher le bot (mode liste d'autorisation) |

<Note>
DingTalk **nécessite toujours une @mention** pour déclencher le bot dans les groupes — c'est une limitation de la plateforme qui ne peut pas être configurée. Le filtrage par ID de groupe n'est pas disponible pour DingTalk ; utilisez le paramètre Group Policy pour contrôler l'accès.
</Note>

## Dépannage

<Note>
Le support DingTalk actuel dans LemonClaw n’est pas encore une intégration complète de workbench de groupe. Les messages de groupe n’entrent dans le flux IA que lorsqu’ils @mentionnent le bot, et les réponses privilégient le sessionWebhook d’origine.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="La connexion s'interrompt fréquemment">
    ...
    - De brèves déconnexions sont normales — le bot se reconnectera automatiquement
