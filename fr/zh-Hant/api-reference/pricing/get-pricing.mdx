---
title: "Obtenir les tarifs"
openapi: "GET /v1/pricing"
description: "Obtenir les informations de tarification des modèles"
---

## Paramètres de requête

<ParamField query="model" type="string">
  L'ID du modèle pour lequel obtenir les tarifs. Si non spécifié, les tarifs de tous les modèles sont retournés.
</ParamField>

## Réponse

<ResponseField name="object" type="string">
  Toujours `list`.
</ResponseField>

<ResponseField name="data" type="array">
  Un tableau d'objets de tarification de modèle.

  Chaque objet contient :
  - `model` (string): Identifiant du modèle
  - `pricing` (object): Informations de tarification
    - `input_per_1m` (string | null): Prix en USD pour 1M de `input tokens`
    - `output_per_1m` (string | null): Prix en USD pour 1M de `output tokens`
    - `per_request` (string | null): Prix en USD par requête (applicable aux modèles à prix fixe)
    - `is_lock_price` (boolean): Si la tarification est un prix fixe par requête
    - `currency` (string): Toujours `USD`
  - `provider` (string): Fournisseur du modèle
</ResponseField>

<RequestExample>
```bash cURL
curl "https://api.lemondata.cc/v1/pricing?model=gpt-4o" \
  -H "Authorization: Bearer sk-your-api-key"
```

```python Python
import requests

response = requests.get(
    "https://api.lemondata.cc/v1/pricing",
    headers={"Authorization": "Bearer sk-your-api-key"},
    params={"model": "gpt-4o"}
)

print(response.json())
```

```javascript JavaScript
const response = await fetch(
  'https://api.lemondata.cc/v1/pricing?model=gpt-4o',
  {
    headers: { 'Authorization': 'Bearer sk-your-api-key' }
  }
);

const data = await response.json();
console.log(data);
```

```go Go
package main

import (
    "encoding/json"
    "fmt"
    "net/http"
)

func main() {
    req, _ := http.NewRequest("GET", "https://api.lemondata.cc/v1/pricing?model=gpt-4o", nil)
    req.Header.Set("Authorization", "Bearer sk-your-api-key")

    client := &http.Client{}
    resp, _ := client.Do(req)
    defer resp.Body.Close()

    var result map[string]interface{}
    json.NewDecoder(resp.Body).Decode(&result)
    fmt.Println(result)
}
```

```php PHP
<?php
$ch = curl_init('https://api.lemondata.cc/v1/pricing?model=gpt-4o');

curl_setopt_array($ch, [
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_HTTPHEADER => [
        'Authorization: Bearer sk-your-api-key'
    ]
]);

$response = curl_exec($ch);
curl_close($ch);

$data = json_decode($response, true);
print_r($data);
```
</RequestExample>

<ResponseExample>
```json Response
{
  "object": "list",
  "data": [
    {
      "model": "gpt-4o",
      "pricing": {
        "input_per_1m": "2.50",
        "output_per_1m": "10.00",
        "per_request": null,
        "is_lock_price": false,
        "currency": "USD"
      },
      "provider": "openai"
    }
  ]
}
```
</ResponseExample>

## Types de tarification

| Type | Description | Exemples de modèles |
|------|-------------|----------------|
| **Basé sur les Tokens** | Facturé par `tokens` d'entrée/sortie | GPT-4o, Claude, Gemini |
| **Par requête** | Prix fixe par requête | DALL-E 3, Sora, Suno |