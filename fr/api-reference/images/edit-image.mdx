...
<Warning>
Cet endpoint documente actuellement le flux de modification multipart de style DALL-E. Les autres familles image-to-image prises en charge sur LemonData utilisent `POST /v1/images/generations` avec `operation: "image-to-image"` et des entrées JSON telles que `image_url` lorsqu’elles sont disponibles.
</Warning>
...
<ParamField body="model" type="string" default="dall-e-2">
  Modèle à utiliser pour la modification d’image. Actuellement, seul `dall-e-2` est pris en charge.
</ParamField>
...
<ParamField body="size" type="string">
  Taille des images générées. Doit être `256x256`, `512x512` ou `1024x1024`.
</ParamField>
