...
<Note>
`gpt-image-2` est pris en charge sur ce point de terminaison. Envoyez des uploads multipart `image`, JSON `image_url`, JSON `image_urls`, ou le tableau JSON officiel `images` avec des références `image_url` ou `file_id` ; les éditions GPT Image acceptent jusqu’à 16 images source. Les valeurs `file_id` doivent être créées via LemonData `/v1/files` avec `purpose=user_data` ou `purpose=vision`, afin que la passerelle puisse les lier au même canal amont d’édition d’image. Les paramètres optionnels sont `mask`, `n`, `size`, `quality`, `response_format`, `output_format`, `output_compression` ou `compression`, `moderation`, `partial_images` et `user`. `background` reste réservé au flux de génération d’images et ne fait pas partie du contrat public actuel d’édition d’images pour `gpt-image-2`. La facturation suit les détails de tokens d’usage renvoyés par l’amont, y compris les tokens d’entrée image et les tokens de sortie image ajoutés par partial images.
</Note>
