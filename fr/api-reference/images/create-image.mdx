...
Certains modèles d’image renvoient immédiatement une tâche asynchrone, et certains peuvent se comporter comme une réponse inline ou une réponse basée sur une tâche selon la route du provider. Lorsque la réponse inclut `status: "pending"`, traitez-la comme une réponse de tâche asynchrone :

<Info>
Utilisez exactement le `poll_url` renvoyé lorsqu’il est fourni. Considérez `task_id` comme l’identité de la tâche asynchrone pour le polling de suivi.
</Info>
...
<ResponseField name="poll_url" type="string">
  URL relative à interroger pour récupérer les résultats (par exemple `/v1/tasks/{id}`).
</ResponseField>

<ResponseField name="data" type="array">
  Tableau avec des données temporaires. `url` restera vide jusqu’à la fin du traitement.
</ResponseField>

<Note>
Lorsque vous recevez `status: "pending"`, utilisez l’endpoint [Obtenir le statut de l’image](/api-reference/images/get-image-status) pour interroger le résultat.
</Note>
