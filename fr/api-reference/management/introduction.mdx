...
description: "Gérez les API Keys de l’organisation et consultez l’usage et la facturation par clé avec un jeton de gestion."
---

## Vue d’ensemble

La Management API vous permet de gérer les API Keys d’organisation et de récupérer l’usage et la facturation d’une clé donnée sans utiliser de clé d’inférence standard.
...
- Par défaut, `limitAmount` côté requête est interprété en USD. Définissez `limitCurrency: "CNY"` pour envoyer un plafond en RMB ; le serveur le convertira en USD avec le taux de change actuel avant le stockage.
- Les champs monétaires côté réponse conservent la valeur USD d’origine et ajoutent un équivalent RMB avec le suffixe `_cny`.
- `exchange_rate` est renvoyé avec les métadonnées de l’API Key et correspond au taux USD→CNY utilisé pour chaque valeur `*_cny` de cette réponse.
...
Commencez par lister les API Keys disponibles pour le jeton de gestion courant :
