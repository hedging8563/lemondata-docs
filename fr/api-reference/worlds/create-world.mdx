...
Les modèles pris en charge sont `marble-1.0`, `marble-1.1` et `marble-1.1-plus`. World Labs documente aussi `marble-1.0-draft`, mais TokenLab n’expose pas encore la génération draft sur cet endpoint.
...
Cet endpoint couvre la génération de monde et le polling. TokenLab ne proxifie pas actuellement l’upload de media assets World Labs, la liste des mondes, la suppression de mondes ni les outils pano autonomes.
