...
<Info>
Pour les modèles Seedance 2.0, TokenLab prépare automatiquement les champs image (`image`, `image_url`, `image_urls`, `reference_images`, `start_image`, `end_image`) comme matériaux d'avatar virtuel réutilisables avant la génération. Si la préparation n'est pas terminée en 60 secondes, l'API renvoie `409 seedance_material_preparing` avec `auto_material_asset_ids`; réessayez lorsque ces matériaux sont `ACTIVE`.
</Info>
...
<ParamField body="draft" type="boolean">
  Sélecteur du workflow Draft de Seedance 1.5 Pro. Utilisez `draft=true` pour créer une tâche draft sur les routes Seedance compatibles. Ne l'envoyez pas avec `draft_task_id`.
</ParamField>
