...
<ParamField body="kling_elements" type="array">
  Définitions de références d’éléments pour `kling-3.0-video`. Pris en charge uniquement pour les requêtes avec condition image. Définissez 1 à 3 éléments ; chaque élément contient `name`, éventuellement `description`, et `element_input_urls` avec 2 à 4 URL d’images. Référencez l’élément dans `prompt` avec `@name`.
</ParamField>
...
<ParamField body="output_audio" type="boolean">
  Commutateur de sortie audio dépendant du modèle. Dans LemonData, la famille Veo 3 est traitée comme `true` par défaut quand ce champ est omis. `kling-3.0-video` accepte aussi ce sélecteur et le mappe vers le contrôle upstream compatible `sound`; les requêtes Kling omises restent silencieuses par défaut. L’alias camelCase `outputAudio` est aussi accepté.
</ParamField>
...
Utilisez `kling_elements` avec `kling-3.0-video` lorsque vous avez besoin de références d’éléments. Fournissez une entrée image (`image_url`, `image_urls`, `start_image` ou `end_image`) et référencez chaque élément dans le prompt avec `@name`.
