...
Variante SSE native et streamée de Gemini `generateContent`. TokenLab conserve les événements, l’ordre et les champs upstream ; les événements contenant uniquement des métadonnées, les chunks intermédiaires sans `finishReason` et une fin EOF naturelle sont valides. Aucun marqueur Chat Completions `[DONE]` n’est ajouté et aucun repli vers Chat n’a lieu.
...
`generationConfig.candidateCount`, les combinaisons d’outils et les champs futurs sont transmis sans modification à l’upstream ; TokenLab n’impose pas sa propre limite de streaming.
