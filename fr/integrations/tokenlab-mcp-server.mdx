...
Les profils `core` et `full` publient par défaut des schémas d'outils `portable` dont la taille et la profondeur sont limitées, tandis que le serveur valide chaque appel avec le contrat OpenAPI complet. Utilisez `TOKENLAB_MCP_SCHEMA_MODE=exact` pour des schémas imbriqués complets, ou `TOKENLAB_MCP_SCHEMA_MODE=strict` pour les providers qui exigent des objets fermés et toutes les propriétés dans `required` ; le mode strict représente les valeurs complexes de premier niveau sous forme de chaînes JSON.
...
| Arguments | `-y`, puis `@tokenlabai/mcp-server` | Utilisez `@tokenlabai/mcp-server@0.6.17` pour verrouiller la version actuellement vérifiée. |
