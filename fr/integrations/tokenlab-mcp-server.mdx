...
Utilisez-le lorsqu'un agent a besoin du contrat API TokenLab actuel au lieu d'une liste d'outils maintenue manuellement. Le profil `core` par défaut expose 29 outils d'endpoint générés ainsi que deux outils de découverte composites. Le profil `full` expose 75 outils d'endpoint générés ainsi que les deux mêmes outils composites.
...
| Arguments | `-y`, puis `@tokenlabai/mcp-server` | Utilisez `@tokenlabai/mcp-server@0.6.10` pour verrouiller la version actuellement vérifiée. |
...
| `full` | 77 | 75 outils d'endpoint générés plus les deux outils de découverte composites. Ajoute le cycle de vie des réponses, les lots, les mondes, la découverte de modèles natifs et d'autres opérations développeur autorisées. |
...
<CardGroup cols={2}>
  <Card title="TokenLab MCP" icon="plug" href="https://tokenlab.sh/mcp">
    Vue d’ensemble du produit et options de connexion pour les ensembles Core 31 et Full 77
  </Card>
