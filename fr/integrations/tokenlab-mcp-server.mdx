...
<CardGroup cols={2}>
  <Card title="Installer dans Cursor" icon="arrow-pointer" href="https://cursor.com/en/install-mcp?name=tokenlab-model-catalog&config=eyJ0eXBlIjoic3RkaW8iLCJjb21tYW5kIjoibnB4IiwiYXJncyI6WyIteSIsIkB0b2tlbmxhYmFpL21jcC1zZXJ2ZXIiXSwiZW52Ijp7IlRPS0VOTEFCX01DUF9UT09MX1BST0ZJTEUiOiJjYXRhbG9nIn19">
    Ajouter le profil public `catalog` à Cursor en un clic
  </Card>
  <Card title="Installer dans VS Code" icon="code" href="https://vscode.dev/redirect/mcp/install?name=tokenlab-model-catalog&config=%7B%22type%22%3A%22stdio%22%2C%22command%22%3A%22npx%22%2C%22args%22%3A%5B%22-y%22%2C%22%40tokenlabai%2Fmcp-server%22%5D%2C%22env%22%3A%7B%22TOKENLAB_MCP_TOOL_PROFILE%22%3A%22catalog%22%7D%7D">
...
Pour tout client stdio local, utilisez le nom `tokenlab-model-catalog`, la commande `npx`, les arguments séparés `-y` et `@tokenlabai/mcp-server`, et aucun répertoire de travail. Utilisez `@tokenlabai/mcp-server@0.6.17` pour figer la version actuellement vérifiée.
