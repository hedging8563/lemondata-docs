...
Google Gemini CLI est un outil en ligne de commande pour interagir avec les modèles Gemini. LemonData fournit un point de terminaison compatible qui vous permet d'utiliser Gemini CLI avec un accès à plus de 300 modèles.


<Warning>
Cette intégration doit être considérée comme expérimentale : Gemini CLI ne documente pas actuellement de flux de travail stable avec une URL de base personnalisée de type LemonData. Si vous avez besoin d’une intégration LemonData réellement prise en charge, privilégiez Codex CLI, Claude Code ou OpenCode.
</Warning>
