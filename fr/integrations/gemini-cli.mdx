...
Google Gemini CLI est un outil en ligne de commande pour interagir avec les modèles Gemini. TokenLab fournit un point de terminaison compatible qui vous permet d'utiliser Gemini CLI avec un accès à plus de 300 modèles.


<Warning>
Cette intégration doit être considérée comme expérimentale : Gemini CLI ne documente pas actuellement de flux de travail stable avec une URL de base personnalisée de type TokenLab. Si vous avez besoin d’une intégration TokenLab réellement prise en charge, privilégiez Codex CLI, Claude Code ou OpenCode.
</Warning>
...
2. Accédez aux [Clés API](https://tokenlab.sh/dashboard/api)
...
<AccordionGroup>
  <Accordion title="Erreur de connexion">
    - Vérifiez que `GOOGLE_GEMINI_BASE_URL` est défini sur `https://api.tokenlab.sh`
  ...
  <Accordion title="Modèle non disponible">
    - Vérifiez la disponibilité des modèles Gemini sur [tokenlab.sh/en/models](https://tokenlab.sh/fr/models)
