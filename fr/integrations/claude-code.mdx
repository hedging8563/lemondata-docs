...
[Claude Code](https://docs.anthropic.com/en/docs/claude-code) est l’outil CLI officiel d’Anthropic pour le développement assisté par IA. Vous pouvez le configurer pour utiliser LemonData comme fournisseur d’API.
...
<Note>
Pour LemonData, conservez `ANTHROPIC_BASE_URL` à `https://api.tokenlab.sh` sans suffixe `/v1`. Cela correspond au format des endpoints de style Anthropic et au modèle de gateway unifiée documenté par Claude Code.
</Note>
