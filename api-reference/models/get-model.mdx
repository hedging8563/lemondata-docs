...
<ResponseField name="lemondata" type="object">
  LemonData-specific public metadata. The detail route can include `category`, `pricing_unit`, `capability_flags`, `supported_operations`, `pricing_provenance`, `public_contract_summary`, the full `public_contract`, and scene-level `agent_preferences` for non-chat recommendation snapshots.
</ResponseField>
...
For non-chat models, the detail route also returns `lemondata.agent_preferences.<scene>`.
