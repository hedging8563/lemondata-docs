...
<ResponseField name="billing_transaction_id" type="string">
  LemonData billing transaction ID when settlement already completed during create. This is the dashboard/accounting transaction identifier and is separate from the async `id` / `task_id`.
</ResponseField>
