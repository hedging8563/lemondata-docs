...
<ParamField body="output_audio" type="boolean">
  Model-dependent audio output toggle. In LemonData, Veo 3 family requests default to `true` when this field is omitted. `kling-3.0-video` accepts this selector and maps it to the compatible upstream sound control; omitted Kling requests default to silent output. Other public video models follow their governed default behavior. The camelCase alias `outputAudio` is accepted for compatibility.
</ParamField>
