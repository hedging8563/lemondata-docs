...
<ParamField body="reference_images" type="array">
  Reference image inputs for models that support dedicated reference conditioning. The supported count is model-dependent. For `seedance-2.0` and `seedance-2.0-fast`, TokenLab currently supports up to 9 reference images plus up to 3 reference videos and 3 reference audios. Public `https` URLs are recommended; compatible models also accept inline `data:` URLs.
</ParamField>
