...
- `image_url.url` accepts a public image URL or a TokenLab material URI such as `asset://asset-YYYYMMDDHHMMSS-xxxxx`. The `role` determines whether that material is a first frame, last frame, or reference image.
- As a TokenLab compatibility shorthand, one top-level `material_asset_id` is treated as the first frame. Multiple top-level `material_asset_ids` are rejected because their roles are ambiguous; put them in `content[]` with explicit roles instead.
...
For existing TokenLab materials, use the public `asset-YYYYMMDDHHMMSS-xxxxx` ID, not an upstream provider asset ID. TokenLab verifies material ownership before generation.
