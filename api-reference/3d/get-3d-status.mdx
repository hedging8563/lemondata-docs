...
<RequestExample>
```bash cURL
curl "https://api.lemondata.cc/v1/tasks/3d_abc123" \
...
async function pollFor3DModel() {
  while (true) {
    const response = await fetch(
      `https://api.lemondata.cc${pollUrl}`,
      { headers: { 'Authorization': 'Bearer sk-your-api-key' } }
    );
...
func main() {
    taskID := "3d_abc123"
    pollURL := fmt.Sprintf("https://api.lemondata.cc/v1/tasks/%s", taskID)
...
<?php
$taskId = '3d_abc123';
$pollUrl = "https://api.lemondata.cc/v1/tasks/{$taskId}";
