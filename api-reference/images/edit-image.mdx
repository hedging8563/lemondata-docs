...
<Warning>
This endpoint currently documents the DALL-E style multipart editing flow. Other supported image-to-image families on LemonData use `POST /v1/images/generations` with `operation: "image-to-image"` and JSON inputs such as `image_url` where supported.
</Warning>
