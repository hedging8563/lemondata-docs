...
<ResponseField name="poll_url" type="string">
  Relative URL to poll for results (for example, `/v1/images/generations/{task_id}` or `/v1/tasks/{task_id}`).
</ResponseField>
    ...
    if data.get("status") == "pending":
        ...
        while True:
            status_resp = requests.get(
                f"https://api.lemondata.cc{poll_url}" if poll_url else f"https://api.lemondata.cc/v1/images/generations/{task_id}",
                headers={"Authorization": "Bearer sk-your-api-key"}
            )
