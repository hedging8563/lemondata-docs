...
<ParamField body="negative_tags" type="string">
  Styles to avoid. Required when `audio_operation` is `add-instrumental`; forwarded to KIE as `negativeTags`.
</ParamField>
...
<ParamField body="make_instrumental" type="boolean">
  Set to `true` to generate instrumental output without vocals. TokenLab maps this public field to KIE's `instrumental` flag. Use TokenLab field names such as `make_instrumental`, `audio_url`, `negative_tags`, and `continue_at`; KIE upstream names such as `instrumental`, `uploadUrl`, `negativeTags`, and `continueAt` are rejected.
</ParamField>
