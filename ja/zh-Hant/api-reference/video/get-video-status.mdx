...
description: "動画生成タスクのステータスと結果を取得します"
---

## パスパラメータ

<ParamField path="task_id" type="string" required>
  動画作成リクエストから返されたタスク ID。
</ParamField>
...
<ResponseField name="status" type="string">
  タスクステータス：`pending`、`processing`、`completed`、`failed`。
</ResponseField>
...
- 5〜10秒ごとにポーリングを実行する
- 実行時間が長いタスクに対しては、指数バックオフ (exponential backoff) を実装する
- 最大タイムアウト時間を設定する（例：10分）
