---
title: "コンテンツのストリーミング生成"
openapi: "POST /v1beta/models/{model}:streamGenerateContent"
description: "Google Gemini API 形式を使用したコンテンツのストリーミング生成"
---

Gemini `generateContent` エンドポイントのストリーミング版です。Server-Sent Events を返します。
...
[コンテンツの生成](/api-reference/gemini/generate-content) と同様です。

## レスポンス

JSON オブジェクトのストリームを返します。各オブジェクトには部分的なレスポンスが含まれます。
