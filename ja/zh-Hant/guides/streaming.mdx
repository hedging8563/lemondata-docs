---
title: "ストリーミング (Streaming)"
description: "リアルタイムのストリーミングレスポンスを実装する"
---

## 概要

ストリーミングを使用すると、レスポンスの生成中にコンテンツの一部をリアルタイムで受信できるため、チャットアプリケーションでより優れたユーザーエクスペリエンスを提供できます。
...
ストリーム内の各チャンク (chunk) は、以下の形式に従います：
...
- `finish_reason: "tool_calls"` - モデルがツール呼び出しを要求
- `data: [DONE]` - 最終メッセージ
