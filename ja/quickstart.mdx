...
<Steps>
  <Step title="アカウントの作成">
    メールアドレスを使用して [lemondata.cc](https://lemondata.cc) でサインアップします。
  </Step>
  <Step title="クレジットの追加">
    ダッシュボードに移動し、アカウントにクレジットを追加します。最低利用料金のない従量課金制です。
  </Step>
  <Step title="APIキーの作成">
    **Dashboard → API Keys** に移動し、新しいキーを作成します。キーは一度しか表示されないため、安全にコピーして保管してください。
  </Step>
</Steps>

<Warning>
  APIキーは安全に保管してください。クライアント側のコードや公開リポジトリには決して公開しないでください。
</Warning>
