...
TokenLab は単一の統一 API を通じて動画生成機能を提供します。動画生成は**非同期**です。リクエスト送信後、`task_id` と `poll_url` が返り、その後はポーリングで最終結果を取得します。

### 可用性とポーリング

公開動画モデルの最新在庫を確認するには、[Models API](/ja/api-reference/models/list-models) または [モデルページ](https://tokenlab.sh/ja/models) を利用してください。

作成レスポンスで `poll_url` が返る場合は、その URL をそのまま使ってください。`/v1/tasks/{id}` を指す場合は、それを固定の正規ステータスエンドポイントとして扱ってください。

### モデルとメディアの動作

音声挙動はモデル依存です。TokenLab では、Veo 3 系列は `output_audio` を省略すると既定で有声として扱われます。一方、無音が既定のモデルや、安定した音声トグルを公開していないモデルもあります。
...
TokenLab の公開動画契約は次の操作値を受け付けます。対応状況はモデルごとに異なり、プロバイダーが機能を追加または終了すると変わります。専用操作に依存する前に、選択したモデル契約を確認してください。
...
`tokenlab.capabilities`、`tokenlab.supported_operations`、`tokenlab.public_contract_summary`、`tokenlab.public_contract` を真値として扱ってください。以下の例はワークフローパターンであり、完全なモデル一覧ではありません。
...
`seedance-2.0` と `seedance-2.0-fast` では、TokenLab は現在最大 9 枚の参照画像に加えて、最大 3 本の参照動画と 3 本の参照音声をサポートします。`duration` は生成される出力長のみを制御し、参照動画入力の長さ上限を個別に定義するものではありません。 `grok-imagine-video` の reference-to-video は最大 7 件の画像参照（`reference_images` または `image_urls`）を受け付け、`duration` は最大 10 秒です。参照画像を `image_url` / `image` の先頭フレーム入力と組み合わせないでください。`grok-imagine-video-1.5-preview` は image-to-video のみです。
...
動画課金はモデルごとに異なります。実質的に従量が「回数ベース」のモデルもあれば、「秒ベース」のモデルもあります。最新の公開価格は [モデルページ](https://tokenlab.sh/ja/models) または [Pricing API](/ja/api-reference/pricing/get-pricing) を確認してください。
