...
description: "AIエージェントが初回のリトライで自己修正できるようにするための、構造化されたエラーヒント"
---
...
すべてのエラーレスポンスには、標準の `error` オブジェクト内に `did_you_mean`、`suggestions`、`hint`、`retryable`、`retry_after` などのオプションフィールドが含まれます。これらのフィールドは後方互換性があり、これらを使用しないクライアントには影響を与えません。
...
| フィールド | 型 | 説明 |
|-------|------|-------------|
| `did_you_mean` | `string` | 最も近い一致するモデル名 |
| `suggestions` | `array` | メタデータ付きの推奨モデル |
| `alternatives` | `array` | 現在利用可能な代替モデル |
| `hint` | `string` | 人間またはエージェントが読み取り可能な次のステップのガイダンス |
| `retryable` | `boolean` | 同じリクエストをリトライして成功する可能性があるかどうか |
| `retry_after` | `number` | リトライ前に待機する秒数 |
| `balance_usd` | `number` | 現在の米ドル（USD）建てのアカウント残高 |
...
<Note>
OpenAI互換のエンドポイントは、`rate_limit_exceeded`、`upstream_error`、`all_channels_failed` などのLemonDataの安定したパブリックエラータイプを使用します。Anthropic互換およびGemini互換のエンドポイントは、独自のネイティブレスポンス形式を使用します。
</Note>
...
| Google (Gemini) | `/v1beta/gemini` | フォーマット変換なし、グラウンディング、安全性設定 |
| OpenAI | — | Chat completionsがすでにネイティブフォーマットです |

これらのヘッダーは、ストリーミングと非ストリーミングの両方のレスポンスに表示されます。

## /v1/models の機能強化

各モデルオブジェクトの `lemondata` 拡張に含まれる3つの新しいフィールド:
...
| フィールド | 値 | 説明 |
|-------|--------|-------------|
| `category` | `chat`, `image`, `video`, `audio`, `tts`, `stt`, `3d`, `embedding`, `rerank` | モデルのタイプ |
...
<CardGroup cols={2}>
  <Card title="迅速に失敗し、有益な情報を返す" icon="bolt">
    エージェントが自己修正するために必要なすべてのデータとともに、エラーを即座に返します。
  </Card>
  <Card title="自動ルーティングなし" icon="route">
    APIが勝手に別のモデルに置き換えることはありません。エージェントが決定します。
  </Card>
  <Card title="データ駆動型の提案" icon="database">
    すべての推奨事項は、ハードコードされたリストではなく、本番環境のデータに基づいています。
  </Card>
