...
description: "AIエージェントが最初の再試行で自己修正するための構造化されたエラーヒント"
---

## 概要

LemonDataのAgent-First APIは、AIエージェントが即座に解析して対応できる構造化されたヒントでエラーレスポンスを強化します。ウェブ検索、ドキュメントの確認、推測は不要です。
...
| `balance_usd` | `number` | 現在の米ドル建てアカウント残高 |
...
|---------------|-------------------|---------|
| Anthropic (Claude) | `/v1/messages` | フォーマット変換なし、拡張思考、プロンプトキャッシュ |
| Google (Gemini) | `/v1beta/gemini` | フォーマット変換なし、グラウンディング、安全設定 |
| OpenAI | — | Chat completionsはすでにネイティブフォーマットです |
...
<CardGroup cols={2}>
  ...
  <Card title="データ駆動型の推奨" icon="database">
