...
現在の OpenClaw バージョンでは、推奨される方法は `models.providers` 経由で TokenLab を構成することです。
...
<AccordionGroup>
  ...
  <Accordion title="lemondata-responses で /v1 を忘れる">
    `openai-responses` は TokenLab の `/v1/responses` パスにマップされるため、`lemondata-responses` は `https://api.tokenlab.sh/v1` を使用する必要があります。
  </Accordion>
