...
サポート範囲は各統合ページを参照してください。このドキュメント群では、`recommended` や `supported` は「文書化されたセットアップ経路が存在する」ことを意味しますが、repo 内のすべてのフレームワーク補助層やプロバイダー固有機能に end-to-end 回帰があることまでは自動的に意味しません。
