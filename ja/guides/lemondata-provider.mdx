...
|----------|----------------|----------|---------|
| `lemondata` | `openai-completions` | GPT、DeepSeek、Qwen、およびほとんどの OpenAI 互換コール | `https://api.tokenlab.sh/v1` |
| `lemondata-responses` | `openai-responses` | `/v1/responses` セマンティクスを期待する OpenAI Responses ワークフロー | `https://api.tokenlab.sh/v1` |
| `lemondata-claude` | `anthropic-messages` | Claude のネイティブ Messages API | `https://api.tokenlab.sh` |
| `lemondata-gemini` | `google-generative-ai` | Gemini のネイティブ API 形式 | `https://api.tokenlab.sh` |
| `lemondata-minimax` | `anthropic-messages` | MiniMax のネイティブルーティング | `https://api.tokenlab.sh` |
...
<AccordionGroup>
  ...
  <Accordion title="lemondata-responses で /v1 を忘れる">
    `openai-responses` は LemonData の `/v1/responses` パスにマップされるため、`lemondata-responses` は `https://api.tokenlab.sh/v1` を使用する必要があります。
  </Accordion>
  <Accordion title="lemondata-claude、lemondata-gemini、lemondata-minimax に /v1 を追加してしまう">
...
- [モデル概要](https://lemondata.cc/ja/models) — エージェントに組み込む前にモデル ID を参照する
