...
TokenLabは、APIコストとレスポンスのレイテンシを大幅に削減できるインテリジェント・キャッシング・システムを提供します。当社のキャッシングは単なるリクエストのマッチングにとどまらず、プロンプトの**セマンティックな意味（意味論的な意味）**を理解します。
...
TokenLabは2層のキャッシング・システムを使用しています：
...
- **ステージ2（フル・コンテキスト）**：会話のコンテキストを含めて95%以上の類似性
