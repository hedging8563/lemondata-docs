...
| 403 | Forbidden - アクセス拒否または許可されていないモデル |
| 404 | Not Found - モデルまたはリソースが見つかりません |
| 413 | Payload Too Large - 入力またはファイルサイズが制限を超過 |
...
| `context_length_exceeded` | 入力がモデルの制限を超えています |
...
<AccordionGroup>
  ...
  <Accordion title="デバッグ用のエラーログ記録">
    サポートのために、リクエストIDを含む完全なエラーレスポンスをログに記録してください：
    ```python
    except APIError as e:
