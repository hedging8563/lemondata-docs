...
<Note>
LemonData では、`ANTHROPIC_BASE_URL` を `/v1` サフィックスなしの `https://api.tokenlab.sh` に設定したままにしてください。これにより、Anthropic スタイルの endpoint 形式および Claude Code で文書化されている統合 gateway パターンに一致します。
</Note>
