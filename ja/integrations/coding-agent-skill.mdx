---
title: "✨ LemonData API 連携スキル"
description: "任意のコーディングエージェント向けに共有の LemonData API 連携スキルをインストールでき、Agent-First のエラーリカバリも利用できます。"
---

<Note>
このスキルは、必要な内容を伝えるだけで、コーディングエージェントに LemonData の **300+ AI API** を組み込む方法を教えます。これは共有の agent-first スキルとして配布され、skill または rules ファイルをサポートする任意のコーディングエージェントで利用できます。

この API は **[Agent-First](/guides/agent-first-api)** です。エージェントがモデル名を間違えて推測しても、エラーレスポンスが修正方法を具体的に教えてくれます。
</Note>
...
<Note>
標準の非対話インストールコマンドを使ってください：

```bash
npx skills add https://github.com/hedging8563/lemondata-skills --skill lemondata-api-integration -y
```

これにより、LemonData skills リポジトリから共有の `lemondata-api-integration` スキルがインストールされます。

ツールがインストーラーをサポートしていない場合は、リポジトリ内の `skills/lemondata-api-integration/` フォルダを、そのツールの共有 skills または rules ディレクトリにコピーしてください。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="どのコーディングエージェントがサポートされていますか？">
    共有の skill または rules ディレクトリをサポートする任意のコーディングエージェントで利用できます。インストーラーに対応している場合、`npx skills add` が共有の `lemondata-api-integration` スキルを検出先へ自動配置します。
  </Accordion>
...
<Info>
**ご質問がありますか？** [GitHub Issues](https://github.com/hedging8563/lemondata-skills/issues)を確認するか、[support@lemondata.cc](mailto:support@lemondata.cc)までお問い合わせください。
</Info>