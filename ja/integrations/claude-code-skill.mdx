...
description: "5分でわかるクイックスタートガイド - Claude Code が LemonData API の統合を自動的に行います"
---

<Note>
このスキルを使用すると、必要な内容を自然な言葉で説明するだけで、LemonData が提供する**数百種類の AI API** を統合できます。
</Note>
...
1. ✅ **LemonData の数百の API を自動的に検索**
2. ✅ **ニーズに最適な API を特定**
3. ✅ **実行可能な完全なコードを生成**
4. ✅ **API Key を設定**
5. ✅ **使用例とベストプラクティスを提供**

**要約すると：自然言語でニーズを伝えるだけで、完全な API 統合コードが手に入ります！**
...
<Tip>
**事前に API Key を設定する必要はありません！** スキルを使用する際、Claude Code が自動的に API Key を尋ね、生成されたコードに含めます。
</Tip>
...
<CardGroup cols={2}>
  <Card title="❌ 絶対にしないでください" icon="xmark" color="#ef4444">
    - フロントエンドのコードに API Key を直接記述する
    - API Key を GitHub にコミットする
    - API Key を他人に共有する
  </Card>
  <Card title="✅ ベストプラクティス" icon="check" color="#22c55e">
    - 環境変数を使用する
    - ウェブサイトの場合はバックエンドから API を呼び出す
...
生成されたコードには基本的なエラーハンドリングが含まれていますが、以下の追加を検討してください：
- リトライメカニズムの追加
- エラーのログ記録
...
<AccordionGroup>
  <Accordion title="スキルが自動的に起動しませんか？">
    リクエストの中で「LemonData」または「LemonData API」に言及するか、ニーズをより具体的に説明してください：
    ```
  ...
  <Accordion title="生成されたコードにエラーがありますか？">
    以下の点を確認してください：
    1. API Key は正しいですか？
    2. 必要なライブラリはインストールされていますか？（例：Python の `openai`）
...
これでスキルの使い方はバッチリです。Claude Code にこう尋ねてみてください：
