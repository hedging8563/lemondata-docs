...
<Warning>
これは実験的な互換構成です。Gemini CLI は現在、LemonData のようなカスタム Base URL ワークフローを安定した形で公式には文書化していません。サポートされた LemonData のターミナル運用が必要なら、Codex CLI、Claude Code、OpenCode を優先してください。
</Warning>
