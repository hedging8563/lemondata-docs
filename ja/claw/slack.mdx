...
デフォルトでは、ClawはSlackに**ペアリング**ポリシーを使用します — ユーザーはボットと対話する前に一度だけペアリングを完了する必要があります。これにより、未承認のワークスペースメンバーがAIクレジットを使用することを防ぎます。

<Tip>
Slack設定を保存すると、ペアリングポリシーが自動的に設定されます。追加の設定は不要です。
</Tip>
