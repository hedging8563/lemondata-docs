...
| `/v1/tasks/{id}` | GET | 動画ジョブの非同期ステータスを取得 |
| `/v1/videos/generations/{id}` | GET | 互換用に残している動画タスクのステータスパス |

<Info>
新しいクライアントでは `/v1/tasks/{id}` を優先し、作成レスポンスで返る `poll_url` をそのまま使ってください。`/v1/videos/generations/{id}` は後方互換のためだけに残しています。
</Info>
