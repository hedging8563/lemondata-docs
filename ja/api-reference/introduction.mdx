...
<Warning>
**インタラクティブPlaygroundについて**：このドキュメントサイトのPlaygroundはデモンストレーション目的のみであり、APIキーの入力には対応していません。APIをテストするには、以下をご利用ください：
- **cURL** - サンプルコマンドをコピーし、`sk-your-api-key` を実際のキーに置き換えてください
- **Postman** - [OpenAPI仕様](/openapi.json)をインポートしてください
- **SDK** - OpenAI/Anthropic SDKを当社のBase URLで使用してください
</Warning>
...
ネイティブGoogle Gemini API形式のサポート：

| エンドポイント | メソッド | 説明 |
|----------|--------|-------------|
| `/v1beta/models/{model}:generateContent` | POST | コンテンツ生成 (Gemini形式) |
| `/v1beta/models/{model}:streamGenerateContent` | POST | ストリーミングコンテンツ生成 (Gemini形式) |

<Note>
Geminiエンドポイントは、標準のBearerトークンに加えて、`?key=`クエリパラメータ認証もサポートしています。
</Note>
