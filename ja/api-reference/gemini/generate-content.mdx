...
<ParamField body="contents" type="array" required>
  ...
  `user` と `model` の role 値は大文字小文字を区別せず正規化されます。`application/octet-stream` の `inlineData` / `inline_data` は、LemonData が対応する画像または動画バイト列として識別できる場合のみ受け付けます。それ以外はルーティング前に失敗します。ネイティブ画像出力リクエストでは Google search/maps 系ツールのみ受け付け、非対応のツール組み合わせは上流リトライ前に失敗します。
</ParamField>
