...
Gemini `generateContent` のネイティブ SSE streaming 版です。TokenLab は upstream のイベント、順序、フィールドを保持します。metadata のみのイベント、`finishReason` のない中間 chunk、自然な EOF はすべて有効です。Chat Completions の `[DONE]` を追加せず、Chat にフォールバックしません。
## パスパラメータ
...
`generationConfig.candidateCount`、ツール構成、将来のフィールドは変更せず upstream へ転送し、TokenLab 独自の streaming 制限は設けません。
