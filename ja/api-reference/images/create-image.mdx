...
description: "prompt を指定して画像を作成します"
---
...
<ResponseField name="data" type="array">
  ...
  各オブジェクトには以下が含まれます:
  - `url` (string): 作成された画像の URL
  - `b64_json` (string): Base64 エンコードされた画像（要求した場合）
  - `revised_prompt` (string): 使用された prompt（DALL-E 3）
</ResponseField>

### 非同期レスポンス（Midjourney、Flux Pro、Ideogram など）

一部のモデルでは処理時間が長くなるため、非同期レスポンスが返されます:

<Info>
`poll_url` が返された場合は、その URL をそのまま使ってください。`task_id` は後続のポーリングで使う非同期タスク ID として扱います。
</Info>
...
<Note>
`status: "pending"` を受け取った場合は、結果をポーリングするために [Get Image Status](/api-reference/images/get-image-status) エンドポイントを使用してください。
</Note>
...
| モデル | タイプ | 機能 |
|-------|------|----------|
| `dall-e-3` | 同期 | 最高品質、prompt 強化 |
| `dall-e-2` | 同期 | より高速、より低コスト |
| `flux-pro` | **非同期** | フォトリアル、高品質 |
| `flux-schnell` | 同期 | 非常に高速 |
| `midjourney` | **非同期** | 芸術的なスタイル |
| `ideogram-v3` | **非同期** | 最高のテキストレンダリング |
| `stable-diffusion-3` | 同期 | オープンソース、カスタマイズ可能 |

<Tip>
**非同期モデル** は `status: "pending"` を返し、ポーリングが必要です。結果の取得方法については [Get Image Status](/api-reference/images/get-image-status) を参照してください。
</Tip>

## 非同期レスポンスの処理

非同期モデルでは、レスポンスに `status: "pending"` が含まれているかを確認してください:
