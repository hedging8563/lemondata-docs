...
- `gemini-3-pro-image-preview` と `nano-banana-pro` は `aspect_ratio` と `resolution`（`1k`、`2k`、`4k`）をサポートしています。
- `gemini-2.5-flash-image`、`gemini-3.1-flash-image-preview`、`nano-banana`、および `nano-banana-edit` は `aspect_ratio` をサポートしますが、公開されている `resolution` の選択肢はありません。
...
<ParamField body="resolution" type="string">
  ...
  `gemini-3-pro-image-preview`、`nano-banana-pro`、`nano-banana-2`、および同種の高解像度ファミリーでサポートされます。一般的な値は `1k`、`2k`、`4k` です。モデル側で明記されていない限り、Gemini Flash 画像ファミリーには送信しないでください。xAI Grok Imagine 画像モデルでは `1k` または `2k` を使用してください。
</ParamField>
...
Flashファミリーの例: `gemini-2.5-flash-image` または `gemini-3.1-flash-image-preview` では、`aspect_ratio` を送信し、`resolution` は省略します:
