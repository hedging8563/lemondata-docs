...
<ParamField body="model" type="string" required>
  使用するモデル（例: `gpt-image-2`、`flux-pro`、`qwen-image`、`nano-banana-pro`）。現在の推奨リストは `GET /v1/models?recommended_for=image` で確認してください。
</ParamField>
...
| `qwen-image` | 多くの場合タスクベース | テキストレンダリングとプロンプト追従が強い |
