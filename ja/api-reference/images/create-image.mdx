...
<ParamField body="size" type="string" default="1024x1024">
  画像サイズ。OpenAI 形式の画像ファミリーや、正確なピクセルサイズを受け付ける他のモデルに使用します。

  Google Gemini 画像ファミリーでは、`size` は互換用の別名として扱われ、モデルの公開 `aspect_ratio` 契約と、対応している場合は `resolution` 契約にマッピングされます。これらのモデルには `aspect_ratio` を直接送ることを推奨します。
</ParamField>
