...
<Warning>
このエンドポイントは現在、DALL-E スタイルの multipart 編集フローを文書化しています。LemonData の他の対応画像-to-画像ファミリーは `POST /v1/images/generations` を使い、対応している場合は `operation: "image-to-image"` と `image_url` のような JSON 入力で呼び出します。
</Warning>
