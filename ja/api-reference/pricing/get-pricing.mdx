...
<ParamField query="model" type="string">
  料金を取得するモデルの ID。指定しない場合、すべてのモデルの料金を返します。
</ParamField>
...
<ResponseField name="data" type="array">
  ...
  - `model` (string): モデル識別子
  - `pricing` (object): 料金情報
    - `input_per_1m` (string | null): 100万入力 token あたりの USD
    - `output_per_1m` (string | null): 100万出力 token あたりの USD
    - `per_request` (string | null): リクエストあたりの USD（固定料金モデルの場合）
...
| タイプ | 説明 | モデルの例 |
|------|-------------|----------------|
| **トークンベース** | 入力/出力 token ごとに課金 | GPT-4o, Claude, Gemini |
| **リクエスト単位** | リクエストごとの固定料金 | DALL-E 3, Sora, Suno |