...
<Info>
Seedance 2.0 モデルでは、TokenLab が生成前に画像フィールド（`image`, `image_url`, `image_urls`, `reference_images`, `start_image`, `end_image`）を再利用可能なバーチャルアバター素材として自動準備します。60 秒以内に準備が完了しない場合、API は `auto_material_asset_ids` 付きの `409 seedance_material_preparing` を返します。素材が `ACTIVE` になってから再試行してください。
</Info>
...
<ParamField body="draft" type="boolean">
  Seedance 1.5 Pro の Draft ワークフロー用フラグです。Draft 対応の Seedance ルートで `draft=true` を指定するとドラフトタスクを作成します。`draft_task_id` と同時に送信しないでください。
</ParamField>
