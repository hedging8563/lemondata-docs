...
<ParamField body="output_audio" type="boolean">
  モデル依存の音声出力トグルです。TokenLab では Veo 3 系列は省略時に `true` として扱われます。`kling-3.0-video` は要素参照を使わないリクエストでこの selector を受け付け、そのモデルの互換 sound 制御を使用します。Kling リクエストで省略した場合は無音が既定です。`output_audio=true` と `kling_elements` は併用できません。他の動画モデルは各ドキュメントの既定値を使用します。camelCase の別名 `outputAudio` も受け付けます。
</ParamField>
