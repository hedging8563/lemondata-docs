...
対応モデルは `marble-1.0`、`marble-1.1`、`marble-1.1-plus` です。World Labs のドキュメントには `marble-1.0-draft` も記載されていますが、TokenLab は現時点でこのエンドポイントでは draft 生成を公開していません。
...
このエンドポイントはワールド生成とポーリングを対象とします。TokenLab は現時点で World Labs のメディアアセットアップロード、ワールド一覧、ワールド削除、単体 pano ツールはプロキシしません。
