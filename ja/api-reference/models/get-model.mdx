...
<ResponseField name="lemondata" type="object">
  LemonData 固有の公開メタデータです。詳細ルートでは、`category`、`pricing_unit`、`capability_flags`、`supported_operations`、`pricing_provenance`、`public_contract_summary`、完全な `public_contract`、そして非チャット推奨スナップショット用のシーン別 `agent_preferences` を含めることができます。
</ResponseField>
...
非チャットモデルの場合、詳細ルートは `lemondata.agent_preferences.<scene>` も返します。
