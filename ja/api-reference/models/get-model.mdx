...
<ResponseField name="lemondata" type="object">
  TokenLab 固有の公開メタデータです。詳細ルートでは、`category`、`pricing_unit`、`capability_flags`、`supported_operations`、`pricing_provenance`、`public_contract_summary`、完全な `public_contract`、そして非チャット推奨スナップショット用のシーン別 `agent_preferences` を含めることができます。
</ResponseField>
