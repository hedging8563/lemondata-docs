...
TokenLab expone esta ruta solo para modelos que anuncian un contrato Gemini nativo y tienen una ruta upstream del mismo protocolo; no hay fallback a Chat Completions. Los nombres ProtoJSON en lowerCamelCase y los nombres proto originales en snake_case son oficiales y se conservan incluso en solicitudes mixtas. Los campos desconocidos se reenvían con el mejor esfuerzo y el upstream decide si los admite.
## Parámetros de Ruta
...
<ParamField body="contents" type="array" required>
  Contenido de la conversación. Los valores de `role` y todos los `parts` se conservan tal como se enviaron; TokenLab no aplica trim/lowercase, reescritura Base64 ni inferencia MIME. `inlineData` / `inline_data` y `fileData` / `file_data` son grafías oficiales, y TokenLab no exige localmente `fileData.mimeType`. Las IDs de Files o Cache creadas por TokenLab se someten a comprobación de propiedad y mantienen su vínculo upstream; el proveedor decide las demás reglas de medios y herramientas.
</ParamField>
...
<ParamField body="generationConfig" type="object">
  Configuración nativa de generación. Se aceptan sin cambios nombres lowerCamelCase como `maxOutputTokens`, `candidateCount` y `thinkingConfig`, así como sus equivalentes snake_case. TokenLab no elimina `thinkingConfig` después de un fallo ni impone un límite local de `candidateCount` para streaming; el upstream decide los rangos y combinaciones.
</ParamField>
