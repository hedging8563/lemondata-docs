...
<ParamField path="model" type="string" required>
  ...
  TokenLab usa la ruta nativa de Gemini cuando está disponible y cambia a una ruta pública compatible cuando el manejo nativo no está disponible para esa solicitud multimodal.
...
<ParamField body="contents" type="array" required>
  ...
  Para las partes multimedia, TokenLab acepta actualmente tipos MIME de imagen, audio y video, y los reenvía mediante los detalles de un modelo compatible con Gemini.
...
<ParamField body="generationConfig" type="object">
  ...
  - `candidateCount` (integer): Número de candidatos para generación sin streaming. Las solicitudes con streaming deben omitirlo o mantenerlo en `1`.
  - `responseModalities` (array): Modalidades de salida solicitadas para rutas nativas compatibles.
  - `responseMimeType` (string): Tipo MIME de salida, como `text/plain` o `application/json`.
  - `responseSchema` (object): JSON schema para salida estructurada cuando `responseMimeType` solicita JSON.
  - `thinkingConfig` / `thinking_config` (object): Opciones de presupuesto de razonamiento para modelos compatibles.
</ParamField>
