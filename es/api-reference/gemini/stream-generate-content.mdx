...
Variante SSE nativa y en streaming de Gemini `generateContent`. TokenLab conserva los eventos, el orden y los campos del upstream; son válidos los eventos solo de metadatos, los fragmentos intermedios sin `finishReason` y el EOF natural. No añade el marcador `[DONE]` de Chat Completions ni vuelve a Chat.
## Parámetros de ruta
...
`generationConfig.candidateCount`, las combinaciones de herramientas y los campos futuros se reenvían sin cambios al upstream; TokenLab no impone un límite propio para streaming.
