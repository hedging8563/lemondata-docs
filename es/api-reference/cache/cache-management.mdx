...
TokenLab gestiona automáticamente el almacenamiento en caché para optimizar el rendimiento y reducir costes. Aunque no existe un endpoint público para borrar las entradas de caché, tienes un control total sobre el comportamiento del almacenamiento en caché a través de controles a nivel de solicitud.

Esta página es una guía de caché del lado de la solicitud.

Para las páginas estrictas de referencia de la API, consulta:

- [Enviar comentarios sobre la caché](/api-reference/cache/submit-cache-feedback)
- [Obtener estadísticas de comentarios de caché](/api-reference/cache/get-cache-feedback-stats)
...
| Tipo | Efecto |
|------|--------|
| `no_cache` | Omite la búsqueda y el almacenamiento en caché; siempre obtiene una respuesta actualizada |
...
Usa el identificador de request/cache entry con cache hit de tu propia organización; los identificadores arbitrarios se rechazan.
