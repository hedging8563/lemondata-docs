...
Los campos opcionales admitidos varían según la familia de modelo. TokenLab valida esta matriz antes de enrutar: OpenAI TTS acepta `voice`, `instructions`, `response_format`, `stream_format` y `speed`; MiniMax speech acepta `voice`, `voice_id`, `response_format`, `stream_format` y `speed`; Gemini TTS acepta `prompt`, `language_code`, `voice`, `response_format`, `stream_format`, `speed` y `temperature`.
