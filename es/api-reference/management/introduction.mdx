...
description: "Administra las claves API de la organización y consulta uso y facturación por clave mediante un token de gestión."
---

## Resumen

La API de gestión te permite administrar las claves API de la organización y consultar uso y facturación por clave sin usar una clave estándar de inferencia.
...
- Por defecto, `limitAmount` en la solicitud se interpreta como USD. Usa `limitCurrency: "CNY"` para enviar un límite en RMB; el servidor lo convierte a USD usando el tipo de cambio actual antes de almacenarlo.
- Los campos monetarios de respuesta conservan el valor original en USD y añaden un equivalente en RMB con el sufijo `_cny`.
- `exchange_rate` se devuelve junto a los metadatos de la API key y representa la tasa USD→CNY usada para cada valor `*_cny` de esa respuesta.
