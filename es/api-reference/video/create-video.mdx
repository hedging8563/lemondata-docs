...
La salida de audio depende del modelo. En TokenLab, la familia Veo 3 activa audio por defecto cuando se omite `output_audio`. Si un modelo admite control de audio, actívalo o desactívalo explícitamente con `output_audio`. El alias camelCase `outputAudio` también se acepta por compatibilidad.
...
<ParamField body="output_audio" type="boolean">
  Selector de salida de audio dependiente del modelo. En TokenLab, las solicitudes de la familia Veo 3 usan `true` por defecto cuando se omite este campo. `kling-3.0-video` acepta este selector para solicitudes sin referencias de elementos y lo asigna al control de sonido compatible de ese modelo; las solicitudes Kling omitidas son silenciosas por defecto. No combines `output_audio=true` con `kling_elements`. Otros modelos de video usan sus valores predeterminados documentados. También se acepta el alias camelCase `outputAudio` por compatibilidad.
</ParamField>
