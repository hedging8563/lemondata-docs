...
<ParamField body="model" type="string" default="sora-2">
  ID del modelo de video. El valor predeterminado de la API es `sora-2`. Consulta la [guía de generación de video](/es/guides/video-generation) para ver la matriz pública actual y las capacidades compatibles.
</ParamField>
...
<ParamField body="kling_elements" type="array">
  Definiciones de referencias de elementos para `kling-3.0-video`. Solo se admiten en solicitudes con imagen de referencia. Define de 1 a 3 elementos; cada elemento incluye `name`, `description` opcional y `element_input_urls` con 2-4 URLs de imagen. Referencia el elemento en `prompt` como `@name`.
</ParamField>
...
<ParamField body="video_urls" type="array">
  Entradas adicionales de video de referencia para modelos con condicionamiento multimodal. La cantidad admitida depende del modelo. Para `seedance-2.0` y `seedance-2.0-fast`, LemonData admite actualmente hasta 3 videos de referencia.
</ParamField>
...
<ParamField body="audio_urls" type="array">
  Entradas adicionales de audio de referencia para modelos con condicionamiento multimodal. La cantidad admitida depende del modelo. Para `seedance-2.0` y `seedance-2.0-fast`, LemonData admite actualmente hasta 3 audios de referencia.
</ParamField>
...
<ParamField body="output_audio" type="boolean">
  Interruptor de salida de audio dependiente del modelo. En LemonData, la familia Veo 3 usa `true` por defecto cuando este campo se omite. `kling-3.0-video` también acepta este selector y lo asigna al control `sound` del upstream compatible; las solicitudes Kling omitidas siguen siendo silenciosas por defecto. El alias camelCase `outputAudio` también se acepta por compatibilidad.
</ParamField>
...
<ResponseField name="billing_transaction_id" type="string">
  ID de transacción de facturación de LemonData cuando la liquidación ya terminó. Es el identificador de dashboard/contabilidad y es distinto del `id` / `task_id` asíncrono.
</ResponseField>
...
Usa `kling_elements` con `kling-3.0-video` cuando necesites referencias de elementos. La solicitud debe incluir una entrada de imagen (`image_url`, `image_urls`, `start_image` o `end_image`) y referenciar cada elemento en el prompt con `@name`.
...
Usa `operation=reference-to-video` cuando el modelo admita condicionamiento de referencia dedicado. En el contrato público de LemonData, las referencias de imagen se envían mediante `reference_images`, mientras que los videos y audios de referencia multimodales se envían mediante `video_urls` y `audio_urls`. Para `seedance-2.0` y `seedance-2.0-fast`, LemonData admite actualmente hasta 9 imágenes de referencia, además de hasta 3 videos de referencia y 3 audios de referencia. `duration` solo controla la duración del resultado generado; no fija un límite independiente para la duración del video de referencia de entrada.
