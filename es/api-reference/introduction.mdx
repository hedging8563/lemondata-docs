...
| `/v1/tasks/{id}` | GET | Obtener el estado asíncrono de tareas de video |
| `/v1/videos/generations/{id}` | GET | Ruta de estado de video compatible con versiones anteriores |

<Info>
Para clientes nuevos, prioriza `/v1/tasks/{id}` y sigue el `poll_url` devuelto por las respuestas de creación. Conserva `/v1/videos/generations/{id}` solo por compatibilidad hacia atrás.
</Info>
