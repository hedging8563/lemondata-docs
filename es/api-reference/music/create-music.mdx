---
title: "Crear música"
openapi: "POST /v1/music/generations"
description: "Crea una tarea de generación de música utilizando Suno"
---

Genera música y letras utilizando IA. Esta es una API asíncrona. La respuesta de creación devuelve una identidad de tarea y, cuando está disponible, también puede incluir un `poll_url` preferido.

## Cuerpo de la solicitud

<ParamField body="model" type="string" default="suno_music">
  Modelo a utilizar: `suno_music` para generación de música, `suno_lyrics` solo para letras.
</ParamField>

<ParamField body="prompt" type="string">
  Descripción de la música. Obligatorio para solicitudes music, lyrics, upload-cover y upload-extend; omítelo para add-instrumental.
</ParamField>

<ParamField body="title" type="string">
  Título para la canción generada.
</ParamField>

<ParamField body="tags" type="string">
  Etiquetas de estilo (por ejemplo, "pop, upbeat, electronic").
</ParamField>

<ParamField body="negative_tags" type="string">
  Estilos que se deben evitar. Es obligatorio cuando `audio_operation` es `add-instrumental`; se envía a KIE como `negativeTags`.
</ParamField>

<ParamField body="action" type="string">
  Tipo de generación: `MUSIC` (predeterminado) o `LYRICS`.
</ParamField>

<ParamField body="mv" type="string">
  Versión oficial del modelo Suno. Obligatoria para la generación de música (cuando se omite `action` o es `MUSIC`); omítela en solicitudes solo de letras. Valores admitidos: `chirp-v3-5`, `chirp-v4`, `chirp-v4-5`, `chirp-v4.5`, `chirp-v4-5plus`, `chirp-v4.5plus`, `chirp-v4-5all`, `chirp-v4.5all`, `chirp-v5`, `chirp-v5-5`, `chirp-v5.5`.
</ParamField>

<ParamField body="make_instrumental" type="boolean">
  Establécelo en `true` para generar una salida instrumental sin voces. TokenLab asigna este campo público al indicador `instrumental` de KIE. Usa nombres de campo de TokenLab como `make_instrumental`, `audio_url`, `negative_tags` y `continue_at`; los nombres upstream de KIE como `instrumental`, `uploadUrl`, `negativeTags` y `continueAt` se rechazan.
</ParamField>

<ParamField body="audio_url" type="string">
  URL de audio de referencia o subido que sea accesible públicamente. Si se envía sin `audio_operation`, TokenLab usa el flujo upload-cover.
</ParamField>

<ParamField body="audio_operation" type="string">
  Modo de audio subido: `upload-cover`, `upload-extend` o `add-instrumental`. `upload-extend` requiere `continue_at`; `add-instrumental` requiere `audio_url`, `title`, `tags` y `negative_tags`.
</ParamField>

<ParamField body="continue_clip_id" type="string">
  ID de un clip anterior desde el cual continuar.
</ParamField>

<ParamField body="continue_at" type="number">
  Marca de tiempo en segundos desde la que continuar. Obligatorio cuando `audio_operation` es `upload-extend`.
</ParamField>

## Respuesta

<ResponseField name="id" type="string">
  ID de la tarea para consultar el estado.
</ResponseField>

<ResponseField name="task_id" type="string">
  Alias del identificador de tarea asíncrona cuando el adaptador lo devuelve.
</ResponseField>

<ResponseField name="poll_url" type="string">
  URL de polling prioritaria cuando está disponible.
</ResponseField>

<ResponseField name="status" type="string">
  Estado de la tarea: `pending`, `processing`, `completed` o `failed`.
</ResponseField>

<ResponseField name="created" type="integer">
  Marca de tiempo Unix de la creación de la tarea.
</ResponseField>

<RequestExample>
```bash cURL
curl -X POST "https://api.tokenlab.sh/v1/music/generations" \
  -H "Authorization: Bearer sk-your-api-key" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "suno_music",
    "mv": "chirp-v4",
    "prompt": "An upbeat electronic dance track with heavy bass",
    "title": "Night Drive",
    "tags": "electronic, EDM, energetic"
  }'
```

```python Python
import requests

response = requests.post(
    "https://api.tokenlab.sh/v1/music/generations",
    headers={"Authorization": "Bearer sk-your-api-key"},
    json={
        "model": "suno_music",
        "mv": "chirp-v4",
        "prompt": "An upbeat electronic dance track with heavy bass",
        "title": "Night Drive",
        "tags": "electronic, EDM, energetic"
    }
)

task_id = response.json()["id"]
print(f"Task ID: {task_id}")
```

```javascript JavaScript
const response = await fetch('https://api.tokenlab.sh/v1/music/generations', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer sk-your-api-key',
    'Content-Type': 'application/json'
  },
  body: JSON.stringify({
    model: 'suno_music',
    mv: 'chirp-v4',
    prompt: 'An upbeat electronic dance track with heavy bass',
    title: 'Night Drive',
    tags: 'electronic, EDM, energetic'
  })
});

const data = await response.json();
console.log(`Task ID: ${data.id}`);
```

```go Go
package main

import (
    "bytes"
    "encoding/json"
    "fmt"
    "net/http"
)

func main() {
    payload := map[string]interface{}{
        "model":  "suno_music",
        "mv":     "chirp-v4",
        "prompt": "An upbeat electronic dance track with heavy bass",
        "title":  "Night Drive",
        "tags":   "electronic, EDM, energetic",
    }
    body, _ := json.Marshal(payload)

    req, _ := http.NewRequest("POST", "https://api.tokenlab.sh/v1/music/generations", bytes.NewBuffer(body))
    req.Header.Set("Authorization", "Bearer sk-your-api-key")
    req.Header.Set("Content-Type", "application/json")

    client := &http.Client{}
    resp, _ := client.Do(req)
    defer resp.Body.Close()

    var result map[string]interface{}
    json.NewDecoder(resp.Body).Decode(&result)
    fmt.Printf("Task ID: %s\n", result["id"])
}
```

```php PHP
<?php
$ch = curl_init('https://api.tokenlab.sh/v1/music/generations');

curl_setopt_array($ch, [
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_POST => true,
    CURLOPT_HTTPHEADER => [
        'Content-Type: application/json',
        'Authorization: Bearer sk-your-api-key'
    ],
    CURLOPT_POSTFIELDS => json_encode([
        'model' => 'suno_music',
        'mv' => 'chirp-v4',
        'prompt' => 'An upbeat electronic dance track with heavy bass',
        'title' => 'Night Drive',
        'tags' => 'electronic, EDM, energetic'
    ])
]);

$response = curl_exec($ch);
curl_close($ch);

$data = json_decode($response, true);
echo "Task ID: " . $data['id'];
```
</RequestExample>

<ResponseExample>
```json Response
{
  "id": "ldtask_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
  "task_id": "ldtask_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
  "poll_url": "/v1/tasks/ldtask_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
  "status": "pending",
  "created": 1706000000,
  "model": "suno_music"
}
```
</ResponseExample>
