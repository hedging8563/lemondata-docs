...
<ParamField body="model" type="string" required>
  Modelo a usar (por ejemplo, `gpt-image-2`, `flux-pro`, `qwen-image` o `nano-banana-pro`). Consulta `GET /v1/models?recommended_for=image` para la lista recomendada actual.
</ParamField>
...
<ParamField body="response_format" type="string" default="url">
  ...
  Para rutas `gpt-image-2` de Azure Official o compatibles con Azure, TokenLab no reenvía `response_format` al upstream. El gateway siempre recibe los datos de imagen upstream como `b64_json`; para solicitudes `url`, sube cada imagen al CDN y devuelve `data[].url`. Si el almacenamiento CDN no está disponible o la subida falla, la solicitud falla en vez de degradar a Base64. Para `b64_json`, devuelve el Base64 sin procesar.
</ParamField>
...
| `qwen-image` | A menudo basado en tareas | Buen renderizado de texto y seguimiento del prompt |
