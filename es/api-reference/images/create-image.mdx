...
Para generación de imágenes con `gpt-image-2`, los parámetros públicos admitidos son `prompt`, `n`, `size`, `quality`, `response_format`, `async`, `background`, `output_format`, `output_compression` o `compression`, `moderation`, `partial_images` y `user`. Si omites `size` o `quality`, LemonData usa `auto`; los valores personalizados de `size` deben usar el contrato flexible `WIDTHxHEIGHT` documentado abajo.

Nota de compatibilidad: si una solicitud `gpt-image-2` incluye `input_fidelity`, LemonData lo elimina antes de reenviarla porque GPT Image 2 ya procesa las entradas de imagen con alta fidelidad.
...
<ParamField body="response_format" type="string" default="url">
  ...
  Para rutas `gpt-image-2` de Azure Official o compatibles con Azure, LemonData no reenvía `response_format` al upstream. El gateway siempre recibe los datos de imagen upstream como `b64_json`; para solicitudes `url`, sube cada imagen al CDN y devuelve `data[].url`. Si el almacenamiento CDN no está disponible o la subida falla, la solicitud falla en vez de degradar a Base64. Para `b64_json`, devuelve el Base64 sin procesar.
</ParamField>
