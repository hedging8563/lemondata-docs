---
title: "Crear modelo 3D"
openapi: "POST /v1/3d/generations"
description: "Crear una tarea de generación de modelos 3D"
---

Genera modelos 3D a partir de texto o imágenes utilizando Tripo3D y otros proveedores. Esta es una API asíncrona.

## Cuerpo de la solicitud (Request Body)

<ParamField body="model" type="string" default="tripo3d-v2.5">
  El modelo a utilizar (por ejemplo: `tripo3d-v2.5`, `tripo3d-v2`).
</ParamField>

<ParamField body="prompt" type="string" required>
  Descripción textual del modelo 3D a generar.
</ParamField>

<ParamField body="image" type="string">
  Imagen codificada en Base64 para la generación de imagen a 3D.
</ParamField>

<ParamField body="image_url" type="string">
  URL de la imagen para la generación de imagen a 3D.
</ParamField>

<ParamField body="format" type="string" default="glb">
  Formato de salida: `glb`, `fbx`, `obj` o `usdz`.
</ParamField>

<ParamField body="quality" type="string" default="standard">
  Nivel de calidad: `draft`, `standard` o `high`.
</ParamField>

<ParamField body="style" type="string">
  Preajuste de estilo del modelo.
</ParamField>

<ParamField body="seed" type="integer">
  Valor de semilla para una generación reproducible.
</ParamField>

<ParamField body="user" type="string">
  Identificador único del usuario final.
</ParamField>

## Respuesta (Response)

<ResponseField name="id" type="string">
  ID de la tarea para consultar el estado.
</ResponseField>

<ResponseField name="status" type="string">
  Estado de la tarea: `pending`, `processing`, `completed` o `failed`.
</ResponseField>

<ResponseField name="created" type="integer">
  Marca de tiempo Unix de la creación de la tarea.
</ResponseField>

<RequestExample>
```bash cURL
curl -X POST "https://api.lemondata.cc/v1/3d/generations" \
  -H "Authorization: Bearer sk-your-api-key" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "tripo3d-v2.5",
    "prompt": "A detailed medieval castle with towers",
    "format": "glb",
    "quality": "high"
  }'
```

```python Python
import requests

response = requests.post(
    "https://api.lemondata.cc/v1/3d/generations",
    headers={"Authorization": "Bearer sk-your-api-key"},
    json={
        "model": "tripo3d-v2.5",
        "prompt": "A detailed medieval castle with towers",
        "format": "glb",
        "quality": "high"
    }
)

task_id = response.json()["id"]
print(f"Task ID: {task_id}")
```

```javascript JavaScript
const response = await fetch('https://api.lemondata.cc/v1/3d/generations', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer sk-your-api-key',
    'Content-Type': 'application/json'
  },
  body: JSON.stringify({
    model: 'tripo3d-v2.5',
    prompt: 'A detailed medieval castle with towers',
    format: 'glb',
    quality: 'high'
  })
});

const data = await response.json();
console.log(`Task ID: ${data.id}`);
```

```go Go
package main

import (
    "bytes"
    "encoding/json"
    "fmt"
    "net/http"
)

func main() {
    payload := map[string]interface{}{
        "model":   "tripo3d-v2.5",
        "prompt":  "A detailed medieval castle with towers",
        "format":  "glb",
        "quality": "high",
    }
    body, _ := json.Marshal(payload)

    req, _ := http.NewRequest("POST", "https://api.lemondata.cc/v1/3d/generations", bytes.NewBuffer(body))
    req.Header.Set("Authorization", "Bearer sk-your-api-key")
    req.Header.Set("Content-Type", "application/json")

    client := &http.Client{}
    resp, _ := client.Do(req)
    defer resp.Body.Close()

    var result map[string]interface{}
    json.NewDecoder(resp.Body).Decode(&result)
    fmt.Printf("Task ID: %s\n", result["id"])
}
```

```php PHP
<?php
$ch = curl_init('https://api.lemondata.cc/v1/3d/generations');

curl_setopt_array($ch, [
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_POST => true,
    CURLOPT_HTTPHEADER => [
        'Content-Type: application/json',
        'Authorization: Bearer sk-your-api-key'
    ],
    CURLOPT_POSTFIELDS => json_encode([
        'model' => 'tripo3d-v2.5',
        'prompt' => 'A detailed medieval castle with towers',
        'format' => 'glb',
        'quality' => 'high'
    ])
]);

$response = curl_exec($ch);
curl_close($ch);

$data = json_decode($response, true);
echo "Task ID: " . $data['id'];
```
</RequestExample>

<ResponseExample>
```json Response
{
  "id": "3d_abc123",
  "status": "pending",
  "created": 1706000000,
  "model": "tripo3d-v2.5"
}
```
</ResponseExample>