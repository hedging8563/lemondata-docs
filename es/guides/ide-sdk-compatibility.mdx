...
description: "Guía de compatibilidad respaldada por evidencia para herramientas de IA para programación, SDKs y frameworks"
---

## Descripción general

TokenLab expone múltiples formatos de API para que las herramientas de programación, SDKs y frameworks comunes puedan integrarse con un mínimo de código de conexión.

Esta página es intencionalmente más específica que una matriz de marketing:

- **Soportado (Supported)** significa que documentamos una ruta de configuración concreta y TokenLab expone la forma del protocolo que dicha ruta espera.
- **Ruta nativa sólida (Strong native path)** significa que el repositorio también cuenta con evidencia de adaptadores directos o formatos de solicitud para esa familia de protocolos.
- **Mejor esfuerzo (Best-effort)** significa que la integración puede funcionar, pero el cliente upstream no trata este flujo de trabajo de gateway personalizado como un contrato estable.
...
|------|---------------|--------|-------|
| **Cursor** | Soportado con límites | Compatible con OpenAI | Funciona para flujos estándar de chat/editor BYOK, no como reemplazo de funciones gestionadas por Cursor como el autocompletado de pestañas |
| **Claude Code CLI** | Ruta nativa sólida | Anthropic | Ruta nativa `/v1/messages` con cobertura de adaptador para razonamiento (thinking) y `tool_choice` |
...
| **OpenCode** | Soportado | Compatible con OpenAI | Utilice un proveedor compatible con OpenAI por defecto; cambie a un proveedor basado en Responses solo cuando lo necesite explícitamente |
...
|-----------------|----------|---------------|-------|
| **OpenAI SDK** | Python/JS/Go | Ruta principal soportada | Ruta de compatibilidad para Chat Completions y Embeddings de `/v1`; utilice rutas de proveedor nativas para comportamientos específicos del proveedor, y no asuma que los campos exclusivos de Responses funcionan en todos los modelos |
| **Anthropic SDK** | Python/JS | Ruta nativa sólida | Ruta nativa Messages con evidencia directa para herramientas, razonamiento (thinking) y caché de prompts |
| **Vercel AI SDK** | TypeScript | Patrón de integración recomendado | Prefiera `@ai-sdk/openai-compatible`; utilice `@ai-sdk/openai` solo cuando desee explícitamente un comportamiento nativo de Responses |
...
| **LlamaIndex** | Python | Soportado vía `OpenAILike` | Utilice `OpenAILike`, no las clases integradas de OpenAI, para gateways de terceros como TokenLab |
| **OpenAI Agents SDK** | Python | Ruta de chat-completions soportada | Utilice `AsyncOpenAI(base_url="https://api.tokenlab.sh/v1")` con `OpenAIChatCompletionsModel`; el comportamiento nativo de Responses depende del modelo y la ruta seleccionados |
| **Langflow** | Python / Web | Soportado con límites de alcance | Utilice el campo **OpenAI API Base** del componente de OpenAI con el endpoint `/v1` de TokenLab |
...
| **Flowise** | Web / Node | Soportado con límites de alcance | Utilice el nodo de chat de TokenLab cuando esté disponible, o un nodo compatible con ChatOpenAI apuntando a `https://api.tokenlab.sh/v1` |
| **Agno** | Python | Ruta soportada | Utilice el wrapper de modelo de TokenLab cuando esté disponible, o la ruta de URL base compatible con OpenAI |
| **Promptfoo** | CLI / Node | Ruta soportada | Utilice proveedores de chat o Responses de OpenAI con `apiBaseUrl: https://api.tokenlab.sh/v1` |
| **Portkey Gateway** | Gateway | Ruta soportada | Utilice TokenLab como proveedor upstream para tráfico de chat y Responses compatible con OpenAI |
| **Helicone** | Gateway / Observabilidad | Ruta de observabilidad soportada | Enrute solicitudes compatibles con OpenAI a través de Helicone Gateway con `Helicone-Target-Url: https://api.tokenlab.sh` |
| **LangBot** | Plataforma de bots | Ruta de chat soportada | Utilice el solicitante de TokenLab cuando esté disponible, o configure un solicitante compatible con OpenAI con el endpoint `/v1` de TokenLab |
| **Open WebUI** | Web Chat | Ruta de aplicación de chat soportada | Agregue TokenLab como una conexión API compatible con OpenAI y actualice la lista de modelos |
| **Chatbox / DeepChat / Jan / LibreChat / Cherry Studio** | Escritorio / Web | Ruta de aplicación de chat soportada | Utilice el flujo de proveedor personalizado compatible con OpenAI de cada aplicación; estas aplicaciones suelen estar orientadas a chat-completions |
...
| `top_logprobs` | integer | Número de probabilidades logarítmicas superiores (0-20) |
...
| `prediction` | object | Salida predicha para completado más rápido |
...
# Seguimiento de uso en Chat Completions
...
<AccordionGroup>
  <Accordion title="Utilice passthrough para parámetros desconocidos">
    Los parámetros desconocidos solo se reenvían cuando la ruta pública y el modelo seleccionados los soportan.
  </Accordion>

  <Accordion title="Utilice stream_options.include_usage solo para Chat Completions">
  ...
  <Accordion title="Utilice el formato de tool_choice apropiado">
