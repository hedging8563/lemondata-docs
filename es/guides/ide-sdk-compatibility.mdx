...
TokenLab expone múltiples formatos de API para que las herramientas de codificación comunes, SDKs y frameworks puedan integrarse con una cantidad mínima de código puente.
...
| **Gemini CLI** | Mejor esfuerzo / experimental | Gemini | El flujo personalizado de URL base de TokenLab no es una integración oficial de Gemini CLI |
...
| Parámetro | Tipo | Descripción |
|-----------|------|-------------|
| `stream_options` | object | Solo Chat Completions: `{ include_usage: true }` para los recuentos de tokens |
...
Los endpoints de generación que exponen `stream: true`, incluidos Chat Completions y Responses, usan Server-Sent Events (SSE):
...
<AccordionGroup>
  ...
  <Accordion title="Usa stream_options.include_usage solo en Chat Completions">
    En streaming de Chat Completions, activa `stream_options.include_usage` para obtener recuentos de tokens precisos. Responses tiene su propio contrato de streaming y no expone esta opción exclusiva de Chat.
  </Accordion>
