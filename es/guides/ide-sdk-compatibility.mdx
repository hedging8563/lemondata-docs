...
description: "Guía de compatibilidad respaldada por evidencia para herramientas de codificación, SDKs y frameworks de IA"
---
...
- **Ruta nativa sólida** significa que el repositorio también cuenta con evidencia de adaptador directo o formato de solicitud para esa familia de protocolos.
...
|-----------------|----------|---------------|-------|
| **OpenAI SDK** | Python/JS/Go | Ruta principal soportada | Ruta de compatibilidad para Chat Completions y Embeddings de `/v1`; use rutas de proveedor nativas para comportamientos específicos del proveedor, y no asuma que los campos exclusivos de Responses funcionan en todos los modelos |
| **Anthropic SDK** | Python/JS | Ruta nativa sólida | Ruta nativa Messages con evidencia directa para herramientas, thinking y caché de prompts |
...
| **Langflow** | Python / Web | Soportado con límites de alcance | Use el campo **OpenAI API Base** del componente OpenAI con el endpoint `/v1` de TokenLab |
| **Dify** | - | Soportado con límites de alcance | El proveedor de OpenAI y los flujos orientados a chat-completions son la ruta prevista; no es adecuado para comportamientos de WebSocket o Responses específicos de Codex |
...
| **Phoenix** | Observabilidad | Ruta de rastreo soportada | Auto-instrumente el cliente SDK de OpenAI configurado con la URL base `/v1` de TokenLab |
| **Opik** | Observabilidad | Ruta de rastreo soportada | Envuelva un cliente SDK de OpenAI configurado con la URL base `/v1` de TokenLab |
...
### Opciones de selección de herramientas (Tool Choice)
