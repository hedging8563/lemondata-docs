...
Para video-to-video con `grok-imagine-video`, envía una URL HTTPS pública `.mp4` en `video_url`. TokenLab la traduce al cuerpo REST de xAI `video.url`; `duration`, `aspect_ratio` y `resolution` no se aceptan en ese flujo de edición.
