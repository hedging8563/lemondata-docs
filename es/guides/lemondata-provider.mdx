...
|----------|----------------|------------|---------|
| `lemondata` | `openai-completions` | GPT, DeepSeek, Qwen y la mayoría de las llamadas compatibles con OpenAI | `https://api.tokenlab.sh/v1` |
| `lemondata-responses` | `openai-responses` | Flujos de trabajo de OpenAI Responses que esperan la semántica de `/v1/responses` | `https://api.tokenlab.sh/v1` |
| `lemondata-claude` | `anthropic-messages` | API nativa de Claude Messages | `https://api.tokenlab.sh` |
| `lemondata-gemini` | `google-generative-ai` | Formato nativo de la API de Gemini | `https://api.tokenlab.sh` |
| `lemondata-minimax` | `anthropic-messages` | Enrutamiento nativo de MiniMax | `https://api.tokenlab.sh` |
...
<AccordionGroup>
  ...
  <Accordion title="Olvidar /v1 en lemondata-responses">
    `openai-responses` se mapea a la ruta `/v1/responses` de LemonData, por lo que `lemondata-responses` debe usar `https://api.tokenlab.sh/v1`.
  </Accordion>
...
- [Resumen de modelos](https://lemondata.cc/es/models) — explora los IDs de modelos antes de integrarlos en los agents
