...
La forma más sencilla de integrar LemonData es elegir el SDK según el comportamiento que necesitas:
...
| Cliente compatible con OpenAI existente | OpenAI SDK en LemonData /v1 |
...
<AccordionGroup>
  ...
  <Accordion title="Usa /v1 compatible con OpenAI para migraciones">
    Los SDKs compatibles con OpenAI son la mejor ruta para clientes existentes estilo OpenAI, chat portable y embeddings. Es una ruta de compatibilidad, no la única superficie API de LemonData.
  </Accordion>

  <Accordion title="Mantén explícitas las base URLs">
    Pasa las base URLs de LemonData directamente en la configuración del cliente en vez de depender de alias antiguos por variables de entorno.
  </Accordion>
</AccordionGroup>
