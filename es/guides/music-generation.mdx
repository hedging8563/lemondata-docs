...
Los ejemplos públicos actuales utilizan `suno_music` para la generación de música e incluyen `mv` con una versión oficial del modelo Suno, como `chirp-v4`. Para flujos solo de letras, envía `action: "LYRICS"` con un modelo cuyos detalles documenten la generación de letras y omite `mv`. Trata los IDs de modelo como IDs públicos de TokenLab, no como una garantía de que los campos específicos del proveedor sean campos de detalles del modelo.
...
La respuesta de creación es un registro de tarea que se consulta por polling, no el audio final:
...
Una respuesta de polling completada puede incluir los campos multimedia finales:
...
Los campos multimedia finales no aparecen hasta que `status` es `completed`. Las tareas fallidas devuelven `status: "failed"` con `error`.
...
| Parámetro no soportado | El campo no está en los detalles del modelo | Elimina los campos específicos del proveedor o elige un modelo que los documente |
