...
### Facturación de tareas asíncronas (Vídeo/Música/3D)

<Warning>
  **Importante**: Las tareas de generación de vídeo, música y 3D se **facturan al enviarlas**, no al completarse.
</Warning>

Para tareas de generación asíncronas (vídeo, música, modelos 3D):

1. **La facturación ocurre inmediatamente** al enviar la tarea
2. Recibirá un `task_id` para consultar el estado
3. **No hay reembolso automático** si la tarea falla

Esto se debe a que los proveedores upstream nos cobran cuando se envía la tarea, independientemente del resultado. Si experimenta fallos en las tareas, contacte a support@lemondata.cc.

```python
# Ejemplo: Facturación de generación de vídeo
response = client.post("/v1/video/generations", json={
    "model": "kling-v1",
    "prompt": "Una puesta de sol sobre el océano"
})
# ⚠️ Se le cobra AHORA, no cuando el vídeo esté listo
