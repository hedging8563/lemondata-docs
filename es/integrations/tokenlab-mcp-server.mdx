...
Los perfiles `core` y `full` publican de forma predeterminada esquemas de herramientas `portable` con límites de tamaño y profundidad, mientras que el servidor valida cada llamada con el contrato OpenAPI completo. Usa `TOKENLAB_MCP_SCHEMA_MODE=exact` para esquemas anidados completos o `TOKENLAB_MCP_SCHEMA_MODE=strict` para providers que exigen objetos cerrados y todas las propiedades en `required`; el modo strict representa los valores complejos de nivel superior como cadenas JSON.
...
| Argumentos | `-y` y después `@tokenlabai/mcp-server` | Usa `@tokenlabai/mcp-server@0.6.17` para fijar la versión verificada actualmente. |
