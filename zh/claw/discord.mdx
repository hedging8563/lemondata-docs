---
title: "Discord"
description: "通过机器人应用将您的 Claw 实例连接到 Discord"
---

将您的 Claw 实例连接到 Discord，直接在您的服务器或私信中与 AI 聊天。
...
<Steps>
  ...
  <Step title="开始聊天">
    1. 在您的 Discord 服务器中，提及（@）机器人或向其发送私信
    2. 完成 **配对** 流程（在您的 Claw WebUI 中输入代码）
    3. 开始与 AI 聊天！
  </Step>
</Steps>
...
在控制面板的 **IM Integrations** 选项卡中配置服务器（guild）行为：

| 设置 | 选项 | 描述 |
|---------|---------|-------------|
| **Group Policy** | Open / Allowlist / Disabled | 控制服务器频道的访问权限 |
| **Require @mention** | On / Off | 机器人仅在被 @提及时响应（推荐） |
| **Group IDs** | Guild ID 列表 | 限制为特定服务器（留空 = 所有服务器） |

<Note>
Discord 使用按服务器的用户白名单，而非全局群组白名单。如需高级的按服务器用户过滤，请直接在 OpenClaw 配置文件中配置。
</Note>
...
| **Allowlist** | 仅您指定的用户 ID 可以向机器人发私聊 |
| **Open** | 任何人都可以向机器人发私聊（请谨慎使用） |
...
<AccordionGroup>
  ...
  <Accordion title="机器人显示为离线">
    - 在控制台中检查您的实例状态（应为绿色/运行中）
    - 尝试重启实例
    - 验证 token 未在开发者门户中被重置
  </Accordion>
