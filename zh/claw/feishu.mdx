...
将飞书 (Feishu) 或 Lark 连接到您的 Claw 实例。使用 WebSocket 模式 —— 无需公网回调 URL。
...
<Steps>
  ...
  <Step title="配置事件订阅">
    ...
    <Tip>
    当您在 LemonClaw WebUI 中配置飞书时，**Encrypt Key** 和 **Verification Token** 会为您自动生成。将这些值复制到飞书开发者后台。
    </Tip>
  </Step>
  <Step title="添加事件权限">
    1. 在事件订阅下，添加以下事件：
...
| 设置 | 选项 | 描述 |
|---------|---------|-------------|
| **Group Policy** | 开启 (默认) / 白名单 / 已禁用 | 控制谁可以在群组中触发机器人 |
| **Require @mention** | 开启 / 关闭 | 机器人仅在被 @提及 时响应 |
| **Group IDs** | 群聊 ID 列表 | 限制在特定群组（为空 = 所有群组） |
...
<AccordionGroup>
  <Accordion title="机器人收不到消息">
    ...
    - 确保已选择 WebSocket 模式（而非 HTTP）
    - 验证飞书后台和 Claw 配置之间的 Encrypt Key 和 Verification Token 是否一致
  </Accordion>
