...
当前取消仅支持排队中的火山引擎 Seedance 视频任务，模型为 `seedance-1.5-pro`、`seedance-2.0` 或 `seedance-2.0-fast`。已经进入 `processing`/running 的任务不能取消。已完成、失败、过期或不支持的任务会继续保留查询能力，用于状态与计费对账，而不是删除上游记录。
