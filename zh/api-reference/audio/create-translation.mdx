...
将任何支持语言的音频翻译为英文文本。与转录不同，此端点始终输出英文文本，无论输入语言是什么。

## 请求体

<ParamField body="file" type="file" required>
  要翻译的音频文件。支持格式：`flac`、`mp3`、`mp4`、`mpeg`、`mpga`、`m4a`、`ogg`、`wav`、`webm`。最大文件大小为 25 MB。
</ParamField>

<ParamField body="model" type="string" default="whisper-1">
  使用的模型。目前仅支持 `whisper-1`。
</ParamField>

<ParamField body="prompt" type="string">
  可选文本，用于引导模型的风格或延续上一段内容。应为英文。
</ParamField>

<ParamField body="response_format" type="string" default="json">
  输出格式。选项：`json`、`text`、`srt`、`verbose_json`、`vtt`。
</ParamField>

<ParamField body="temperature" type="number">
  采样温度，介于 0 和 1 之间。较高的值（如 0.8）使输出更随机，较低的值（如 0.2）使输出更集中和确定。
</ParamField>
...
|------|------|------|
| 输出语言 | 始终为英文 | 与输入相同 |
| 使用场景 | 将外语音频转为英文 | 保留原始语言 |
| language 参数 | 不适用 | 可选提示 |

<Note>
  翻译端点会自动检测源语言并翻译为英文。转录中的 `language` 参数在此处会被忽略。
</Note>
