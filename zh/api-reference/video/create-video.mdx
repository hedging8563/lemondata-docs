...
<ParamField body="output_audio" type="boolean">
  模型相关的音频输出开关。在 LemonData 中，Veo 3 家族在省略该字段时默认按 `true` 处理；`kling-3.0-video` 仅在非元素引用请求中接受该 selector，并会映射到兼容上游的 sound 控制，Kling 请求省略时默认无声。不要将 `output_audio=true` 与 `kling_elements` 组合使用。其他公开视频模型按各自治理后的默认行为执行。为了兼容历史请求，也接受 camelCase 别名 `outputAudio`。
</ParamField>
