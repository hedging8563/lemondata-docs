...
<Warning>
音频输出是否开启取决于具体模型。Veo 3 家族默认开启音频；当模型支持时，可通过 `output_audio` 显式请求音频。为了兼容历史请求，也接受 camelCase 别名 `outputAudio`。
</Warning>
...
<ParamField body="output_audio" type="boolean">
  模型相关的音频输出开关。Veo 3 家族在省略该字段时默认按 `true` 处理；其他视频模型按各自治理后的默认行为执行。为了兼容历史请求，也接受 camelCase 别名 `outputAudio`。
</ParamField>
...
使用 `start_image` 和 `end_image` 控制第一帧和最后一帧（由 Vidu Q2 Pro 支持）：
