...
<ParamField body="material_asset_id" type="string">
  [创建素材](/zh/api-reference/video/create-material-asset)返回的 TokenLab Seedance 素材 ID。素材变为 `ACTIVE` 后，可在 Seedance 2.0 生成模型中使用。
</ParamField>

<ParamField body="material_asset_ids" type="array">
  多个 TokenLab Seedance 素材 ID。它们与 `reference_images` 共用 Seedance 图片参考数量限制；Seedance 2.0 最多传 9 个图片参考。
</ParamField>
...
- 为了兼容已有调用，TokenLab 也接受 `ratio`、`generate_audio`、`outputAudio`、`referenceImages` 和 `referenceImageType`。
...
对于 Seedance 1.5/2.0 模型，统一接口以 TokenLab 字段为主，同时接受官方常见别名 `ratio` 和 `generate_audio`。省略 Seedance 参数时会使用这些默认值：`duration=5`、`resolution=720p`、`aspect_ratio=adaptive`、`output_audio=true`、`watermark=false`、`return_last_frame=false`、`execution_expires_after=172800`、`priority=0`、`seed=-1`。

`duration=-1` 表示让 Seedance 在模型支持范围内自动选择输出时长。TokenLab 会在任务完成前按保守方式预估费用，并在可获得完成任务 usage 时按实际结果结算。`service_tier=default` 对 Seedance 2.0 作为兼容 no-op 接受；`service_tier=flex`、`frames`、`camera_fixed` 会在所选模型不支持时被拒绝。
