...
<Note>
`gpt-image-2` 已支持此端点。可以发送 multipart `image`，也可以发送 JSON `image_url`；可选参数包括 `mask`、`n`、`size`、`quality`、`response_format`、`background`、`output_format`、`output_compression` 或 `compression`、`moderation`、`partial_images` 和 `user`。计费以 NoSpamBro/上游返回的 usage token 明细为准，包括图片输入 token，以及 partial images 产生的额外图片输出 token。
</Note>
