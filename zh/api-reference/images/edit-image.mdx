...
<Note>
`gpt-image-2` 已支持此端点。可以发送 multipart `image` 上传、JSON `image_url`、JSON `image_urls`，或官方 JSON `images` 数组中的 `image_url` / `file_id` 引用；GPT Image 编辑最多接受 16 张源图。`file_id` 必须先通过 LemonData `/v1/files` 以 `purpose=user_data` 或 `purpose=vision` 创建，这样网关才能将它绑定到同一个上游图像编辑渠道。可选参数包括 `mask`、`n`、`size`、`quality`、`response_format`、`output_format`、`output_compression` 或 `compression`、`moderation`、`partial_images` 和 `user`。`background` 仅保留在图像生成路径中，不属于当前 `gpt-image-2` 图像编辑公共契约。计费以上游返回的 usage token 明细为准，包括图片输入 token，以及 partial images 产生的额外图片输出 token。
</Note>
