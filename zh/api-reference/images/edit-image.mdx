...
<Warning>
此端点当前文档化的是 DALL-E 风格的 multipart 编辑流程。LemonData 上其他受支持的图像到图像模型使用 `POST /v1/images/generations`，并在支持的情况下通过 `operation: "image-to-image"` 及诸如 `image_url` 的 JSON 输入来调用。
</Warning>
