...
- `gemini-3.1-flash-image-preview`、`gemini-3-pro-image-preview` 和 `nano-banana-pro` 在公开的文生图、图片编辑/图生图操作中支持 `aspect_ratio` 和 `resolution`（`1k`、`2k`、`4k`）。
...
<ParamField body="model" type="string" required>
  要使用的模型（例如 `gpt-image-2`、`flux-pro`、`qwen-image` 或 `nano-banana-pro`）。使用 `GET /v1/models?recommended_for=image` 获取当前推荐列表。
</ParamField>
...
<ParamField body="resolution" type="string">
  ...
  `gemini-3.1-flash-image-preview` 和 `gemini-3-pro-image-preview` 在文生图与图片编辑中支持该参数；`nano-banana-pro` 在文生图与图生图中支持；`nano-banana-2` 仅文生图支持。常见值为 `1k`、`2k`、`4k`。除非模型明确说明，否则不要发送给仅支持比例选择的 Gemini 图片家族。xAI Grok Imagine 图片模型请使用 `1k` 或 `2k`。
</ParamField>
...
| `qwen-image` | 通常是任务式 | 文本渲染和提示词遵循较强 |
