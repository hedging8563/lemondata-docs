...
<ParamField body="response_format" type="string" default="url">
  ...
  对于 Azure Official 或 Azure-compatible 的 `gpt-image-2` 路由，TokenLab 不会把 `response_format` 传给上游。网关始终从上游拿 `b64_json` 图片数据；请求 `url` 时会把每张图上传到 CDN 并返回 `data[].url`。如果 CDN 存储不可用或上传失败，请求会失败，而不是降级返回 Base64；请求 `b64_json` 时直接返回原始 Base64。
</ParamField>
