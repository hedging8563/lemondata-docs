...
该接口返回单个 API Key 维度的分页用量明细，不暴露物理路由元数据。
...
- 内部路由和物理渠道元数据会被隐藏。
- 所有金额字段都保留 USD 原值，并额外返回人民币展示字段，例如 `cost_cny`、`total_cost_cny`，以及各类 breakdown 内的 `cost_cny`。
- `api_key.exchange_rate` 表示本次响应中所有 `*_cny` 字段使用的 USD→CNY 汇率。
