---
title: 'API 参考'
description: 'LemonData API 的完整参考'
---
...
LemonData API 是**原生优先，同时兼容 OpenAI** 的。需要提供商原生行为时，使用 Anthropic 的 `POST /v1/messages` 或 Gemini 的 `/v1beta/models/...:generateContent`；迁移已有 OpenAI 风格 SDK 或工具时，使用 OpenAI 兼容的 `/v1` 端点。`POST /v1/responses` 仍是需要 Responses 特定行为时的高级可选路径。
...
从 [Dashboard](https://lemondata.cc/dashboard) 获取你的 API 密钥。
