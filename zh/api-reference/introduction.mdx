...
| `/v1/tasks/{id}` | GET | 获取视频任务的统一异步状态 |
| `/v1/videos/generations/{id}` | GET | 仅为兼容保留的视频任务状态路径 |

<Info>
新客户端请优先使用 `/v1/tasks/{id}`，并跟随创建响应返回的 `poll_url`。`/v1/videos/generations/{id}` 仅保留向后兼容。
</Info>
...
| 端点 | 方法 | 描述 |
|----------|--------|-------------|
| `/v1/tasks/{id}` | GET | 统一异步任务查询端点。跟随返回的 `poll_url` 时推荐使用 |
