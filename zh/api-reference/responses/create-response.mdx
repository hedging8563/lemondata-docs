...
Responses API 是 OpenAI 更新的有状态对话 API。LemonData 将此格式作为兼容模型的高级可选路径提供支持；除非你明确需要 Responses 特定行为，否则请将 `POST /v1/chat/completions` 作为默认的兼容 OpenAI 路由。
...
<ParamField body="tools" type="array">
  ...
  对于使用默认图片工具模型，或显式设置 `model: "gpt-image-2"` 的 hosted `image_generation` 工具，LemonData 会在转发请求前移除不支持的 `input_fidelity`，因为 GPT Image 2 已经以高保真处理图片输入。请不要对这个工具传入 `background: "transparent"`；LemonData 不会静默移除它，因为这会改变输出语义。
</ParamField>
