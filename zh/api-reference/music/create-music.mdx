...
<ParamField body="negative_tags" type="string">
  需要避开的风格。`audio_operation` 为 `add-instrumental` 时必填；会转发给 KIE 的 `negativeTags`。
</ParamField>
...
<ParamField body="make_instrumental" type="boolean">
  设为 `true` 时生成无歌声的纯音乐。TokenLab 会把该公开字段映射到 KIE 的 `instrumental`。请使用 TokenLab 字段名，例如 `make_instrumental`、`audio_url`、`negative_tags`、`continue_at`；KIE 上游字段名 `instrumental`、`uploadUrl`、`negativeTags`、`continueAt` 会被拒绝。
</ParamField>
