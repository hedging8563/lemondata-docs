...
| 403 | Forbidden - 访问被拒绝或不允许使用该模型 |
| 404 | Not Found - 找不到模型或资源 |
| 413 | Payload Too Large - 输入或文件大小超过限制 |
| 429 | Too Many Requests - 超过速率限制 |
| 500 | Internal Server Error - 服务器内部错误 |
| 502 | Bad Gateway - 上游供应商错误 |
| 503 | Service Unavailable - 所有渠道皆失败 |
...
<AccordionGroup>
  <Accordion title="实现指数退避 (Exponential Backoff)">
    当遇到速率限制时，在重试之间逐渐增加等待时间：
    ```python
    wait_time = 2 ** attempt  # 1s, 2s, 4s, 8s...
    ```
  </Accordion>

  <Accordion title="设置超时时间 (Timeouts)">
    务必设置合理的超时时间，以避免请求挂起：
    ```python
  ...
  <Accordion title="记录错误以供调试">
    记录完整的错误响应（包括 request ID）以便寻求支持：
    ```python
    except APIError as e:
  ...
  <Accordion title="处理特定模型的错误">
    某些模型有特定要求（例如：最大 token 数、图片格式）。
    在发送请求前验证输入。
  </Accordion>
</AccordionGroup>