---
title: "Vercel AI SDK"
description: "将 LemonData 与 Vercel AI SDK 集成，用于 React 和 Next.js 应用程序"
---

## 概览

Vercel AI SDK 是一个 TypeScript 工具包，用于使用 React、Next.js、Vue、Svelte 等框架构建 AI 驱动的流式应用程序。LemonData 可通过与 OpenAI 兼容的 provider 无缝运行。
...
<AccordionGroup>
  <Accordion title="使用流式传输以获得更好的用户体验 (UX)">
    在对话应用程序中始终使用流式传输，以提供实时反馈。
  </Accordion>

  <Accordion title="优雅地处理错误">
    在 React 组件中实现适当的错误边界 (Error Boundaries)。
  </Accordion>

  <Accordion title="保护您的 API key">
    切勿在客户端代码中泄露您的 API key。请务必使用服务端 API 路由。
  </Accordion>
</AccordionGroup>