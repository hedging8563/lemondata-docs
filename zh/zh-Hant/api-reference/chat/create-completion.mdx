---
title: "创建对话补全 (Create Chat Completion)"
...
<ParamField body="model" type="string" required>
  要使用的模型 ID。请参阅 [Models](https://lemondata.cc/zh-TW/models) 以查看可用选项。
</ParamField>
...
<ParamField body="temperature" type="number" default="1">
  介于 0 到 2 之间的采样温度。较高的数值会使输出更具随机性。
</ParamField>
...
<ParamField body="stream" type="boolean" default="false">
  若为 true，部分消息增量将以 SSE 事件形式发送。
</ParamField>

<ParamField body="stream_options" type="object">
  流式传输选项。设置 `include_usage: true` 以在流式传输块中接收 token 使用情况。
</ParamField>

<ParamField body="top_p" type="number" default="1">
  核采样 (Nucleus sampling) 参数。我们建议调整此参数或 temperature，而非两者同时调整。
</ParamField>
...
<ParamField body="presence_penalty" type="number" default="0">
  介于 -2.0 到 2.0 之间的数值。正值会惩罚已在文本中出现过的 token。
</ParamField>

<ParamField body="stop" type="string | array">
  最多 4 个序列，API 将在这些序列处停止生成 token。
</ParamField>
...
<ParamField body="max_completion_tokens" type="integer">
  补全的最大 token 数量。`max_tokens` 的替代方案，建议用于 o1/o3 等较新的模型。
</ParamField>

<ParamField body="reasoning_effort" type="string">
  o1/o3 模型的推理强度。选项：`low`、`medium`、`high`。
</ParamField>
...
<ParamField body="top_logprobs" type="integer">
  要返回的前几个对数概率数量 (0-20)。需要设置 `logprobs: true`。
</ParamField>
...
<ParamField body="response_format" type="object">
  响应格式规范。使用 `{"type": "json_object"}` 开启 JSON 模式，或使用 `{"type": "json_schema", "json_schema": {...}}` 进行结构化输出。
</ParamField>

<ParamField body="logit_bias" type="object">
  修改指定 token 出现的可能性。将 token ID（字符串形式）映射到 -100 到 100 之间的偏差值。
</ParamField>

<ParamField body="user" type="string">
  代表您终端用户的唯一识别码，用于滥用监控。
</ParamField>
...
<ParamField body="cache_control" type="object">
  ...
  - `max_age` (integer)：缓存存活时间 (TTL)，以秒为单位（最大 86400）
</ParamField>
