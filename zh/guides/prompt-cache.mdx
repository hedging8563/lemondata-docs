...
除了 TokenLab 的 [平台语义缓存](/guides/caching) 之外，许多 AI 供应商也提供自有的 **Prompt 缓存** 功能。这是一种运作于供应商级别（Anthropic、OpenAI、DeepSeek 等）的独立缓存机制。
...
| 类型 | 位置 | 工作原理 | 成本 |
|------|-------|--------------|------|
| **平台缓存** | TokenLab | 语义相似度匹配 | 正常价格的 10% |
...
<Info>
折扣会自动应用。TokenLab 会将供应商的缓存定价直接传递给您。
</Info>
