...
LemonData 提供智能缓存系统，可以显著降低您的 API 成本和响应延迟。我们的缓存不仅仅是简单的请求匹配——它能理解您提示词的**语义含义**。

<CardGroup cols={2}>
  <Card title="节省成本" icon="piggy-bank">
    缓存命中的计费仅为正常成本的一小部分。
  </Card>
  <Card title="更快响应" icon="bolt">
    缓存响应即时返回，无需模型推理。
  </Card>
...
对于确定性请求（`temperature=0`），我们缓存精确响应：
...
| 类型 | 效果 |
|------|--------|
| `no_cache` | 跳过缓存查找，始终获取新响应 |
| `no_store` | 不将此响应存储到缓存 |
| `response_only` | 仅使用精确匹配缓存（跳过语义） |
...
| 模式 | 描述 |
|------|-------------|
| **共享** | 启用缓存，响应可在用户间共享（个人账户默认） |
| **隔离** | 启用缓存，但响应仅限于您的组织（组织默认） |
| **禁用** | 完全不缓存 |
...
当缓存条目收到足够多的负面反馈时，它会自动失效。

## 最佳实践

<AccordionGroup>
  <Accordion title="对可缓存查询使用 temperature=0">
    确定性设置可最大化缓存命中率。
  </Accordion>
  ...
  <Accordion title="对时间敏感查询使用 no-cache">
    当前事件、实时数据应跳过缓存。
  </Accordion>
