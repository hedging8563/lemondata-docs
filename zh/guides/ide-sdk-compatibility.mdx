...
- **强原生路径 (Strong native path)**：意味着代码库中还针对该协议族提供了直接的适配器或请求格式证据。
...
| **Claude Code CLI** | 强原生路径 | Anthropic | 原生 `/v1/messages` 路由，涵盖思维链 (thinking) 和 `tool_choice` 的适配器支持 |
...
| SDK / 框架 | 语言 | 支持级别 | 备注 |
|-----------------|----------|---------------|-------|
| **OpenAI SDK** | Python/JS/Go | 支持核心路径 | `/v1` Chat Completions 和 Embeddings 的兼容路径；针对特定提供商的行为请使用原生提供商路由，不要假设仅限 Responses 的字段在所有模型上都有效 |
| **Anthropic SDK** | Python/JS | 强原生路径 | 原生 Messages 路由，直接支持工具、思维链和提示词缓存 |
| **Vercel AI SDK** | TypeScript | 推荐集成模式 | 优先使用 `@ai-sdk/openai-compatible`；仅在明确需要 Responses 原生行为时使用 `@ai-sdk/openai` |
| **LangChain** | Python/JS | 支持标准接口 | `ChatOpenAI` 和 `OpenAIEmbeddings` 是预期的使用范围；供应商原生扩展不在支持范围内 |
...
| **Semantic Kernel** | .NET | 支持自定义端点路径 | 使用 OpenAI 聊天补全连接器的自定义端点，并指向 `https://api.tokenlab.sh/v1` |
...
| **Langflow** | Python / Web | 支持（有限制） | 在 OpenAI 组件的 **OpenAI API Base** 字段中使用 TokenLab 的 `/v1` 端点 |
| **Dify** | - | 支持（有限制） | 预期路径为 OpenAI 提供商和 chat-completions 导向的流程；不适用于 Codex 特定的 Responses 或 WebSocket 行为 |
...
| **Agno** | Python | 支持路径 | 在可用时使用 TokenLab 模型封装，或使用 OpenAI 兼容的基础 URL 路径 |
...
| **Helicone** | 网关 / 可观测性 | 支持可观测性路径 | 通过 Helicone 网关路由 OpenAI 兼容请求，并设置 `Helicone-Target-Url: https://api.tokenlab.sh` |
| **Phoenix** | 可观测性 | 支持追踪路径 | 自动检测配置了 TokenLab `/v1` 基础 URL 的 OpenAI SDK 客户端 |
| **Opik** | 可观测性 | 支持追踪路径 | 封装配置了 TokenLab `/v1` 基础 URL 的 OpenAI SDK 客户端 |
| **LangBot** | 机器人平台 | 支持聊天路径 | 在可用时使用 TokenLab 请求器，或配置指向 TokenLab `/v1` 端点的 OpenAI 兼容请求器 |
| **Open WebUI** | Web 聊天 | 支持聊天应用路径 | 将 TokenLab 添加为 OpenAI 兼容的 API 连接并刷新模型列表 |
| **Chatbox / DeepChat / Jan / LibreChat / Cherry Studio** | 桌面 / Web | 支持聊天应用路径 | 使用各应用自带的 OpenAI 兼容自定义提供商流程；这些应用通常面向 chat-completions |
...
| OpenAI 对象 | `{ "type": "function", "function": { "name": "fn" } }` | 强制使用特定函数 |
...
| `reasoning_effort` | string | `"low"`, `"medium"`, `"high"`，用于支持推理的 GPT-5 模型 |
| `service_tier` | string | `"auto"` 或 `"default"` |
| `seed` | integer | 确定性输出 |
| `logprobs` | boolean | 返回对数概率 |
| `top_logprobs` | integer | 前 N 个对数概率的数量 (0-20) |
...
| `prediction` | object | 预测输出以实现更快的补全 |
...
支持 `stream: true` 的生成端点（包括 Chat Completions 和 Responses）使用服务器发送事件 (SSE)：
