...
- **支持 (Supported)**：意味着我们记录了具体的设置路径，且 TokenLab 暴露了该路径所期望的协议形态。
- **强原生路径 (Strong native path)**：意味着仓库还针对该协议族提供了直接的适配器或请求格式证据。
- **尽力而为 (Best-effort)**：意味着集成可以工作，但上游客户端并不将此自定义网关工作流视为稳定的契约。
...
| 工具 | 支持级别 | 格式 | 备注 |
|------|---------------|--------|-------|
| **Cursor** | 有限支持 | OpenAI 兼容 | 适用于 BYOK 标准聊天/编辑器流程，不能替代 Cursor 管理的功能（如 Tab Completion） |
| **Claude Code CLI** | 强原生路径 | Anthropic | 原生 `/v1/messages` 路由，涵盖 thinking 和 `tool_choice` 的适配器支持 |
| **Codex CLI** | 模型/路径受限支持 | OpenAI Responses | 将 `/v1/responses` 视为 Codex 特定工作流的高级路径；并非所有模型和路由路径都能保证支持仅限 Responses 的字段 |
| **Gemini CLI** | 尽力而为 / 实验性 | Gemini | 自定义 TokenLab 基础 URL 流程并非官方 Gemini CLI 集成 |
| **OpenCode** | 支持 | OpenAI 兼容 | 默认使用 OpenAI 兼容的提供商；仅在明确需要时切换到基于 Responses 的提供商 |
...
| SDK / 框架 | 语言 | 支持级别 | 备注 |
|-----------------|----------|---------------|-------|
| **OpenAI SDK** | Python/JS/Go | 核心路径支持 | `/v1` Chat Completions 和 Embeddings 的兼容路径；针对特定提供商的行为请使用原生提供商路由，不要假设仅限 Responses 的字段在所有模型上都有效 |
| **Anthropic SDK** | Python/JS | 强原生路径 | 原生 Messages 路由，直接支持 tools、thinking 和 prompt caching |
| **Vercel AI SDK** | TypeScript | 推荐集成模式 | 优先使用 `@ai-sdk/openai-compatible`；仅在明确需要 Responses 原生行为时使用 `@ai-sdk/openai` |
| **LangChain** | Python/JS | 标准接口支持 | `ChatOpenAI` 和 `OpenAIEmbeddings` 是预期的使用范围；供应商原生扩展功能不在支持范围内 |
| **LlamaIndex** | Python | 通过 `OpenAILike` 支持 | 对于 TokenLab 等第三方网关，请使用 `OpenAILike`，而非内置的 OpenAI 类 |
| **OpenAI Agents SDK** | Python | 聊天补全路径支持 | 将 `AsyncOpenAI(base_url="https://api.tokenlab.sh/v1")` 与 `OpenAIChatCompletionsModel` 配合使用；原生 Responses 行为取决于所选模型和路由 |
| **Langflow** | Python / Web | 范围受限支持 | 在 OpenAI 组件的 **OpenAI API Base** 字段中使用 TokenLab 的 `/v1` 端点 |
| **Dify** | - | 范围受限支持 | OpenAI 提供商和面向聊天补全的流程是预期的路径；不适用于 Codex 特定的 Responses 或 WebSocket 行为 |
| **Flowise** | Web / Node | 范围受限支持 | 在可用时使用 TokenLab 聊天节点，或指向 `https://api.tokenlab.sh/v1` 的 ChatOpenAI 兼容节点 |
| **Agno** | Python | 支持路径 | 在可用时使用 TokenLab 模型包装器，或使用 OpenAI 兼容的基础 URL 路径 |
| **Promptfoo** | CLI / Node | 支持路径 | 使用 OpenAI 聊天或 Responses 提供商，并设置 `apiBaseUrl: https://api.tokenlab.sh/v1` |
| **Portkey Gateway** | 网关 | 支持路径 | 将 TokenLab 用作 OpenAI 兼容聊天和 Responses 流量的上游提供商 |
| **Helicone** | 网关 / 可观测性 | 可观测性路径支持 | 通过 Helicone Gateway 路由 OpenAI 兼容请求，并设置 `Helicone-Target-Url: https://api.tokenlab.sh` |
| **LangBot** | 机器人平台 | 聊天路径支持 | 在可用时使用 TokenLab 请求器，或配置指向 TokenLab `/v1` 端点的 OpenAI 兼容请求器 |
| **Open WebUI** | Web 聊天 | 聊天应用路径支持 | 添加 TokenLab 作为 OpenAI 兼容的 API 连接并刷新模型列表 |
| **Chatbox / DeepChat / Jan / LibreChat / Cherry Studio** | 桌面 / Web | 聊天应用路径支持 | 使用各应用的 OpenAI 兼容自定义提供商流程；这些应用通常面向聊天补全 |
...
| 参数 | 类型 | 描述 |
|-----------|------|-------------|
| `stream_options` | object | 仅限 Chat Completions：`{ include_usage: true }` 用于获取 token 计数 |
| `reasoning_effort` | string | 启用推理的 GPT-5 模型使用 `"low"`, `"medium"`, `"high"` |
...
| 参数 | 类型 | 描述 |
|-----------|------|-------------|
| `modalities` | array | 多模态使用 `["text", "audio"]` |
| `audio` | object | 音频输出配置 (语音、格式) |
| `prediction` | object | 用于更快补全的预测输出 |
...
<AccordionGroup>
  ...
  <Accordion title="仅在 Chat Completions 中使用 stream_options.include_usage">
    对于 Chat Completions 流式传输，启用 `stream_options.include_usage` 以获取准确的 token 计数。Responses 有其自己的流式传输契约，不暴露此仅限 Chat 的选项。
  </Accordion>
