...
LemonData 的 Agent 优先 API 通过结构化提示增强了错误响应，AI Agent 可以立即解析并采取行动——无需网页搜索，无需查阅文档，无需凭空猜测。
...
<Note>
`retryable` 在原因为 `no_channels`（未为此模型配置渠道）时为 `false`。仅在断路器触发或配额耗尽等瞬时故障时为 `true`。
</Note>
...
| 模型提供商 | 建议端点 | 优势 |
|---------------|-------------------|---------|
| Anthropic (Claude) | `/v1/messages` | 无需格式转换，支持扩展思考 (extended thinking)，支持 Prompt 缓存 |
...
在首次 API 调用前读取 `llms.txt` 的 AI Agent 通常可以在第一次尝试时就获得成功。
...
<CardGroup cols={2}>
  <Card title="快速失败，信息丰富" icon="bolt">
    错误会立即返回，并包含 Agent 自我纠错所需的所有数据。
  </Card>
  <Card title="无自动路由" icon="route">
    API 绝不会静默替换为其他模型。由 Agent 做出决定。
  </Card>
