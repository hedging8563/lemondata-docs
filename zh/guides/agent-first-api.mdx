...
LemonData 的 Agent-First API 会在错误响应中加入结构化提示，AI Agent 可以立即解析并采取行动 —— 无需网络搜索、无需查阅文档、无需猜测。
...
| `cache_pricing` | `object` 或 `null` | 仅在模型存在 prompt cache 价格时返回；平台语义缓存折扣不会单独出现在模型列表中 |
...
```bash
GET https://api.lemondata.cc/v1/models?category=chat          # 仅聊天模型
GET https://api.lemondata.cc/v1/models?category=image         # 图片生成模型
GET https://api.lemondata.cc/v1/models?tag=coding&category=chat  # 面向 coding 的聊天模型
```
