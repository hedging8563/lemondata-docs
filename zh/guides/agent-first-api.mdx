---
title: "✨ 以代理为先的 API"
description: "为 AI 代理在第一次重试时提供结构化错误提示以实现自我修正"
---

## 概览

LemonData 的以代理为先的 API 在错误响应中加入结构化提示，AI 代理可以立即解析并采取行动 —— 无需网络搜索、无需查阅文档、无需猜测。
...
| `cache_pricing` | object or `null` | 上游提示缓存价格 + 平台语义缓存折扣 |
| `agent_preferences.<scene>` | object | 该场景的非聊天推荐快照 |

`agent_preferences` 来源于缓存的 24 小时成功率快照：
