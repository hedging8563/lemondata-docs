...
Agent 应将其视为平台可用性信号，而不是拼写错误。只有在它出现在 `GET /v1/models` 后，才应重试同一个模型。
...
```bash
GET https://api.lemondata.cc/v1/models?category=chat          # Chat 模型
GET https://api.lemondata.cc/v1/models?category=image         # 图像生成模型
GET https://api.lemondata.cc/v1/models?tag=coding&category=chat  # 为编码优化的 Chat 模型
```
