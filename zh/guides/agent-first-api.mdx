...
LemonData 的 Agent-First API 通过结构化提示增强了错误响应，AI 智能体可以立即解析并采取行动——无需网络搜索、无需查阅文档、无需猜测。

每个错误响应在标准 `error` 对象中都包含可选字段，如 `did_you_mean`、`suggestions`、`hint`、`retryable` 和 `retry_after`。这些字段是向后兼容的——不使用它们的客户端不会察觉到任何差异。
...
`suggestions` 包含比预估成本更便宜的模型，智能体可以切换到这些模型。
...
<Note>
当原因为 `no_channels`（未为此模型配置渠道）时，`retryable` 为 `false`。仅在发生熔断器触发或配额耗尽等瞬态故障时，它才为 `true`。
</Note>
...
<Note>
OpenAI 兼容端点使用 LemonData 稳定的公共错误类型，例如 `rate_limit_exceeded`、`upstream_error` 和 `all_channels_failed`。Anthropic 兼容和 Gemini 兼容端点则使用它们各自的原生响应格式。
</Note>

### context_length_exceeded (400)

当输入超过模型的上下文窗口时（上游错误，已增加提示增强）：
...
当您使用具有原生端点（Anthropic 或 Gemini）的模型调用 `/v1/chat/completions` 时，**成功响应**将包含优化 Header：
...
| 模型提供商 | 建议端点 | 优势 |
|---------------|-------------------|---------|
| Anthropic (Claude) | `/v1/messages` | 无需格式转换，支持 Extended Thinking，支持 Prompt Caching |
| Google (Gemini) | `/v1beta/gemini` | 无需格式转换，支持 Grounding，支持安全设置 |
...
| `cache_pricing` | 对象或 `null` | 上游 Prompt Cache 价格 + 平台语义缓存折扣 |
...
GET /v1/models?tag=coding&category=chat  # 针对编程优化的聊天模型
```

## llms.txt

机器可读的 API 概览可在以下地址获取：
...
在首次调用 API 之前阅读 `llms.txt` 的 AI 智能体通常可以在第一次尝试时就获得成功。
