...
TokenLab 支持使用单个 API 密钥的**三种原生 API 格式**。选择最适合您用例的格式 — 无需更改配置。
...
如果 TokenLab 无法把带原生工具的请求路由到支持原生格式的上游路径，会返回明确的 unsupported-field 错误，而不是静默丢弃工具或伪装成 Chat Completions 函数。用户自定义函数工具仍然是最可移植的工具路径。
