...
LemonData 的[语义缓存](/zh/guides/caching)按语义匹配请求，而非精确文本。这对 Coding Agent 特别有效：
...
### Prompt Cache（Provider 级别）

[上游 Prompt Cache](/zh/guides/prompt-cache) 通过 LemonData 自动生效。长系统提示——Coding Agent 每次都会包含——在 Provider 层被缓存：

| Provider | 缓存折扣 | 最低 token 数 |
|----------|---------|-------------|
