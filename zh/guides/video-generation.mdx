...
跨模型视频生成请使用 TokenLab 统一的 `/v1/videos/generations`。如果你正在迁移已经使用火山风格 `content[]` 或 Action 请求的 Seedance 2.0 集成，可以使用 `/api/v3` 下的 Seedance 兼容入口。两种入口都使用 TokenLab Bearer API Key 和异步轮询，但请求与响应结构不同。
