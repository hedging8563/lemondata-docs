...
LemonData 通过单一统一的 API 提供来自 11 个供应商的 **69 多个视频生成模型**。视频生成是**异步**的 —— 您提交请求后会得到任务 ID 和 `poll_url`，然后再轮询结果。
...
- **T2V (文生视频)**：从文本提示词生成视频
- **I2V (图生视频)**：将静态图像动画化为视频。为了获得最广泛的兼容性，优先提供公开可访问的 `image_url`
...
使静态图像动起来。为了获得跨视频模型的最广泛兼容性，请先托管源图像并传入 `image_url`。
...
| `image` | string | 内联数据 URL 图像。兼容的模型可以使用，但 `image_url` 提供最广泛的兼容性。 |
