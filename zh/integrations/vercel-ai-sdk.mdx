---
title: "Vercel AI SDK"
description: "使用 OpenAI-compatible 或 Responses-native 提供方将 LemonData 集成到 Vercel AI SDK 中"
---
...
**主要路径**: 默认 OpenAI-compatible 路径

**支持级别**: 推荐集成模式
</Note>

对于 LemonData，Vercel AI SDK 中最稳定的默认选择是 **OpenAI-compatible 提供方**。

如果你明确需要 **Responses-native** 行为，可以切换到 OpenAI 提供方，同时保持相同的 LemonData base URL。

请将本页视为推荐集成模式，而不是“仓库里对 Vercel AI SDK 每个 helper 都做了端到端回归”的声明。

## 推荐默认方案：OpenAI-compatible 提供方
...
<AccordionGroup>
  ...
  <Accordion title="在服务器端保管密钥">
    绝不要在客户端代码中暴露你的 LemonData API key。请将提供方配置放在服务器文件或 API 路由中。
  </Accordion>
</AccordionGroup>
