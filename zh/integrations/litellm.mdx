---
title: "LiteLLM"
description: "将 TokenLab 与 LiteLLM 结合使用，可作为上游提供方，也可作为内部网关层"
---
...
- 把 TokenLab 作为 LiteLLM 背后的 **OpenAI 兼容 上游**
- 在 TokenLab 前面再放一层 LiteLLM，统一做虚拟 key、路由策略或额外观测
...
<AccordionGroup>
  <Accordion title="对 TokenLab 优先使用 custom_openai">
    除非你有非常明确的理由去构造更复杂的提供方映射，否则请把 TokenLab 视为 OpenAI 兼容 上游。
  </Accordion>

  <Accordion title="只在确实需要额外网关层时再用 LiteLLM">
    如果你的平台自己还需要 virtual keys、额外路由策略或集中日志，LiteLLM 放在 TokenLab 前面是有价值的。
  </Accordion>
