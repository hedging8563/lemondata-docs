...
[Claude Code](https://docs.anthropic.com/en/docs/claude-code) 是 Anthropic 官方提供的用于 AI 辅助编码的 CLI 工具。你可以将其配置为使用 LemonData 作为 API 提供方。
...
<Note>
对于 LemonData，请将 `ANTHROPIC_BASE_URL` 保持为 `https://api.lemondata.cc`，不要添加 `/v1` 后缀。这与 Claude Code 文档中记录的 Anthropic 风格端点结构以及统一网关模式保持一致。
</Note>
