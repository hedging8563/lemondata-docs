---
title: "Claude Code"
description: "将 LemonData 与 Anthropic 的 Claude Code CLI 一起使用"
---

## 概述

[Claude Code](https://docs.anthropic.com/en/docs/claude-code) 是 Anthropic 官方提供的用于 AI 辅助编码的 CLI 工具。你可以将其配置为使用 LemonData 作为 API 提供商。
...
<Note>
对于 LemonData，请将 `ANTHROPIC_BASE_URL` 保持为 `https://api.lemondata.cc`，不要添加 `/v1` 后缀。这与 Claude Code 文档中记录的 Anthropic 风格 endpoint 结构以及统一网关模式保持一致。
</Note>
...
<AccordionGroup>
  <Accordion title="连接错误">
    请确认 base URL 精确为 `https://api.lemondata.cc`（对于 Anthropic 风格请求，不要添加 `/v1` 后缀）。
  </Accordion>

  <Accordion title="无效的 API key">
    请确保你的 LemonData API key 以 `sk-` 开头，并且余额充足。
  </Accordion>

  <Accordion title="网关需要 Authorization bearer token">
    Claude Code 的网关模式也支持 `ANTHROPIC_AUTH_TOKEN`。对于标准的 LemonData 配置，请继续使用 `ANTHROPIC_API_KEY`，除非你有意切换了网关认证行为。
  </Accordion>
