...
<Note>
  LangChain 中的 Agent API 正在不断演进。对于新项目，请考虑使用 [LangGraph](https://python.langchain.com/docs/langgraph) 以获得更灵活的智能体架构。
</Note>
...
<AccordionGroup>
  <Accordion title="根据成本选择合适的模型">
    在链中使用更便宜的模型（如 `gpt-4o-mini`）处理简单任务。
  </Accordion>

  <Accordion title="实现重试机制">
    LangChain 为临时错误内置了重试逻辑。
  </Accordion>
