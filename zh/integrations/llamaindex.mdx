---
title: "LlamaIndex"
description: "使用 OpenAI 兼容集成将 LemonData 与 LlamaIndex 集成"
---
...
对于 LemonData，更稳健的 LlamaIndex 配置方式是使用 **OpenAI 兼容集成**，而不是内置的 OpenAI 类。

当前的 LlamaIndex 文档明确建议对第三方 OpenAI 兼容端点使用 `OpenAILike`，因为内置的 OpenAI 类会根据官方模型名称推断元数据。

换句话说：这里应把 `OpenAILike` 视为受支持的 LemonData 路径，而不是内置 OpenAI 类。
