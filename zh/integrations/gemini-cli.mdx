...
Google Gemini CLI 是一个用于与 Gemini 模型交互的命令行工具。LemonData 提供了一个兼容的端点，允许您使用 Gemini CLI 访问数百个模型。


<Warning>
这只是实验性兼容方案：Gemini CLI 目前并没有正式文档化稳定的 LemonData 自定义 Base URL 工作流。如果您需要受支持的 LemonData 终端方案，优先使用 Codex CLI、Claude Code 或 OpenCode。
</Warning>
