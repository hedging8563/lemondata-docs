...
OpenCode 是一个在终端中运行的开源 AI 编码助手。它支持多个 LLM 提供商，并且可以配置为使用 LemonData 的 API，以访问 300+ 模型。
...
OpenCode 当前文档建议通过 `opencode.json` 或 `opencode.jsonc` 配置 provider。对于 LemonData，请使用自定义 provider，而不是复用内置的 `openai` provider：
...
<Note>
使用 `@ai-sdk/openai-compatible` 以获得 chat-completions 风格的兼容性。如果你明确需要基于 `/v1/responses` 构建的 provider 路径，OpenCode 的 provider 文档建议将 package 切换为 `@ai-sdk/openai`。
</Note>
