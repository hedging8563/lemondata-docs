...
OpenCode 是一个在终端中运行的开源 AI 编码助手。它支持多个 LLM 提供方，并且可以配置为使用 LemonData 的 API，以访问数百个模型。
...
OpenCode 当前文档建议通过 `opencode.json` 或 `opencode.jsonc` 配置提供方。对于 LemonData，请使用自定义提供方，而不是复用内置的 `openai` 提供方：
