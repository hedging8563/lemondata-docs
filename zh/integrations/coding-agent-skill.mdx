...
description: "为任意编码代理安装共享的 TokenLab API 集成技能，并具备 Agent-First 错误恢复能力。"
---

<Note>
这个技能会教你的编码代理只需通过描述需求，就能集成 TokenLab 的 **数百个 AI API**。它被打包为一个共享的 Agent-First 技能，并适用于任何支持 skill 或 rules 文件的编码代理。
...
这会从 TokenLab skills 仓库安装共享的 `tokenlab-api-integration` 技能。

如果你的工具不支持安装器，请将仓库中的 `skills/tokenlab-api-integration/` 文件夹复制到你工具的共享 skills 或 rules 目录中。
</Note>
...
如果你看到了 `tokenlab-api-integration`，则表示安装成功。
...
<Steps>
  ...
  <Step title="获取 API Key">
    进入 [Dashboard → API Keys](https://tokenlab.sh/dashboard/api) 并创建一个新的 key
  </Step>
  <Step title="复制 key">
...
请把这些 header 视为路由提示，而不是强制自动切换。Claude 原生调用使用 `/v1/messages`；Gemini 原生调用应遵循 `X-LemonData-Native-Endpoint` 返回的端点，通常是 `/v1beta/gemini`。
...
<CardGroup cols={2}>
  ...
  <Card title="成本管理" icon="wallet">
    在你的 [dashboard](https://tokenlab.sh/dashboard) 中设置使用限制。检查 `error.balance_usd` 以通过编程方式监控成本。
  </Card>
  <Card title="直接表达需求" icon="bullseye">
    说“我想在 Python 中生成图像”，而不是“TokenLab 有图像 API 吗？”
  </Card>
...
<AccordionGroup>
  <Accordion title="技能没有自动触发？">
    尝试在你的请求中提及 “TokenLab” 或 “TokenLab API”：
    ```
    我想用 TokenLab 把 GPT-4 集成到我的项目中
    ```
  </Accordion>

  <Accordion title="API 如何帮助 agent 找到正确的 model？">
    TokenLab 的 [Agent-First API](/guides/agent-first-api) 会返回结构化错误提示。如果 agent 猜错了 model 名称，错误中会包含带有正确名称的 `did_you_mean` 以及带有替代项的 `suggestions`。agent 会读取这些信息，并在下一次尝试时自动纠正。
  </Accordion>

  <Accordion title="支持哪些 coding agent？">
    任何支持共享 skill 或 rules 目录的 coding agent 都可以。当支持安装器时，`npx skills add` 会自动将共享的 `tokenlab-api-integration` 技能放置到检测到的位置。
  </Accordion>
...
<Info>
**有问题？** 请查看我们的 [GitHub Issues](https://github.com/hedging8563/tokenlab-skills/issues) 或联系 [support@tokenlab.sh](mailto:support@tokenlab.sh)
</Info>
