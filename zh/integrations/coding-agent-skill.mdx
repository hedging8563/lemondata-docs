...
description: "为任何编程智能体安装通用的 LemonData API 集成技能，并具备 Agent-First 错误恢复能力。"
---

<Note>
这个技能会教你的编程智能体仅通过描述需求，就把 LemonData 的 **300+ AI API** 接入到代码里。它被打包成一个共享的 agent-first 技能，适用于任何支持 skill 或 rules 文件的编程智能体。

这个 API 采用 **[Agent-First](/guides/agent-first-api)** 设计 —— 即使智能体猜错了模型名，错误响应也会明确告诉它如何修正。
</Note>
...
如果你的工具不支持安装器，就把仓库里的 `skills/lemondata-api-integration/` 目录复制到该工具的共享 skills 或 rules 目录。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="支持哪些编程智能体？">
    任何支持共享 skill 或 rules 目录的编程智能体都可以使用它。若工具支持安装器，`npx skills add` 会自动把共享的 `lemondata-api-integration` 技能放到检测到的位置。
  </Accordion>
...
<Info>
**有疑问？** 请查看我们的 [GitHub Issues](https://github.com/hedging8563/lemondata-skills/issues) 或联系 [support@lemondata.cc](mailto:support@lemondata.cc)
</Info>