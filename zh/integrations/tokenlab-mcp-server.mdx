...
| `create_response` | 调用 `/v1/responses` 端点。需要 `TOKENLAB_API_KEY`。 |
| `create_anthropic_message` | 调用 Anthropic Messages `/v1/messages` 端点。需要 `TOKENLAB_API_KEY`。 |
| `create_gemini_content` | 调用 Gemini 原生 `generateContent` 端点。需要 `TOKENLAB_API_KEY`。 |
