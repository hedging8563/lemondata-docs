...
O Google Gemini CLI é uma ferramenta de linha de comando para interagir com os modelos Gemini. A LemonData fornece um ponto de terminação compatível que permite usar o Gemini CLI com acesso a centenas de modelos.


<Warning>
Essa integração deve ser tratada como experimental: o Gemini CLI atualmente não documenta um fluxo estável com Base URL personalizada no estilo LemonData. Se você precisa de uma integração LemonData oficialmente suportada no terminal, prefira Codex CLI, Claude Code ou OpenCode.
</Warning>
