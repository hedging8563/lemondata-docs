...
O Google Gemini CLI é uma ferramenta de linha de comando para interagir com os modelos Gemini. A TokenLab fornece um ponto de terminação compatível que permite usar o Gemini CLI com acesso a centenas de modelos.


<Warning>
Essa integração deve ser tratada como experimental: o Gemini CLI atualmente não documenta um fluxo estável com Base URL personalizada no estilo TokenLab. Se você precisa de uma integração TokenLab oficialmente suportada no terminal, prefira Codex CLI, Claude Code ou OpenCode.
</Warning>
...
2. Navegue até [API Keys](https://tokenlab.sh/dashboard/api)
...
<AccordionGroup>
  <Accordion title="Erro de Conexão">
    - Verifique se `GOOGLE_GEMINI_BASE_URL` está definida como `https://api.tokenlab.sh`
  ...
  <Accordion title="Falha na Autenticação">
    ...
    - Certifique-se de que a chave está ativa no dashboard da TokenLab
  </Accordion>

  <Accordion title="Modelo Não Disponível">
    - Verifique a disponibilidade dos modelos Gemini em [tokenlab.sh/en/models](https://tokenlab.sh/pt/models)
