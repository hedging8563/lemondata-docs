...
description: "Instale o skill compartilhado de integração da API TokenLab para qualquer agente de programação, com recuperação de erros Agent-First."
---

<Note>
Este skill ensina seu agente de programação a integrar as **centenas de APIs de IA** da TokenLab apenas descrevendo o que você precisa. Ele é empacotado como um skill compartilhado agent-first e funciona com qualquer agente de programação que suporte arquivos de skill ou regras.
...
Isso instala o skill compartilhado `tokenlab-api-integration` do repositório de skills da TokenLab.

Se sua ferramenta não suportar o instalador, copie a pasta `skills/tokenlab-api-integration/` do repositório para o diretório compartilhado de skills ou regras da sua ferramenta.
</Note>
...
<Steps>
  ...
  <Step title="Obtenha a API Key">
    Navegue até [Dashboard → API Keys](https://tokenlab.sh/dashboard/api) e crie uma nova chave
  </Step>
...
Trate esses headers como dicas de roteamento, não como uma troca automática obrigatória. Chamadas nativas do Claude usam `/v1/messages`; chamadas nativas do Gemini devem seguir o endpoint indicado por `X-TokenLab-Native-Endpoint`, geralmente `/v1beta/gemini`.
...
| Geração de Vídeo | Sora, Kling, Veo, Seedance |
...
<CardGroup cols={2}>
  ...
  <Card title="Gestão de Custos" icon="wallet">
    Defina limites de uso no seu [dashboard](https://tokenlab.sh/dashboard). Verifique `error.balance_usd` para monitorar custos programaticamente.
  </Card>
...
<AccordionGroup>
  ...
  <Accordion title="Como a API ajuda o agente a encontrar o modelo certo?">
    A [API Agent-First](/guides/agent-first-api) da TokenLab retorna dicas de erro estruturadas. Se o agente adivinhar um nome de modelo errado, o erro inclui `did_you_mean` com o nome correto e `suggestions` com alternativas. O agente lê isso e corrige automaticamente na próxima tentativa.
  </Accordion>
...
<Info>
**Tem dúvidas?** Consulte nossas [GitHub Issues](https://github.com/hedging8563/tokenlab-skills/issues) ou entre em contato com [support@tokenlab.sh](mailto:support@tokenlab.sh)
</Info>
