...
Os perfis `core` e `full` publicam por padrão esquemas de ferramentas `portable` com limites de tamanho e profundidade, enquanto o servidor valida cada chamada com o contrato OpenAPI completo. Use `TOKENLAB_MCP_SCHEMA_MODE=exact` para esquemas aninhados completos ou `TOKENLAB_MCP_SCHEMA_MODE=strict` para providers que exigem objetos fechados e todas as propriedades em `required`; o modo strict representa valores complexos de nível superior como strings JSON.
...
| Argumentos | `-y` e depois `@tokenlabai/mcp-server` | Use `@tokenlabai/mcp-server@0.6.15` para fixar a versão atualmente verificada. |
