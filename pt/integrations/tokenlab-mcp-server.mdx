...
Use-o quando um agente precisar do contrato atual da API do TokenLab em vez de uma lista de ferramentas mantida manualmente. O perfil `core` padrão expõe 29 ferramentas de endpoint geradas, além de duas ferramentas de descoberta compostas. O perfil `full` expõe 75 ferramentas de endpoint geradas, além das mesmas duas ferramentas compostas.
...
| Argumentos | `-y` e depois `@tokenlabai/mcp-server` | Use `@tokenlabai/mcp-server@0.6.10` para fixar a versão atualmente verificada. |
...
| `full` | 77 | 75 ferramentas de endpoint geradas mais as duas ferramentas de descoberta compostas. Adiciona ciclo de vida de resposta, lotes, mundos, descoberta de modelos nativos e outras operações de desenvolvedor permitidas. |
