...
description: "Conecte clientes MCP ao catálogo público de modelos, detalhes de modelos, preços e orientações de endpoints nativos do TokenLab."
---

O [servidor MCP do TokenLab](https://github.com/hedging8563/tokenlab-mcp-server) oferece aos agentes compatíveis com MCP uma forma somente leitura de descobrir modelos do TokenLab antes de escreverem código ou escolherem uma rota de API.

Use-o quando um agente precisar comparar modelos disponíveis, inspecionar formatos de solicitação suportados, verificar preços ou entender qual família de endpoint nativo usar para uma tarefa.

<Note>
As ferramentas do catálogo público não exigem uma chave de API do TokenLab. O servidor não faz proxy de solicitações de inferência pagas.
</Note>
...
5. Use `get_api_overview` quando o agente precisar de um mapa compacto das famílias de API do TokenLab.
...
<AccordionGroup>
  ...
  <Accordion title="Este servidor pode chamar APIs de inferência pagas do TokenLab?">
    Não. Ele é intencionalmente somente leitura para dados de catálogo público, contrato, preços e visão geral. Use a API do TokenLab diretamente para inferência.
  </Accordion>
</AccordionGroup>
