...
A Anthropic agora recomenda o instalador nativo. A instalação via npm continua disponível por compatibilidade, mas está depreciada na origem.

```bash
curl -fsSL https://claude.ai/install.sh | bash
```

Se você realmente precisar do pacote npm, use:
...
<Warning>
Não use `sudo npm install -g` para Claude Code.
</Warning>
