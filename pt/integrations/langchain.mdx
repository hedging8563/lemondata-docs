...
<AccordionGroup>
  <Accordion title="Passe `base_url` explicitamente">
    A configuração mais confiável do LemonData é passar `base_url="https://api.tokenlab.sh/v1"` diretamente para `ChatOpenAI` e `OpenAIEmbeddings`, em vez de depender de aliases antigos de variáveis de ambiente.
  </Accordion>
