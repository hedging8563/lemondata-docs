...
O streaming permite que você receba partes do conteúdo em tempo real à medida que a resposta é gerada, proporcionando uma melhor experiência de usuário para aplicativos de chat.

## Habilitando o Streaming

Configure `stream: true` em sua requisição:
...
Cada bloco (`chunk`) no streaming segue o seguinte formato:
...
O streaming termina em:
- `finish_reason: "stop"` - Conclusão normal
- `finish_reason: "length"` - Limite de `max_tokens` atingido
- `finish_reason: "tool_calls"` - O modelo deseja chamar ferramentas
- `data: [DONE]` - Última mensagem
...
## Streaming Assíncrono (Async Streaming)

Para aplicativos assíncronos:
...
Para interfaces de chat na web:
