...
description: "Obtém o status e o resultado de uma tarefa de geração de vídeo"
---

## Parâmetros de Path

<ParamField path="task_id" type="string" required>
  O ID da tarefa retornado na solicitação de criação de vídeo.
</ParamField>
...
<ResponseField name="completed_at" type="integer">
  Timestamp de conclusão (quando concluído).
</ResponseField>
...
- Realize o polling a cada 5-10 segundos
- Implemente o backoff exponencial para tarefas mais demoradas
- Defina um tempo limite (timeout) máximo (por exemplo, 10 minutos)
- Trate adequadamente o status `failed`
