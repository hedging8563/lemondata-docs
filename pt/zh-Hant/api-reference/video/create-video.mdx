---
title: "Criar Vídeo"
openapi: "POST /v1/videos/generations"
description: "Criar uma tarefa de geração de vídeo"
---

## Visão Geral

A geração de vídeo é assíncrona. Você envia uma solicitação e recebe um `task_id`, e então realiza o polling para obter o resultado.

## Corpo da Requisição

<ParamField body="model" type="string" default="kling-video">
  Modelo de vídeo (ex: `kling-video`, `veo3.1`, `wan2.6-i2v`).
</ParamField>

<ParamField body="prompt" type="string" required>
  Descrição em texto do vídeo a ser gerado.
</ParamField>

<ParamField body="image_url" type="string">
  Imagem inicial opcional para geração de imagem para vídeo (`image-to-video`).
</ParamField>

<ParamField body="duration" type="integer">
  Duração do vídeo em segundos (depende do modelo).
</ParamField>

<ParamField body="aspect_ratio" type="string">
  Proporção de tela (ex: `16:9`, `9:16`, `1:1`).
</ParamField>

<ParamField body="resolution" type="string">
  Resolução do vídeo (ex: `1080p`, `720p`).
</ParamField>

<ParamField body="fps" type="integer">
  Quadros por segundo (1-120).
</ParamField>

<ParamField body="negative_prompt" type="string">
  Conteúdo a ser evitado na geração do vídeo.
</ParamField>

<ParamField body="seed" type="integer">
  Semente aleatória para gerações reproduzíveis.
</ParamField>

<ParamField body="cfg_scale" type="number">
  Escala de orientação sem classificador (0-20).
</ParamField>

<ParamField body="motion_strength" type="number">
  Intensidade do movimento (0-1).
</ParamField>

<ParamField body="start_image" type="string">
  URL da imagem do quadro inicial.
</ParamField>

<ParamField body="end_image" type="string">
  URL da imagem do quadro final.
</ParamField>

<ParamField body="user" type="string">
  Identificador único do usuário final.
</ParamField>

## Resposta

<ResponseField name="task_id" type="string">
  Identificador único da tarefa para polling.
</ResponseField>

<ResponseField name="status" type="string">
  Status inicial: `pending`.
</ResponseField>

<ResponseField name="model" type="string">
  Modelo utilizado.
</ResponseField>

<RequestExample>
```bash cURL
curl -X POST "https://api.lemondata.cc/v1/videos/generations" \
  -H "Authorization: Bearer sk-your-api-key" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "kling-video",
    "prompt": "A cat walking through a garden, cinematic lighting",
    "duration": 5
  }'
```

```python Python
import requests

response = requests.post(
    "https://api.lemondata.cc/v1/videos/generations",
    headers={
        "Authorization": "Bearer sk-your-api-key",
        "Content-Type": "application/json"
    },
    json={
        "model": "kling-video",
        "prompt": "A cat walking through a garden, cinematic lighting",
        "duration": 5
    }
)

data = response.json()
task_id = data["task_id"]
print(f"Task ID: {task_id}")
```

```javascript JavaScript
const response = await fetch('https://api.lemondata.cc/v1/videos/generations', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer sk-your-api-key',
    'Content-Type': 'application/json'
  },
  body: JSON.stringify({
    model: 'kling-video',
    prompt: 'A cat walking through a garden, cinematic lighting',
    duration: 5
  })
});

const data = await response.json();
console.log(`Task ID: ${data.task_id}`);
```

```go Go
package main

import (
    "bytes"
    "encoding/json"
    "fmt"
    "net/http"
)

func main() {
    payload := map[string]interface{}{
        "model":    "kling-v2.6-pro",
        "prompt":   "A cat walking through a garden, cinematic lighting",
        "duration": 5,
    }
    body, _ := json.Marshal(payload)

    req, _ := http.NewRequest("POST", "https://api.lemondata.cc/v1/videos/generations", bytes.NewBuffer(body))
    req.Header.Set("Authorization", "Bearer sk-your-api-key")
    req.Header.Set("Content-Type", "application/json")

    client := &http.Client{}
    resp, _ := client.Do(req)
    defer resp.Body.Close()

    var result map[string]interface{}
    json.NewDecoder(resp.Body).Decode(&result)
    fmt.Printf("Task ID: %s\n", result["task_id"])
}
```

```php PHP
<?php
$ch = curl_init('https://api.lemondata.cc/v1/videos/generations');

curl_setopt_array($ch, [
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_POST => true,
    CURLOPT_HTTPHEADER => [
        'Content-Type: application/json',
        'Authorization: Bearer sk-your-api-key'
    ],
    CURLOPT_POSTFIELDS => json_encode([
        'model' => 'kling-v2.6-pro',
        'prompt' => 'A cat walking through a garden, cinematic lighting',
        'duration' => 5
    ])
]);

$response = curl_exec($ch);
curl_close($ch);

$data = json_decode($response, true);
echo "Task ID: " . $data['task_id'];
```
</RequestExample>

<ResponseExample>
```json Response
{
  "task_id": "video_abc123",
  "status": "pending",
  "model": "kling-video",
  "created_at": 1706000000
}
```
</ResponseExample>

## Imagem para Vídeo

```python
response = requests.post(
    "https://api.lemondata.cc/v1/videos/generations",
    headers={"Authorization": "Bearer sk-your-api-key"},
    json={
        "model": "wan2.6-i2v",
        "prompt": "The person starts walking forward",
        "image_url": "https://example.com/image.jpg"
    }
)
```

## Modelos Disponíveis

| Modelo | Tipo | Descrição |
|-------|------|-------------|
| `kling-video` | T2V/I2V | Movimento realista, múltiplas versões (v1.0-v2.6) |
| `veo3.1` | T2V | Modelo mais recente do Google |
| `veo3.1-pro` | T2V | Qualidade de nível profissional |
| `minimax/video-01` | T2V | Alto ritmo e estabilidade |
| `wan2.6-i2v` | I2V | Imagem para vídeo |