...
description: "Gere conteúdo usando o formato da API Google Gemini"
---

LemonData suporta o formato nativo da API Google Gemini para modelos Gemini. Isso permite compatibilidade direta com o Google AI SDK.
...
<ParamField query="key" type="string">
  API key (alternativa para autenticação via cabeçalho).
</ParamField>

## Autenticação

O endpoint Gemini suporta vários métodos de autenticação:
...
<ParamField body="generationConfig" type="object">
  ...
  - `topP` (number): Probabilidade de amostragem de núcleo (nucleus sampling)
  - `topK` (integer): Amostragem Top-K
  - `maxOutputTokens` (integer): Número máximo de tokens de saída
...
<ResponseField name="candidates" type="array">
  Candidatos de conteúdo gerados.
</ResponseField>
