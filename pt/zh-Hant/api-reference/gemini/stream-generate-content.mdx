---
title: "Streaming de Geração de Conteúdo"
openapi: "POST /v1beta/models/{model}:streamGenerateContent"
description: "Gere conteúdo via streaming usando o formato da API Google Gemini"
---

Versão de streaming do endpoint `generateContent` do Gemini. Retorna Server-Sent Events.
...
<ParamField query="key" type="string">
  Chave de API (alternativa para autenticação via cabeçalho).
</ParamField>

## Corpo da Requisição

O mesmo que [Gerar Conteúdo](/api-reference/gemini/generate-content).

## Resposta

Retorna um fluxo de objetos JSON, cada um contendo uma resposta parcial.
