...
<Warning>
  **Importante**: Tarefas de geração de vídeo, música e 3D são **cobradas no envio**, não na conclusão.
</Warning>

Para tarefas de geração assíncronas (vídeo, música, modelos 3D):

1. **A cobrança ocorre imediatamente** ao enviar a tarefa
2. Você recebe um `task_id` para consultar o status
3. **Sem reembolso automático** se a tarefa falhar

Isso ocorre porque os provedores upstream nos cobram quando a tarefa é enviada, independentemente do resultado. Se você tiver falhas nas tarefas, entre em contato com support@lemondata.cc.

```python
# Exemplo: Cobrança de geração de vídeo
response = client.post("/v1/video/generations", json={
    "model": "kling-v1",
    "prompt": "Um pôr do sol sobre o oceano"
})
# ⚠️ Você é cobrado AGORA, não quando o vídeo estiver pronto
