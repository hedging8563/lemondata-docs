...
Os exemplos públicos atuais usam `suno_music` para geração de música e incluem `mv` com uma versão oficial do modelo Suno, como `chirp-v4`. Para fluxos apenas de letras, envie `action: "LYRICS"` com um modelo cujo detalhes do modelo documente a geração de letras e omita `mv`. Trate os IDs de modelo como IDs públicos do TokenLab, não como uma garantia de que campos específicos do provedor são campos de detalhes do modelo.
...
A resposta de criação é um registro de tarefa consultável, não o áudio final:
...
Uma resposta de polling concluída pode incluir os campos de mídia finais:
...
Os campos de mídia finais ficam ausentes até que `status` seja `completed`. Tarefas com falha retornam `status: "failed"` com `error`.
