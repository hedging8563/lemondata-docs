...
- **Strong native path** significa que o repositório também possui evidências diretas de adaptador ou formato de requisição para essa família de protocolos.
- **Best-effort** significa que a integração pode funcionar, mas o cliente upstream não trata este fluxo de gateway personalizado como um contrato estável.
...
| **Anthropic SDK** | Python/JS | Strong native path | Rota Messages nativa com evidências diretas para ferramentas, *thinking* e cache de prompt |
...
| **LlamaIndex** | Python | Suportado via `OpenAILike` | Use `OpenAILike`, não as classes nativas da OpenAI, para gateways de terceiros como o TokenLab |
...
| **Helicone** | Gateway / Observabilidade | Caminho de observabilidade suportado | Roteie requisições compatíveis com OpenAI através do Helicone Gateway com `Helicone-Target-Url: https://api.tokenlab.sh` |
| **LangBot** | Plataforma de Bot | Caminho de chat suportado | Use o requisitante do TokenLab quando disponível, ou configure um requisitante compatível com OpenAI com o endpoint `/v1` do TokenLab |
...
# Rastreamento de uso em Chat Completions
