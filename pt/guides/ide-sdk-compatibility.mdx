...
- **Suportado** significa que documentamos um caminho de configuração concreto e o TokenLab expõe a estrutura de protocolo que esse caminho espera.
- **Strong native path** (Caminho nativo robusto) significa que o repositório também possui evidências de adaptador direto ou formato de solicitação para essa família de protocolos.
- **Best-effort** (Melhor esforço) significa que a integração pode funcionar, mas o cliente upstream não trata esse fluxo de gateway personalizado como um contrato estável.
...
|------|---------------|--------|-------|
| **Cursor** | Suportado com limites | Compatível com OpenAI | Funciona para fluxos padrão de chat/editor BYOK, não como substituto para recursos gerenciados pelo Cursor, como Tab Completion |
| **Claude Code CLI** | Strong native path | Anthropic | Rota nativa `/v1/messages` com cobertura de adaptador para *thinking* e `tool_choice` |
...
| **Gemini CLI** | Best-effort / experimental | Gemini | O fluxo de URL base personalizado do TokenLab não é uma integração oficial da Gemini CLI |
...
<Tabs>
  <Tab title="Cursor">
    ...
    ```
    O Cursor usa internamente o formato de ferramenta estilo Anthropic. O TokenLab suporta ambos:
...
| **Anthropic SDK** | Python/JS | Strong native path | Rota nativa Messages com evidência direta para ferramentas, *thinking* e *prompt caching* |
| **Vercel AI SDK** | TypeScript | Padrão de integração recomendado | Prefira `@ai-sdk/openai-compatible`; use `@ai-sdk/openai` apenas quando quiser explicitamente o comportamento nativo de Responses |
...
| **Dify** | - | Suportado com limites de escopo | O provedor OpenAI e fluxos orientados a chat-completions são o caminho pretendido; não é adequado para Responses ou comportamento de WebSocket específicos do Codex |
...
| **Phoenix** | Observabilidade | Caminho de rastreamento suportado | Auto-instrumente o cliente SDK da OpenAI configurado com a URL base `/v1` do TokenLab |
| **Opik** | Observabilidade | Caminho de rastreamento suportado | Envolva um cliente SDK da OpenAI configurado com a URL base `/v1` do TokenLab |
| **LangBot** | Plataforma de Bot | Caminho de chat suportado | Use o solicitante do TokenLab quando disponível, ou configure um solicitante compatível com OpenAI com o endpoint `/v1` do TokenLab |
...
| `max_tokens` | integer | Tokens de saída máximos |
...
| `max_output_tokens` | integer | Tokens de saída máximos |
...
Suporta formatos de ferramenta da OpenAI e da Anthropic:
...
<AccordionGroup>
  ...
  <Accordion title="Use stream_options.include_usage apenas para Chat Completions">
    Para streaming de Chat Completions, habilite `stream_options.include_usage` para contagens precisas de tokens. Responses tem seu próprio contrato de streaming e não expõe essa opção exclusiva de Chat.
  </Accordion>
