...
TokenLab expõe múltiplos formatos de API para que ferramentas comuns de codificação, SDKs e frameworks possam se integrar com código auxiliar mínimo.

Esta página é intencionalmente mais restrita do que uma matriz de marketing:

- **Suportado** significa que documentamos um caminho de configuração concreto e que a TokenLab expõe o formato de protocolo que esse caminho espera.
...
<Note>
  Outros editores e ferramentas de agentes compatíveis com OpenAI geralmente funcionam com o mesmo padrão de base URL; verifique o suporte a provedores personalizados antes de usar em produção.
</Note>
...
<Tabs>
  <Tab title="Cursor">
    ...
    ```
    O Cursor usa internamente o formato de ferramentas no estilo Anthropic. A TokenLab suporta ambos:
...
Endpoints de geração que expõem `stream: true`, incluindo Chat Completions e Responses, usam streaming via Server-Sent Events (SSE):
...
A TokenLab retorna respostas de erro compatíveis com OpenAI:
...
<AccordionGroup>
  <Accordion title="Use passthrough para parâmetros desconhecidos">
    Parâmetros desconhecidos só são encaminhados quando a rota pública e o modelo selecionados oferecem suporte.
  </Accordion>

  <Accordion title="Use stream_options.include_usage apenas em Chat Completions">
    Em streaming de Chat Completions, habilite `stream_options.include_usage` para contagens de tokens precisas. Responses tem seu próprio contrato de streaming e não expõe esta opção exclusiva de Chat.
  </Accordion>
