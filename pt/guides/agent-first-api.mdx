...
A Agent-First API da LemonData enriquece respostas de erro com dicas estruturadas que agentes de IA podem analisar e agir imediatamente — sem buscas na web, sem consultas à documentação, sem suposições.
...
<Note>
Endpoints compatíveis com OpenAI usam os tipos de erro públicos estáveis da LemonData, como `rate_limit_exceeded`, `upstream_error` e `all_channels_failed`. Endpoints compatíveis com Anthropic e Gemini usam suas próprias formas de resposta nativas.
</Note>
...
| `cache_pricing` | object or `null` | Preços do cache de prompt upstream + desconto do cache semântico da plataforma |
| `agent_preferences.<scene>` | object | Snapshot de recomendação não-chat para essa cena |

`agent_preferences` é derivado de um snapshot de taxa de sucesso em cache de 24 horas:
