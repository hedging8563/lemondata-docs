...
A API Agent-First da LemonData enriquece as respostas de erro com dicas estruturadas que os agentes de IA podem analisar e processar imediatamente — sem buscas na web, sem consultas a documentos, sem adivinhações.

Cada resposta de erro inclui campos opcionais como `did_you_mean`, `suggestions`, `hint`, `retryable` e `retry_after` dentro do objeto `error` padrão. Esses campos são retrocompatíveis — clientes que não os utilizam não verão diferença.
