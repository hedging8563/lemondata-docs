...
O [cache semântico](/guides/caching) do LemonData corresponde requisições por significado, não por texto exato. Isso é poderoso para agentes de código porque:
...
O [cache de prompt upstream](/guides/prompt-cache) é automático pelo LemonData. Prompts de sistema longos — que agentes de código sempre incluem — são armazenados em cache no nível do provedor:
...
<AccordionGroup>
  ...
  <Accordion title="Cursor">
    Configurações → Modelos → Chave da API da OpenAI: `sk-your-key`, URL base: `https://api.lemondata.cc/v1`
