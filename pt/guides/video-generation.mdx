...
Quando o modelo Seedance selecionado pode usar a biblioteca de materiais do TokenLab, URLs de imagem e data URLs inline compatíveis são preparadas como materiais reutilizáveis do TokenLab antes da geração. Se a preparação levar mais de 60 segundos, tente novamente quando os `auto_material_asset_ids` retornados estiverem `ACTIVE`. Se o modelo selecionado não puder usar a biblioteca de materiais, entradas comuns de imagem continuam pelo caminho normal de imagem.
...
Use os IDs de modelo mostrados pelo TokenLab em `model` e escolha variantes de recurso com `operation` e as entradas de mídia correspondentes. Exemplos: `wan-2.7`, `happyhorse-1.0`, `viduq3`, `viduq3-mix`, `pixverse-v6`, `kling-3.0-video`, `veo3.1` e `seedance-2.0`; não use sufixos específicos de operação como nomes de modelo do TokenLab.
