...
Para video-to-video com `grok-imagine-video`, envie uma URL HTTPS pública `.mp4` em `video_url`. O TokenLab traduz isso para o corpo REST xAI `video.url`; `duration`, `aspect_ratio` e `resolution` não são aceitos nesse fluxo de edição.
