...
A Responses API é a API de conversação com estado mais recente da OpenAI. A TokenLab suporta esse formato como um caminho opcional avançado para modelos compatíveis; use `POST /v1/chat/completions` como rota padrão compatível com OpenAI, a menos que você precise explicitamente do comportamento específico de Responses.
