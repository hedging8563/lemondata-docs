...
| `/v1/tasks/{id}` | GET | Obter o status assíncrono de tarefas de vídeo |
| `/v1/videos/generations/{id}` | GET | Caminho de status de vídeo mantido por compatibilidade |

<Info>
Para clientes novos, priorize `/v1/tasks/{id}` e siga o `poll_url` retornado pelas respostas de criação. Mantenha `/v1/videos/generations/{id}` apenas por compatibilidade com versões anteriores.
</Info>
