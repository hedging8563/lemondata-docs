...
Variante SSE nativa e em streaming do Gemini `generateContent`. O TokenLab preserva eventos, ordem e campos do upstream; eventos apenas com metadados, chunks intermediários sem `finishReason` e EOF natural são válidos. Não adiciona o marcador `[DONE]` de Chat Completions nem faz fallback para Chat.
...
`generationConfig.candidateCount`, combinações de ferramentas e campos futuros são encaminhados sem alteração ao upstream; o TokenLab não impõe limite próprio de streaming.
