...
<ParamField body="contents" type="array" required>
  ...
  - `parts` (array): Partes do conteúdo (texto ou parts de imagem `inline_data`)
</ParamField>
...
Para solicitações de visão do Gemini, coloque as imagens em `contents[].parts[]` como parts estruturadas `inline_data`. O campo `data` deve conter os bytes do arquivo codificados em Base64, e `mime_type` deve corresponder ao formato real da imagem.
