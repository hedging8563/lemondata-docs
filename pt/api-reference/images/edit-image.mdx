...
<Note>
`gpt-image-2` é compatível com este endpoint. Envie uploads multipart `image`, JSON `image_url`, JSON `image_urls` ou o array JSON oficial `images` com referências `image_url` ou `file_id`; as edições GPT Image aceitam até 16 imagens de origem. Os valores `file_id` devem ser criados pelo LemonData `/v1/files` com `purpose=user_data` ou `purpose=vision`, para que o gateway possa vinculá-los ao mesmo canal upstream de edição de imagens. Os parâmetros opcionais são `mask`, `n`, `size`, `quality`, `response_format`, `output_format`, `output_compression` ou `compression`, `moderation`, `partial_images` e `user`. `background` permanece apenas no fluxo de geração de imagens e não faz parte do contrato público atual de edição de imagem do `gpt-image-2`. A cobrança segue os detalhes de tokens de uso do upstream, incluindo tokens de entrada de imagem e tokens de saída de imagem adicionados por partial images.
</Note>
...
<ParamField body="image" type="file" required>
  A imagem a ser editada. Deve ser um arquivo PNG válido, menor que 50MB e quadrado. A imagem deve ter transparência onde as edições forem desejadas.
</ParamField>
