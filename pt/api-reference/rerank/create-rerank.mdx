...
<ParamField body="top_n" type="integer">
  Número de resultados principais a retornar. O padrão é retornar todos os documentos. Deve ser pelo menos `1` e não pode ser maior que `documents.length`. Atualmente a LemonData não tem um limite rígido inferior específico de provedor sob governança; se um provedor publicar um posteriormente, ele deverá ser adicionado primeiro à truth de request-shape de rerank antes de ser documentado ou aplicado.
</ParamField>
