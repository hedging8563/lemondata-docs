...
تثري واجهة برمجة تطبيقات LemonData Agent-First استجابات الأخطاء بإرشادات مهيكلة يمكن لوكلاء الذكاء الاصطناعي تحليلها والتصرف بناءً عليها فورًا — دون بحث على الويب، دون الرجوع إلى الوثائق، ودون تخمين.
...
<Note>
تستخدم نقاط النهاية المتوافقة مع OpenAI أنواع أخطاء عامة ومستقرة من LemonData مثل `rate_limit_exceeded` و`upstream_error` و`all_channels_failed`. تستخدم نقاط النهاية المتوافقة مع Anthropic وGemini هياكل الاستجابة الأصلية الخاصة بها.
</Note>
...
| `cache_pricing` | object or `null` | أسعار ذاكرة التخزين المؤقت للمطالبات الصاعدة + خصم ذاكرة التخزين الدلالية على المنصة |
| `agent_preferences.<scene>` | object | لقطة توصية غير محادثة لذلك المشهد |

يُستمد `agent_preferences` من لقطة معدل نجاح مخبأة لمدة 24 ساعة:
