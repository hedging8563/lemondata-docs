...
تكشف TokenLab عن عدة صيغ API حتى تتمكن أدوات الترميز الشائعة وSDKs والأطر من التكامل مع أقل قدر ممكن من كود الربط.
...
| المعلمة | النوع | الوصف |
|-----------|------|-------------|
| `stream_options` | object | Chat Completions فقط: `{ include_usage: true }` لحساب عدد الرموز |
...
تستخدم نقاط نهاية التوليد التي تعرض `stream: true`، بما في ذلك Chat Completions وResponses، بث Server-Sent Events (SSE):
...
<AccordionGroup>
  <Accordion title="استخدم passthrough للمعلمات غير المعروفة">
    لا يتم تمرير المعلمات غير المعروفة إلا عندما يدعمها المسار العام والنموذج المحددان.
  </Accordion>

  <Accordion title="استخدم stream_options.include_usage مع Chat Completions فقط">
    في بث Chat Completions، فعّل `stream_options.include_usage` للحصول على عدّ دقيق للـ tokens. تستخدم Responses تنسيق بث خاصًا بها ولا تعرض خيار Chat-only هذا في واجهة API العامة.
  </Accordion>
