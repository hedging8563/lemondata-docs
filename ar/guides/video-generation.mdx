...
في video-to-video مع `grok-imagine-video`، أرسل رابط HTTPS عام بصيغة `.mp4` في `video_url`. يحوله TokenLab إلى جسم REST الخاص بـ xAI بصيغة `video.url`؛ ولا يقبل هذا مسار التحرير `duration` أو `aspect_ratio` أو `resolution`.
