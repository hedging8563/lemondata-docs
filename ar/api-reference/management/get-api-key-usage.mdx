...
- تظل جميع الحقول المالية متاحة بقيم USD الأصلية، وتضيف الآن أيضًا حقول عرض بالرنمينبي مثل `cost_cny` و `total_cost_cny` و `cost_cny` داخل كل breakdown.
- يمثل `api_key.exchange_rate` سعر USD→CNY المستخدم لكل قيم `*_cny` في تلك الاستجابة.
