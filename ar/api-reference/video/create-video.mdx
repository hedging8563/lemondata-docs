...
يعتمد إخراج الصوت على النموذج. في TokenLab، تُعامل عائلة Veo 3 على أن الصوت مفعّل افتراضيًا عند حذف `output_audio`. وإذا كان النموذج يدعم التحكم بالصوت، فاستخدم `output_audio` لتفعيله أو تعطيله بشكل صريح. كما يُقبل الاسم البديل camelCase وهو `outputAudio` لأغراض التوافق.
