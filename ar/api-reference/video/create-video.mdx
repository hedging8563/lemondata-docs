...
<ParamField body="kling_elements" type="array">
  تعريفات مراجع العناصر في `kling-3.0-video`. مدعومة فقط للطلبات المشروطة بصورة. عرّف من 1 إلى 3 عناصر؛ يحتوي كل عنصر على `name` و`description` اختياري و`element_input_urls` مع 2-4 روابط صور. استخدم `@name` داخل `prompt` للإشارة إلى العنصر.
</ParamField>
...
<ParamField body="output_audio" type="boolean">
  مفتاح إخراج صوت يعتمد على النموذج. في LemonData، تستخدم عائلة Veo 3 القيمة الافتراضية `true` عندما يُحذف هذا الحقل. يقبل `kling-3.0-video` هذا المحدد أيضًا ويحوّله إلى تحكم upstream المتوافق `sound`؛ وتبقى طلبات Kling صامتة افتراضيًا عند حذفه. كما يُقبل الاسم البديل camelCase وهو `outputAudio` للتوافق.
</ParamField>
...
استخدم `kling_elements` مع `kling-3.0-video` عندما تحتاج إلى مراجع عناصر. يجب أن يتضمن الطلب إدخال صورة (`image_url` أو `image_urls` أو `start_image` أو `end_image`) ثم تشير إلى كل عنصر في النص باستخدام `@name`.
