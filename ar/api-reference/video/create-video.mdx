...
<Info>
بالنسبة إلى نماذج Seedance 2.0، يجهّز TokenLab حقول الصور (`image` و`image_url` و`image_urls` و`reference_images` و`start_image` و`end_image`) تلقائيًا كمواد أفاتار افتراضي قابلة لإعادة الاستخدام قبل التوليد. إذا لم تكتمل التهيئة خلال 60 ثانية، تعيد الواجهة `409 seedance_material_preparing` مع `auto_material_asset_ids`؛ أعد المحاولة بعد أن تصبح هذه المواد `ACTIVE`.
</Info>
...
<ParamField body="draft" type="boolean">
  علامة سير عمل المسودة في Seedance 1.5 Pro. استخدم `draft=true` لإنشاء مهمة مسودة على مسارات Seedance التي تدعم المسودات. لا ترسله مع `draft_task_id`.
</ParamField>
