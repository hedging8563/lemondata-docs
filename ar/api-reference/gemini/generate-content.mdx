...
يعرض TokenLab هذا المسار فقط للنماذج التي تعلن عقد Gemini الأصلي ولها قناة upstream من البروتوكول نفسه؛ ولا يعود الطلب إلى Chat Completions. أسماء ProtoJSON بصيغة lowerCamelCase وأسماء proto الأصلية بصيغة snake_case رسمية معًا، وتُحفظ بما في ذلك الطلبات المختلطة. تُمرر الحقول غير المعروفة بأفضل جهد ويقرر upstream دعمها.
## معلمات المسار
...
<ParamField body="contents" type="array" required>
  محتوى المحادثة. تُحفظ قيم `role` وكل `parts` كما أرسلها العميل من دون trim أو lowercase أو إعادة ترميز Base64 أو استنتاج MIME. الصيغتان `inlineData` / `inline_data` و`fileData` / `file_data` رسميتان؛ ولا تشترط TokenLab محليًا `fileData.mimeType`. عند الإشارة إلى Files أو Cache أنشأتها TokenLab، يُفحص المالك ويُربط مع upstream الأصلي، وتُترك صلاحية باقي الوسائط والأدوات للمزوّد.
</ParamField>
...
<ParamField body="generationConfig" type="object">
  إعدادات الإنشاء الأصلية. يُقبل كل من أسماء lowerCamelCase مثل `maxOutputTokens` و`candidateCount` و`thinkingConfig` وأسماء snake_case المناظرة كما هو. لا تزيل TokenLab `thinkingConfig` بعد الفشل ولا تفرض حدًا محليًا لـ `candidateCount` في البث؛ ويقرر upstream النطاقات والتركيبات المدعومة.
</ParamField>
