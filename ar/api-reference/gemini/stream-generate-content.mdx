...
نسخة SSE الأصلية المتدفقة من Gemini `generateContent`. تحفظ TokenLab أحداث upstream وترتيبها وحقولها، وتسمح بالأحداث التي تحتوي metadata فقط، وبالأجزاء الوسيطة من دون `finishReason`، وبنهاية EOF طبيعية. لا تضيف علامة Chat Completions `[DONE]` ولا تعود إلى Chat.
## معلمات المسار
...
تُمرر `generationConfig.candidateCount` وتركيبات الأدوات والحقول المستقبلية إلى upstream كما هي؛ ولا تفرض TokenLab حدًا محليًا خاصًا بالبث.
