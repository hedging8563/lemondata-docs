...
<Note>
`gpt-image-2` مدعوم في نقطة النهاية هذه. Send multipart `image` uploads, JSON `image_url`, JSON `image_urls`, or the official JSON `images` array with `image_url` references; GPT Image edits accept up to 16 source images. Optional parameters are `mask`, `n`, `size`, `quality`, `response_format`, `output_format`, `output_compression` or `compression`, `moderation`, `partial_images`, and `user`. يبقى `background` خاصًا بمسار إنشاء الصور ولا يُعد جزءًا من عقد التحرير العام الحالي لـ `gpt-image-2`. Billing follows the upstream usage token details, including image input tokens and any image output tokens added for partial images.
</Note>
