...
<Note>
`gpt-image-2` مدعوم في نقطة النهاية هذه. يمكنك إرسال تحميلات multipart باسم `image`، أو JSON `image_url`، أو JSON `image_urls`، أو مصفوفة JSON الرسمية `images` مع مراجع `image_url` أو `file_id`؛ تقبل تعديلات GPT Image حتى 16 صورة مصدر. يجب إنشاء قيم `file_id` عبر LemonData `/v1/files` باستخدام `purpose=user_data` أو `purpose=vision` حتى تتمكن البوابة من ربطها بنفس قناة تحرير الصور upstream. المعلمات الاختيارية هي `mask` و `n` و `size` و `quality` و `response_format` و `output_format` و `output_compression` أو `compression` و `moderation` و `partial_images` و `user`. يبقى `background` خاصًا بمسار إنشاء الصور ولا يُعد جزءًا من عقد التحرير العام الحالي لـ `gpt-image-2`. تتبع الفوترة تفاصيل usage token من upstream، بما في ذلك رموز إدخال الصور وأي رموز إخراج صور إضافية تنتج عن partial images.
</Note>

## جسم الطلب

<ParamField body="image" type="file" required>
  الصورة المراد تعديلها. يجب أن تكون ملف PNG صالح، أقل من 50MB، ومربعة. يجب أن تحتوي الصورة على مناطق شفافة حيث يُراد التعديل.
</ParamField>
