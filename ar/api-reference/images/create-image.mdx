...
<ParamField body="response_format" type="string" default="url">
  ...
  بالنسبة إلى مسارات `gpt-image-2` على Azure Official أو القنوات المتوافقة مع Azure، لا تمرّر TokenLab الحقل `response_format` إلى upstream. يحصل gateway دائمًا على بيانات الصور من upstream بصيغة `b64_json`؛ عند طلب `url` يرفع كل صورة إلى CDN ويعيد `data[].url`. إذا لم يكن تخزين CDN متاحًا أو فشل الرفع، يفشل الطلب بدلاً من الرجوع إلى Base64. عند طلب `b64_json` يُعاد Base64 الخام.
</ParamField>
