...
<ParamField body="resolution" type="string">
  ...
  مدعوم في `gemini-3-pro-image-preview` و`nano-banana-pro` و`nano-banana-2` وعائلات الدقة العالية المشابهة. القيم النموذجية هي `1k` و`2k` و`4k`. لا ترسل هذه المعلمة إلى عائلات Gemini Flash إلا إذا وثّقها النموذج صراحةً. لنماذج صور xAI Grok Imagine استخدم `1k` أو `2k`.
</ParamField>
...
مثال لعائلة Flash: بالنسبة إلى `gemini-2.5-flash-image` أو `gemini-3.1-flash-image-preview`، أرسل `aspect_ratio` لكن احذف `resolution`:
