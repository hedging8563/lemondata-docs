...
<ParamField body="negative_tags" type="string">
  الأنماط المطلوب تجنبها. مطلوب عندما تكون `audio_operation` هي `add-instrumental`؛ ويتم تمريره إلى KIE باسم `negativeTags`.
</ParamField>
...
<ParamField body="make_instrumental" type="boolean">
  اضبطه على `true` لإنشاء نتيجة موسيقية بلا غناء. يربط TokenLab هذا الحقل العام بعلم KIE `instrumental`. استخدم أسماء حقول TokenLab مثل `make_instrumental` و `audio_url` و `negative_tags` و `continue_at`؛ يتم رفض أسماء KIE upstream مثل `instrumental` و `uploadUrl` و `negativeTags` و `continueAt`.
</ParamField>
