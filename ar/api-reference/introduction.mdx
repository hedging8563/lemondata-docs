...
| `/v1/tasks/{id}` | GET | الحصول على حالة المهام غير المتزامنة الخاصة بالفيديو |
| `/v1/videos/generations/{id}` | GET | مسار حالة فيديو متوافق مع الإصدارات السابقة |

<Info>
للعملاء الجدد، فضّل `/v1/tasks/{id}` واتبع `poll_url` الذي تعيده استجابات الإنشاء. أبقِ `/v1/videos/generations/{id}` فقط للتوافق مع الإصدارات السابقة.
</Info>
