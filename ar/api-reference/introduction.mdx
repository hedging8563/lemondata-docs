...
<Warning>
**حول Playground التفاعلي**: إن Playground في موقع التوثيق هذا مخصص لأغراض العرض التوضيحي فقط ولا يدعم إدخال مفاتيح API. لاختبار API، يرجى استخدام:
- **cURL** - انسخ أوامر المثال واستبدل `sk-your-api-key` بمفتاحك الفعلي
...
دعم أصلي لتنسيق Google Gemini API:

| نقطة النهاية | الطريقة | الوصف |
|----------|--------|-------------|
| `/v1beta/models/{model}:generateContent` | POST | إنشاء محتوى (تنسيق Gemini) |
| `/v1beta/models/{model}:streamGenerateContent` | POST | إنشاء محتوى متدفق (تنسيق Gemini) |

<Note>
تدعم نقاط نهاية Gemini مصادقة معلمة الاستعلام `?key=` بالإضافة إلى رمز Bearer القياسي.
</Note>
