---
title: "الحصول على حالة الموسيقى"
openapi: "GET /v1/music/generations/{id}"
description: "الحصول على حالة ونتائج مهمة إنشاء الموسيقى"
---

قم بعمل استعلام دوري (Polling) لهذا المسار للتحقق من حالة مهمة إنشاء الموسيقى.

## معلمات المسار

<ParamField path="id" type="string" required>
  معرف (ID) مهمة إنشاء الموسيقى.
</ParamField>

## الاستجابة

<ResponseField name="id" type="string">
  معرف المهمة.
</ResponseField>

<ResponseField name="status" type="string">
  حالة المهمة: `pending` أو `processing` أو `completed` أو `failed`.
</ResponseField>

<ResponseField name="progress" type="integer">
  نسبة التقدم المئوية (0-100).
</ResponseField>

<ResponseField name="audio_url" type="string">
  رابط تحميل ملف الصوت (عند الاكتمال).
</ResponseField>

<ResponseField name="video_url" type="string">
  رابط نسخة الفيديو مع التأثيرات البصرية (عند الاكتمال).
</ResponseField>

<ResponseField name="title" type="string">
  عنوان الأغنية التي تم إنشاؤها.
</ResponseField>

<ResponseField name="lyrics" type="string">
  كلمات الأغنية المنشأة أو المقدمة.
</ResponseField>

<ResponseField name="error" type="string">
  رسالة الخطأ (في حال الفشل).
</ResponseField>

<RequestExample>
```bash cURL
curl "https://api.lemondata.cc/v1/music/generations/music_abc123" \
  -H "Authorization: Bearer sk-your-api-key"
```

```python Python
import requests
import time

task_id = "music_abc123"

while True:
    response = requests.get(
        f"https://api.lemondata.cc/v1/music/generations/{task_id}",
        headers={"Authorization": "Bearer sk-your-api-key"}
    )
    result = response.json()

    if result["status"] == "completed":
        print(f"Audio URL: {result['audio_url']}")
        break
    elif result["status"] == "failed":
        print(f"Error: {result['error']}")
        break

    time.sleep(5)  # Poll every 5 seconds
```

```javascript JavaScript
const taskId = 'music_abc123';

async function pollForMusic() {
  while (true) {
    const response = await fetch(
      `https://api.lemondata.cc/v1/music/generations/${taskId}`,
      { headers: { 'Authorization': 'Bearer sk-your-api-key' } }
    );

    const result = await response.json();

    if (result.status === 'completed') {
      console.log(`Audio URL: ${result.audio_url}`);
      return result.audio_url;
    } else if (result.status === 'failed') {
      throw new Error(result.error);
    }

    await new Promise(r => setTimeout(r, 5000));
  }
}
```

```go Go
package main

import (
    "encoding/json"
    "fmt"
    "net/http"
    "time"
)

func main() {
    taskID := "music_abc123"

    for {
        req, _ := http.NewRequest("GET",
            fmt.Sprintf("https://api.lemondata.cc/v1/music/generations/%s", taskID), nil)
        req.Header.Set("Authorization", "Bearer sk-your-api-key")

        client := &http.Client{}
        resp, _ := client.Do(req)

        var result map[string]interface{}
        json.NewDecoder(resp.Body).Decode(&result)
        resp.Body.Close()

        if result["status"] == "completed" {
            fmt.Printf("Audio URL: %s\n", result["audio_url"])
            break
        } else if result["status"] == "failed" {
            fmt.Printf("Error: %s\n", result["error"])
            break
        }

        time.Sleep(5 * time.Second)
    }
}
```

```php PHP
<?php
$taskId = 'music_abc123';

while (true) {
    $ch = curl_init("https://api.lemondata.cc/v1/music/generations/{$taskId}");

    curl_setopt_array($ch, [
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_HTTPHEADER => [
            'Authorization: Bearer sk-your-api-key'
        ]
    ]);

    $response = curl_exec($ch);
    curl_close($ch);

    $result = json_decode($response, true);

    if ($result['status'] === 'completed') {
        echo "Audio URL: {$result['audio_url']}\n";
        break;
    } elseif ($result['status'] === 'failed') {
        echo "Error: {$result['error']}\n";
        break;
    }

    sleep(5);
}
```
</RequestExample>

<ResponseExample>
```json Response (Completed)
{
  "id": "music_abc123def456",
  "status": "completed",
  "audio_url": "https://cdn.example.com/music/abc123.mp3",
  "video_url": "https://cdn.example.com/music/abc123.mp4",
  "title": "Night Drive",
  "lyrics": "[Verse 1]\nDriving through the night..."
}
```
</ResponseExample>