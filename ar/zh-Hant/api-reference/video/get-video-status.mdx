...
description: "الحصول على حالة ونتائج مهمة إنشاء الفيديو"
---

## معلمات المسار

<ParamField path="task_id" type="string" required>
  معرف المهمة `task_id` الذي تم إرجاعه من طلب إنشاء الفيديو.
</ParamField>
...
<ResponseField name="progress" type="number">
  نسبة التقدم المئوية (0-100).
</ResponseField>

<ResponseField name="video_url" type="string">
  رابط URL للفيديو الذي تم إنشاؤه (عند الاكتمال).
</ResponseField>
...
<ResponseField name="completed_at" type="integer">
  الطابع الزمني للاكتمال (عند الاكتمال).
</ResponseField>
...
## أفضل الممارسات للاستعلام الدوري (Polling)

- إجراء الاستعلام الدوري (Polling) كل 5-10 ثوانٍ.
- تنفيذ التراجع الأسي (exponential backoff) للمهام التي تستغرق وقتاً طويلاً.
- تعيين حد أقصى لوقت المهلة (مثلاً 10 دقائق).
- التعامل مع حالة `failed` بشكل صحيح.
