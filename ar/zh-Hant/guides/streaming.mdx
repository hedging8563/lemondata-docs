...
description: "تنفيذ استجابات البث المباشر"
---

## نظرة عامة

يتيح لك البث (Streaming) استقبال أجزاء من المحتوى في الوقت الفعلي أثناء إنشاء الاستجابة، مما يوفر تجربة مستخدم أفضل لتطبيقات الدردشة.
...
يتبع كل جزء (chunk) في البث التنسيق التالي:
...
## التعامل مع نهاية البث

ينتهي البث عند:
- `finish_reason: "stop"` - اكتمال طبيعي
- `finish_reason: "length"` - الوصول إلى حد `max_tokens`
- `finish_reason: "tool_calls"` - النموذج يرغب في استدعاء أدوات
- `data: [DONE]` - الرسالة الأخيرة

## جمع الاستجابة الكاملة

لجمع الاستجابة الكاملة أثناء البث:
...
بالنسبة للتطبيقات غير المتزامنة:
