...
Use it when an agent needs the current TokenLab API contract instead of a manually maintained tool list. The default `core` profile exposes 29 generated endpoint tools plus two composite discovery tools. The `full` profile exposes 76 generated endpoint tools plus the same two composite tools.
...
| Arguments | `-y`, then `@tokenlabai/mcp-server` | Use `@tokenlabai/mcp-server@0.6.5` to pin the currently verified release. |
...
| `full` | 78 | 76 generated endpoint tools plus the two composite discovery tools. Adds response lifecycle, batches, worlds, native model discovery, and other allowlisted developer operations. |
