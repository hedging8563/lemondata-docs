...
Use it when an agent needs the current TokenLab API contract instead of a manually maintained tool list. The default `core` profile exposes 29 generated endpoint tools plus two composite discovery tools. The `full` profile exposes 75 generated endpoint tools plus the same two composite tools.
...
| Arguments | `-y`, then `@tokenlabai/mcp-server` | Use `@tokenlabai/mcp-server@0.6.10` to pin the currently verified release. |
...
| `full` | 77 | 75 generated endpoint tools plus the two composite discovery tools. Adds response lifecycle, batches, worlds, native model discovery, and other allowlisted developer operations. |
...
MCP tool calls return one final result. Streaming-only and realtime operations are excluded, and generated request schemas constrain supported `stream` fields to `false`. Small image and audio responses return native MCP content; larger binary responses are written to `TOKENLAB_ARTIFACT_DIR` and returned with their local path, MIME type, and byte count.
...
<CardGroup cols={2}>
  <Card title="TokenLab MCP" icon="plug" href="https://tokenlab.sh/mcp">
    Product overview and connection options for the Core 31 and Full 77 toolsets
  </Card>
