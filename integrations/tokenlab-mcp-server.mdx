...
| `create_response` | Call the `/v1/responses` endpoint. Requires `TOKENLAB_API_KEY`. |
| `create_anthropic_message` | Call the Anthropic Messages `/v1/messages` endpoint. Requires `TOKENLAB_API_KEY`. |
| `create_gemini_content` | Call the Gemini native `generateContent` endpoint. Requires `TOKENLAB_API_KEY`. |
