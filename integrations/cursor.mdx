...
[Cursor](https://cursor.sh) supports bring-your-own-key (BYOK) providers, but custom API keys only work with standard chat models. Use LemonData here as an OpenAI-compatible endpoint for chat-style editor workflows, not as a full replacement for every Cursor model feature.
...
| Model | Best For |
|-------|----------|
| `gpt-4o` | General coding, debugging, chat |
| `gpt-4o-mini` | Fast edits and lightweight prompts |
...
<AccordionGroup>
  ...
  <Accordion title="Model unsupported in BYOK mode">
    Custom API keys only work with standard chat models. If a model uses reasoning-only or specialized Cursor infrastructure, switch to a standard chat model such as `gpt-4o`.
  </Accordion>
  ...
  <Accordion title="Another provider stopped working after enabling LemonData">
...
<CardGroup cols={2}>
  <Card title="Use appropriate models" icon="gauge-high">
    Use `gpt-4o-mini` for quick chat-style tasks and `gpt-4o` for heavier coding work.
  </Card>
