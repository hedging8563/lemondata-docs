...
description: "Use LemonData with Pi, a minimal terminal coding harness that supports OpenAI, Anthropic, and Google-style providers"
---
...
[Pi](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) is a minimal terminal coding harness. In the LemonData docs IA, it belongs under **Coding Tools**, not under native SDKs.
...
For LemonData, Pi is usually easiest to configure through an **OpenAI-compatible** provider path, unless you intentionally want Anthropic-native or Google-native behavior.

## Recommended LemonData Path

Use LemonData as an OpenAI-compatible endpoint:
...
Then configure Pi's provider or custom model settings so its OpenAI-style path points at LemonData.
...
That is why this page focuses on the practical LemonData connection path rather than calling Pi a pure SDK.

## Practical Setup Notes

<AccordionGroup>
  <Accordion title="OpenAI-compatible default">
    Start with LemonData's `https://api.lemondata.cc/v1` path unless your Pi workflow explicitly depends on a native provider protocol.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Connection errors">
    - Verify the configured OpenAI-style base URL is exactly `https://api.lemondata.cc/v1`
