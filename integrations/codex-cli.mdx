...
For LemonData, Codex CLI can use `/v1/responses`, but you should treat it as an advanced compatibility path. Some Responses-only features are not guaranteed across every model and routed path.
...
<Note>
If a request uses Responses-only fields that are not supported on the selected model or route, LemonData returns an explicit error instead of silently downgrading the request.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Responses-native field rejected">
    - Some fields are only available on `/v1/responses` when LemonData can guarantee that behavior for the selected model and route
  ...
  <Accordion title="/compact or auto-compact fails">
    ...
    - Keep `wire_api = "responses"` and `base_url = "https://api.lemondata.cc/v1"`
  </Accordion>
</AccordionGroup>
