...
OpenCode is an open-source AI coding assistant that runs in your terminal. It supports multiple LLM providers and can be configured to use LemonData's API for access to hundreds of models.
...
OpenCode's current docs recommend configuring providers through `opencode.json` or `opencode.jsonc`. For LemonData, use a custom provider instead of overloading the built-in `openai` provider:
...
<AccordionGroup>
  <Accordion title="Connection Error">
    - Verify `options.baseURL` is set to `https://api.lemondata.cc/v1`
