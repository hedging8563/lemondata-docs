...
description: "Install the shared TokenLab API integration skill for any coding agent, with Agent-First error recovery."
---

<Note>
This skill teaches your coding agent how to integrate TokenLab's **hundreds of AI APIs** just by describing what you need. It is packaged as one shared agent-first skill and works with any coding agent that supports skill or rules files.
...
If your tool does not support the installer, copy the `skills/tokenlab-api-integration/` folder from the repository into your tool's shared skills or rules directory.
</Note>
...
If you see `tokenlab-api-integration`, the installation was successful.
...
<Steps>
  ...
  <Step title="Get API Key">
    Navigate to [Dashboard → API Keys](https://tokenlab.sh/dashboard/api) and create a new key
  </Step>
...
Treat these headers as routing hints, not a forced automatic switch. Claude-native calls use `/v1/messages`; Gemini-native calls should follow the endpoint advertised by `X-LemonData-Native-Endpoint`, commonly `/v1beta/gemini`.
...
| Video Generation | Sora, Kling, Veo, Seedance |
...
<CardGroup cols={2}>
  ...
  <Card title="Cost Management" icon="wallet">
    Set usage limits in your [dashboard](https://tokenlab.sh/dashboard). Check `error.balance_usd` to monitor costs programmatically.
  </Card>
  <Card title="Be Direct" icon="bullseye">
    Say "I want to generate images in Python" instead of "Does TokenLab have an image API?"
  </Card>
...
<AccordionGroup>
  <Accordion title="Skill not triggering automatically?">
    Try mentioning "TokenLab" or "TokenLab API" in your request:
    ```
    Use TokenLab to integrate GPT-4 in my project
    ```
  </Accordion>

  <Accordion title="How does the API help the agent find the right model?">
    TokenLab's [Agent-First API](/guides/agent-first-api) returns structured error hints. If the agent guesses a wrong model name, the error includes `did_you_mean` with the correct name and `suggestions` with alternatives. The agent reads these and auto-corrects on the next attempt.
  </Accordion>

  <Accordion title="Which coding agents are supported?">
    Any coding agent that supports shared skill or rules directories. When the installer is supported, `npx skills add` places the shared `tokenlab-api-integration` skill in the detected location automatically.
  </Accordion>
...
<Info>
**Have questions?** Check our [GitHub Issues](https://github.com/hedging8563/tokenlab-skills/issues) or contact [support@tokenlab.sh](mailto:support@tokenlab.sh)
</Info>
