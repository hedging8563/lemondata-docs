...
description: "Generate videos with models across 11 providers — text-to-video, image-to-video, keyframe control, video extension, and editing"
---

## Overview

LemonData provides access to video generation models from 11 providers through a single unified API. Video generation is **asynchronous** — you submit a request, receive a task ID and `poll_url`, then poll for the result.
...
- **I2V (Image-to-Video)**: Animate a static image into video. For the broadest compatibility, provide a public `image_url`.
...
| Model | Quality | Notes |
|-------|---------|-------|
| `sora-2` | Standard | Default example model |
...
| `kling-3.0-motion-control` | Motion Control | Subject image + motion reference video |

### Veo (Google)

| Model | Quality | Notes |
|-------|---------|-------|
| `veo2` | Standard | Veo 2 series model |
...
| `grok-video-3-15s` | 15-second variant |
...
Animate a static image. For the broadest compatibility across video models, host the source image and pass `image_url`.
...
Control both the first and last frames for precise transitions. Currently supported by Vidu Q2 Pro.
...
| `image` | string | Inline image as a data URL. Supported by compatible models, but `image_url` provides the broadest compatibility. |
