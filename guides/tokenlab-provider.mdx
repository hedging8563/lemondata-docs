...
For current OpenClaw versions, the recommended approach is to configure TokenLab through `models.providers`.
...
<AccordionGroup>
  ...
  <Accordion title="Forgetting /v1 on lemondata-responses">
    `openai-responses` maps to TokenLab's `/v1/responses` path, so `lemondata-responses` must use `https://api.tokenlab.sh/v1`.
  </Accordion>
