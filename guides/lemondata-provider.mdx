...
|----------|----------------|----------|---------|
| `lemondata` | `openai-completions` | GPT, DeepSeek, Qwen, and most OpenAI-compatible calls | `https://api.tokenlab.sh/v1` |
| `lemondata-responses` | `openai-responses` | OpenAI Responses workflows that expect `/v1/responses` semantics | `https://api.tokenlab.sh/v1` |
| `lemondata-claude` | `anthropic-messages` | Native Claude Messages API | `https://api.tokenlab.sh` |
...
Native providers such as `anthropic-messages` and `google-generative-ai` should use `https://api.tokenlab.sh` without `/v1`, otherwise OpenClaw may construct the wrong upstream path.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Forgetting /v1 on lemondata-responses">
    `openai-responses` maps to LemonData's `/v1/responses` path, so `lemondata-responses` must use `https://api.tokenlab.sh/v1`.
  </Accordion>
