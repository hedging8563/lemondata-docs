...
| `model_price_not_configured` | Model exists internally but is not currently available for public requests |
...
When a model exists in internal governance or pricing workflows but is not exposed as a public request target yet, LemonData returns:
