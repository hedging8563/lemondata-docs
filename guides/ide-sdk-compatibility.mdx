...
| **Codex CLI** | Supported with model/channel limits | OpenAI Responses | Best when configured for `/v1/responses`; some Responses-native fields require native passthrough support from the selected model and channel |
...
|-----------------|----------|---------------|-------|
| **OpenAI SDK** | Python/JS/Go | Supported core path | Chat Completions, Embeddings, and common Responses workflows are documented; some Responses-native-only fields depend on model/channel passthrough support |
