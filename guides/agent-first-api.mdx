...
Do not infer native protocol availability from a model or provider name, or from Chat response headers. Before choosing a native endpoint, read `GET /v1/models/{model}` and use only a format in `tokenlab.accepted_request_formats`. Native requests are dispatched only to same-protocol physical routes; temporary route availability is still decided at request time.
