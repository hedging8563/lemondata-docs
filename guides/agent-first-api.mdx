...
LemonData's Agent-First API enriches error responses with structured hints that AI agents can parse and act on immediately — no web searches, no doc lookups, no guesswork.
...
When a model exists internally but is not currently exposed for public requests:
...
Agents should treat this as a platform availability signal, not as a typo. Retry the same model only after it appears in `GET /v1/models`.
...
<Note>
OpenAI-compatible endpoints use LemonData's stable public error types such as `rate_limit_exceeded`, `upstream_error`, and `all_channels_failed`. Anthropic-compatible and Gemini-compatible endpoints use their own native response shapes.
</Note>
...
| `agent_preferences.<scene>` | object | Non-chat recommendation snapshot for that scene |

`agent_preferences` is derived from a cached 24-hour success-rate snapshot:
