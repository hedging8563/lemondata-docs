...
If your integration already sends Volc-style Seedance requests, you can use the compatibility endpoint instead of reshaping the body for `/v1/videos/generations`. Send your TokenLab API key as `Authorization: Bearer ...`, use `POST /api/v3/contents/generations/tasks` for REST-style creation, or use `/api/v3?Action=CreateContentsGenerationsTasks` for Action-style requests. The response returns a `cgt-...` task ID and status responses use `queued`, `running`, `succeeded`, `failed`, `cancelled`, or `expired`.

This compatibility endpoint accepts `content[]` items such as `text`, `image_url`, `video_url`, `audio_url`, and `draft_task`. Image roles such as `first_frame`, `last_frame`, and `reference_image` are preserved. `callback_url` is not supported yet; poll the task endpoint instead.
