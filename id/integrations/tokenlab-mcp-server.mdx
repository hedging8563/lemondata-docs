...
description: "Hubungkan klien MCP ke katalog model publik, detail model, harga, dan panduan endpoint native TokenLab."
---

[TokenLab MCP server](https://github.com/hedging8563/tokenlab-mcp-server) memberikan agen yang kompatibel dengan MCP cara baca-saja (read-only) untuk menemukan model TokenLab sebelum mereka menulis kode atau memilih rute API.

Gunakan server ini ketika agen perlu membandingkan model yang tersedia, memeriksa format permintaan yang didukung, mengecek harga, atau memahami keluarga endpoint native mana yang harus digunakan untuk suatu tugas.

<Note>
Alat katalog publik tidak memerlukan API key TokenLab. Server ini tidak memproksi permintaan inferensi berbayar.
</Note>
...
Tambahkan server ke konfigurasi klien MCP Anda. Ganti path dengan path klon lokal Anda:
...
5. Gunakan `get_api_overview` ketika agen membutuhkan peta ringkas keluarga API TokenLab.
...
<AccordionGroup>
  <Accordion title="Klien tidak dapat memulai server">
    Periksa apakah path `args` mengarah ke file `src/index.js` lokal Anda dan apakah `npm install` telah selesai di repositori yang dikloning.
  </Accordion>
  ...
  <Accordion title="Bisakah server ini memanggil API inferensi TokenLab berbayar?">
    Tidak. Server ini sengaja dibuat read-only untuk data katalog publik, kontrak, harga, dan ringkasan. Gunakan API TokenLab secara langsung untuk inferensi.
  </Accordion>
</AccordionGroup>
