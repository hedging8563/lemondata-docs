---
title: "OpenCode"
description: "Konfigurasikan OpenCode untuk menggunakan LemonData API"
---

## Ringkasan

OpenCode adalah asisten coding AI open-source yang berjalan di terminal Anda. Ini mendukung banyak provider LLM dan dapat dikonfigurasikan untuk menggunakan API LemonData guna mengakses 300+ model.
...
Dokumentasi OpenCode saat ini merekomendasikan konfigurasi provider melalui `opencode.json` atau `opencode.jsonc`. Untuk LemonData, gunakan provider kustom alih-alih menimpa provider bawaan `openai`:
...
<Note>
Gunakan `@ai-sdk/openai-compatible` untuk kompatibilitas gaya chat-completions. Jika Anda secara khusus membutuhkan path provider yang dibangun di atas `/v1/responses`, dokumentasi provider OpenCode merekomendasikan untuk mengganti package menjadi `@ai-sdk/openai`.
</Note>
