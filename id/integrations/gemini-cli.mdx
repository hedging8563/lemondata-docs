...
Google Gemini CLI adalah alat baris perintah (command-line tool) untuk berinteraksi dengan model Gemini. LemonData menyediakan endpoint yang kompatibel yang memungkinkan Anda menggunakan Gemini CLI dengan akses ke ratusan model.


<Warning>
Anggap integrasi ini sebagai eksperimental: Gemini CLI saat ini belum mendokumentasikan alur Base URL kustom ala LemonData secara stabil. Jika Anda membutuhkan integrasi LemonData terminal yang benar-benar didukung, gunakan Codex CLI, Claude Code, atau OpenCode.
</Warning>
