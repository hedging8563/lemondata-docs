...
description: "Pasang skill integrasi API LemonData bersama untuk agent coding apa pun, dengan pemulihan error Agent-First."
---

<Note>
Skill ini mengajarkan agent coding Anda cara mengintegrasikan **300+ AI API** LemonData hanya dengan menjelaskan kebutuhan Anda. Skill ini dikemas sebagai skill agent-first bersama dan bekerja dengan agent coding apa pun yang mendukung file skill atau rules.

API ini bersifat **[Agent-First](/guides/agent-first-api)** — bahkan jika agent menebak nama model yang salah, respons error akan memberi tahu dengan tepat cara memperbaikinya.
</Note>
...
<Note>
Gunakan perintah instalasi non-interaktif resmi berikut:

```bash
npx skills add https://github.com/hedging8563/lemondata-skills --skill lemondata-api-integration -y
```

Perintah ini memasang skill bersama `lemondata-api-integration` dari repositori LemonData skills.

Jika alat Anda tidak mendukung installer, salin folder `skills/lemondata-api-integration/` dari repositori ke direktori skills atau rules bersama milik alat tersebut.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Coding agent mana saja yang didukung?">
    Ini dapat digunakan dengan agent coding apa pun yang mendukung direktori skills atau rules bersama. Jika alat mendukung installer, `npx skills add` akan otomatis menempatkan skill bersama `lemondata-api-integration` di lokasi yang terdeteksi.
  </Accordion>
