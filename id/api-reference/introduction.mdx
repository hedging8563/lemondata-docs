---
title: 'Referensi API'
description: 'Referensi lengkap untuk LemonData API'
---

## Gambaran Umum

LemonData adalah **native-first dan kompatibel dengan OpenAI**. Gunakan rute native penyedia seperti `POST /v1/messages` untuk Anthropic dan `/v1beta/models/...:generateContent` untuk Gemini ketika Anda memerlukan perilaku native, dan gunakan endpoint `/v1` yang kompatibel dengan OpenAI ketika Anda memigrasikan SDK atau alat bergaya OpenAI yang sudah ada. `POST /v1/responses` tetap menjadi jalur opsional tingkat lanjut untuk perilaku khusus Responses.
