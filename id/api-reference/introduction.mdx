...
| `/v1/tasks/{id}` | GET | Ambil status asinkron untuk tugas video |
| `/v1/videos/generations/{id}` | GET | Jalur status tugas video yang tetap kompatibel |

<Info>
Untuk klien baru, utamakan `/v1/tasks/{id}` dan ikuti `poll_url` yang dikembalikan respons create. Simpan `/v1/videos/generations/{id}` hanya untuk kompatibilitas ke belakang.
</Info>
