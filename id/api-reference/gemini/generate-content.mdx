...
<ParamField body="contents" type="array" required>
  ...
  - `parts` (array): Bagian konten (teks atau part gambar `inline_data`)
</ParamField>
...
Untuk request vision Gemini, letakkan gambar di dalam `contents[].parts[]` sebagai part `inline_data` yang terstruktur. Field `data` harus berisi byte file yang sudah di-Base64-encode, dan `mime_type` harus sesuai dengan format gambar yang diunggah.
