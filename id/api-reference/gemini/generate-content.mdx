...
<ParamField path="model" type="string" required>
  ...
  TokenLab menggunakan jalur Gemini native jika tersedia dan beralih ke jalur publik yang kompatibel jika penanganan native tidak tersedia untuk request multimodal tersebut.
...
<ParamField body="contents" type="array" required>
  ...
  Untuk bagian media, TokenLab saat ini menerima MIME type gambar, audio, dan video lalu meneruskannya melalui detail model yang kompatibel dengan Gemini.
...
<ParamField body="generationConfig" type="object">
  ...
  - `candidateCount` (integer): Jumlah kandidat untuk generasi non-streaming. Permintaan streaming harus menghapusnya atau mempertahankannya di `1`.
  - `responseModalities` (array): Modalitas output yang diminta untuk rute native yang kompatibel.
  - `responseMimeType` (string): Tipe MIME output seperti `text/plain` atau `application/json`.
  - `responseSchema` (object): JSON schema untuk output terstruktur saat `responseMimeType` meminta JSON.
  - `thinkingConfig` / `thinking_config` (object): Opsi anggaran berpikir untuk model yang kompatibel.
</ParamField>
