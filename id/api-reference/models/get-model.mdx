...
<ResponseField name="lemondata" type="object">
  Metadata publik khusus LemonData. Route detail dapat mencakup `category`, `pricing_unit`, `capability_flags`, `supported_operations`, `pricing_provenance`, `public_contract_summary`, `public_contract` lengkap, dan `agent_preferences` level scene untuk snapshot rekomendasi non-chat.
</ResponseField>
...
Untuk model non-chat, route detail juga mengembalikan `lemondata.agent_preferences.<scene>`.
...
Respons publik sengaja menjaga `basis.channel_id` dan `basis.physical_model` tetap `null` agar agents bisa memakai sinyal rekomendasi tanpa mengetahui identifier routing internal.
