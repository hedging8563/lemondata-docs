...
<ResponseField name="data" type="array">
  ...
  - `lemondata.category` (string): Kategori model publik
  - `lemondata.pricing_unit` (string atau `null`): Satuan harga publik jika tersedia
  - `lemondata.capability_flags` (object atau `null`): Ringkasan kapabilitas publik
  - `lemondata.supported_operations` (array atau `null`): Operasi non-chat publik untuk model
  - `lemondata.pricing_provenance` (object atau `null`): Metadata sumber harga publik
  - `lemondata.public_contract_summary` (object atau `null`): Ringkasan discovery non-chat ringan yang hanya berisi `public_operations` dan `request_endpoint`
  - `lemondata.agent_preferences` (object): Hanya dikembalikan saat `recommended_for` ada
</ResponseField>
