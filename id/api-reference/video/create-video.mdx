...
<ParamField body="reference_images" type="array">
  Gambar referensi untuk alur dengan conditioning khusus. Jumlah yang didukung bergantung pada model. Untuk `seedance-2.0` dan `seedance-2.0-fast`, LemonData saat ini mendukung hingga 9 gambar referensi, ditambah hingga 3 video referensi dan 3 audio referensi. URL publik `https` lebih disarankan; model yang kompatibel juga menerima URL `data:`.
</ParamField>
...
<ParamField body="kling_elements" type="array">
  Definisi referensi elemen untuk `kling-3.0-video`. Hanya didukung untuk request berbasis gambar. Definisikan 1-3 elemen; setiap elemen memiliki `name`, `description` opsional, dan `element_input_urls` berisi 2-4 URL gambar. Referensikan elemen di `prompt` sebagai `@name`.
</ParamField>
...
<ParamField body="video_urls" type="array">
  Input video referensi tambahan untuk model yang mendukung conditioning referensi multimodal. Jumlah yang didukung bergantung pada model. Untuk `seedance-2.0` dan `seedance-2.0-fast`, LemonData saat ini mendukung hingga 3 video referensi.
</ParamField>
...
<ParamField body="audio_urls" type="array">
  Input audio referensi tambahan untuk model yang mendukung conditioning referensi multimodal. Jumlah yang didukung bergantung pada model. Untuk `seedance-2.0` dan `seedance-2.0-fast`, LemonData saat ini mendukung hingga 3 audio referensi.
</ParamField>
...
<ParamField body="output_audio" type="boolean">
  Sakelar output audio yang bergantung pada model. Di LemonData, keluarga Veo 3 menggunakan `true` sebagai default ketika field ini dihilangkan. `kling-3.0-video` juga menerima selector ini dan memetakannya ke kontrol upstream `sound` yang kompatibel; request Kling yang menghilangkan field ini tetap senyap secara default. Alias camelCase `outputAudio` juga diterima untuk kompatibilitas.
</ParamField>
...
<ResponseField name="billing_transaction_id" type="string">
  ID transaksi billing LemonData saat settlement sudah selesai. Ini adalah identitas transaksi untuk dashboard/rekonsiliasi dan terpisah dari `id` / `task_id` async.
</ResponseField>
...
Gunakan `kling_elements` dengan `kling-3.0-video` saat membutuhkan referensi elemen. Request harus menyertakan input gambar (`image_url`, `image_urls`, `start_image`, atau `end_image`) dan mereferensikan setiap elemen di prompt dengan `@name`.
...
Gunakan `operation=reference-to-video` ketika model mendukung conditioning referensi khusus. Dalam kontrak publik LemonData, referensi gambar menggunakan `reference_images`, sedangkan video dan audio referensi multimodal menggunakan `video_urls` dan `audio_urls`. Untuk `seedance-2.0` dan `seedance-2.0-fast`, LemonData saat ini mendukung hingga 9 gambar referensi, ditambah hingga 3 video referensi dan 3 audio referensi. `duration` hanya mengatur panjang output yang dihasilkan; field ini tidak menetapkan batas terpisah untuk durasi input video referensi.
