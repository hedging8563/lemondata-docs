...
Untuk video-to-video dengan `grok-imagine-video`, kirim URL HTTPS publik `.mp4` di `video_url`. TokenLab menerjemahkannya menjadi body REST xAI `video.url`; `duration`, `aspect_ratio`, dan `resolution` tidak diterima untuk alur edit ini.
