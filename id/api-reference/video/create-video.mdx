...
<Info>
Untuk model Seedance 2.0, TokenLab otomatis menyiapkan field gambar (`image`, `image_url`, `image_urls`, `reference_images`, `start_image`, `end_image`) sebagai material avatar virtual yang dapat digunakan ulang sebelum generasi. Jika belum selesai dalam 60 detik, API mengembalikan `409 seedance_material_preparing` dengan `auto_material_asset_ids`; coba lagi setelah material tersebut menjadi `ACTIVE`.
</Info>
...
<ParamField body="draft" type="boolean">
  Flag alur Draft Seedance 1.5 Pro. Gunakan `draft=true` untuk membuat tugas draft pada rute Seedance yang mendukung Draft. Jangan kirim bersama `draft_task_id`.
</ParamField>
