...
<ParamField body="output_audio" type="boolean">
  Toggle output audio bergantung pada model. Di LemonData, request keluarga Veo 3 default ke `true` saat field ini dihilangkan. `kling-3.0-video` menerima selector ini untuk request tanpa referensi elemen dan memetakannya ke kontrol sound upstream yang kompatibel; request Kling yang menghilangkan field ini defaultnya silent. Jangan gabungkan `output_audio=true` dengan `kling_elements`. Model video publik lain mengikuti perilaku default yang sudah digovern. Alias camelCase `outputAudio` diterima untuk kompatibilitas.
</ParamField>
