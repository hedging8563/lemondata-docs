---
title: "API Manajemen"
description: "Kelola API key organisasi dan ambil usage serta billing per key dengan management token."
---

## Ikhtisar

Management API memungkinkan Anda mengelola API key organisasi dan mengambil usage serta billing untuk key tertentu tanpa memakai inference API key biasa.
...
| `limitCurrency` | enum | default `USD` | Opsional: `USD` atau `CNY`. Jika diatur ke `CNY`, `limitAmount` dianggap RMB dan dikonversi ke USD di server sebelum disimpan |
...
- Secara default, `limitAmount` pada request diartikan sebagai USD. Set `limitCurrency: "CNY"` untuk mengirim batas dalam RMB; server akan mengonversinya ke USD memakai kurs saat ini sebelum disimpan.
- Field moneter pada respons mempertahankan field USD asli dan menambahkan pasangan tampilan RMB dengan akhiran `_cny`.
- `exchange_rate` dikembalikan bersama metadata API key dan merupakan kurs USD→CNY yang dipakai untuk setiap nilai `*_cny` pada respons itu.
