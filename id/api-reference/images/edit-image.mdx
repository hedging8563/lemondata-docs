...
<Note>
`gpt-image-2` didukung pada endpoint ini. Send multipart `image` uploads, JSON `image_url`, JSON `image_urls`, or the official JSON `images` array with `image_url` references; GPT Image edits accept up to 16 source images. Optional parameters are `mask`, `n`, `size`, `quality`, `response_format`, `output_format`, `output_compression` or `compression`, `moderation`, `partial_images`, and `user`. `background` tetap khusus untuk alur generate image dan bukan bagian dari kontrak publik edit gambar `gpt-image-2` saat ini. Billing follows the upstream usage token details, including image input tokens and any image output tokens added for partial images.
</Note>
