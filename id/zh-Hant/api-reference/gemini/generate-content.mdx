---
title: "Menghasilkan Konten"
openapi: "POST /v1beta/models/{model}:generateContent"
description: "Menghasilkan konten menggunakan format Google Gemini API"
---

LemonData mendukung format Google Gemini API asli untuk model Gemini. Hal ini memungkinkan kompatibilitas langsung dengan Google AI SDK.
...
Endpoint Gemini mendukung berbagai metode autentikasi:
...
<ParamField body="generationConfig" type="object">
  ...
  - `temperature` (number): suhu pengambilan sampel
  - `topP` (number): probabilitas nucleus sampling
  - `topK` (integer): pengambilan sampel Top-K
  - `maxOutputTokens` (integer): jumlah maksimum token output
  - `stopSequences` (array): urutan penghentian
</ParamField>
