...
Streaming memungkinkan Anda menerima konten secara parsial saat respons sedang dibuat, memberikan pengalaman pengguna yang lebih baik untuk aplikasi chat.
...
Setiap blok (chunk) dalam streaming mengikuti format berikut:
...
## Menangani Akhir Streaming

Streaming berakhir pada:
- `finish_reason: "stop"` - Penyelesaian normal
- `finish_reason: "length"` - Mencapai batas `max_tokens`
- `finish_reason: "tool_calls"` - Model ingin memanggil tool
...
## Streaming Asinkron (Async Streaming)

Untuk aplikasi asinkron:
