...
<Tip>
Ingin pengaturan yang dihosting sebagai gantinya? [Buat instance LemonClaw](/claw/introduction) — LemonData sudah dikonfigurasi secara default, jadi Anda bisa langsung memulai dengan WebUI.
</Tip>
...
| `lemondata-responses` | `openai-responses` | Alur kerja OpenAI Responses yang mengharapkan semantik `/v1/responses` | `https://api.lemondata.cc/v1` |
...
| `lemondata-minimax` | `anthropic-messages` | Routing MiniMax native | `https://api.lemondata.cc` |

<Warning>
Gunakan sufiks `/v1` hanya untuk `openai-completions` dan `openai-responses`.

Penyedia native seperti `anthropic-messages` dan `google-generative-ai` harus menggunakan `https://api.lemondata.cc` tanpa `/v1`, jika tidak OpenClaw mungkin menyusun path upstream yang salah.
</Warning>
...
- [Error Handling](/guides/error-handling) — pelajari mode kegagalan umum dan pola pemulihan
- [Models Overview](https://lemondata.cc/id/models) — telusuri ID model sebelum menghubungkannya ke agen
