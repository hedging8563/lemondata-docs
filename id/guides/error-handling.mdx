...
Field dasar wajib (`message`, `type`) selalu hadir. `code` dan `param` bersifat opsional dan hanya muncul jika relevan. Field petunjuk (`did_you_mean`, `suggestions`, `hint`, `retryable`, `retry_after`, `balance_usd`, `estimated_cost_usd`) adalah ekstensi opsional untuk koreksi mandiri agen AI. Lihat [panduan Agent-First API](/guides/agent-first-api) untuk detail.

Endpoint yang kompatibel dengan OpenAI menggunakan tipe kesalahan gateway stabil TokenLab. Endpoint yang kompatibel dengan Anthropic dan Gemini menggunakan keluarga kesalahan dan bentuk respons asli mereka sendiri.
