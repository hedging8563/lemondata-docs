...
Respons pembuatan adalah catatan tugas yang dapat di-polling, bukan audio final:
...
Respons polling yang selesai dapat menyertakan field media final:
...
Field media final tidak ada sampai `status` menjadi `completed`. Tugas yang gagal mengembalikan `status: "failed"` dengan `error`.
