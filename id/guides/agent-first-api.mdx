...
Agent-First API LemonData memperkaya respons kesalahan dengan petunjuk terstruktur yang dapat diurai dan ditindaklanjuti langsung oleh agen AI — tanpa pencarian web, tanpa mencari dokumentasi, tanpa tebak-tebakan.
...
| `cache_pricing` | object or `null` | Harga cache prompt hulu + diskon cache semantik platform |
| `agent_preferences.<scene>` | object | Snapshot rekomendasi non-chat untuk scene tersebut |

`agent_preferences` diturunkan dari snapshot tingkat keberhasilan 24 jam yang di-cache:
