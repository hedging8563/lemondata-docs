...
TokenLab 會自動管理快取以優化效能並降低成本。雖然目前沒有公開的端點可用於清除快取項目，但您可以透過請求層級的控制項來完全掌控快取行為。

本頁是 request-side cache 指南。

如需嚴格的 API 參考頁面，請參閱：

- [提交快取回饋](/zh-Hant/api-reference/cache/submit-cache-feedback)
- [取得快取回饋統計](/zh-Hant/api-reference/cache/get-cache-feedback-stats)
...
請使用目前組織下快取命中的 request/cache entry 識別碼；任意 ID 會被拒絕。
