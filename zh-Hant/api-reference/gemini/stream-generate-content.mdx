...
Gemini `generateContent` 的原生 SSE 串流版本。TokenLab 會保留 upstream 事件、順序與欄位；只有 metadata 的事件、沒有 `finishReason` 的中間 chunk，以及自然 EOF 都是合法情況。不會加入 Chat Completions 的 `[DONE]`，也不會回落 Chat。
## 路徑參數
...
`generationConfig.candidateCount`、工具組合與未來欄位都會原樣交給 upstream；TokenLab 不設定自己的串流限制。
