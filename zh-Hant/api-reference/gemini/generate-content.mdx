...
TokenLab 只對公開 Gemini 原生契約且具備同協議 upstream 路徑的模型提供此入口，絕不回落 Chat Completions。ProtoJSON lowerCamelCase 與原始 proto snake_case 名稱都是官方拼法，混合請求也會原樣保留。未知欄位會 best-effort 透傳，由 upstream 決定是否支援。
## 路徑參數
...
<ParamField body="contents" type="array" required>
  對話內容。`role` 值與所有 `parts` 都會保持客戶端原樣；TokenLab 不做 trim/lowercase、Base64 改寫或 MIME 推斷。`inlineData` / `inline_data` 與 `fileData` / `file_data` 都是官方拼法，TokenLab 不會在本地強制要求 `fileData.mimeType`。引用 TokenLab 建立的 Files/Cache ID 時會檢查所有權並保留原 upstream 綁定；其他媒體與工具規則交由供應商判斷。
</ParamField>
...
<ParamField body="generationConfig" type="object">
  原生生成設定。`maxOutputTokens`、`candidateCount`、`thinkingConfig` 等 lowerCamelCase 與對應 snake_case 名稱都會原樣接受。TokenLab 不會在失敗後刪除 `thinkingConfig`，也不會對 streaming 的 `candidateCount` 設本地限制；範圍與組合由 upstream 判斷。
</ParamField>
