...
<Note>
`gpt-image-2` 已支援此端點。可傳送 multipart `image`，也可傳送 JSON `image_url`；可選參數包含 `mask`、`n`、`size`、`quality`、`response_format`、`background`、`output_format`、`output_compression` 或 `compression`、`moderation`、`partial_images`、`input_fidelity` 和 `user`。計費以 NoSpamBro/上游回傳的 usage token 明細為準，包含圖像輸入 token，以及 partial images 產生的額外圖像輸出 token。
</Note>
