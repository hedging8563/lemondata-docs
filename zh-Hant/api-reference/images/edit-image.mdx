...
<Note>
`gpt-image-2` 已支援此端點。可傳送 multipart `image` 上傳、JSON `image_url`、JSON `image_urls`，或官方 JSON `images` 陣列中的 `image_url` / `file_id` 參照；GPT Image 編輯最多接受 16 張來源圖片。`file_id` 必須先透過 LemonData `/v1/files` 以 `purpose=user_data` 或 `purpose=vision` 建立，讓閘道能將它綁定到同一個上游圖像編輯渠道。可選參數包含 `mask`、`n`、`size`、`quality`、`response_format`、`output_format`、`output_compression` 或 `compression`、`moderation`、`partial_images` 和 `user`。`background` 僅保留在圖像生成路徑中，不屬於目前 `gpt-image-2` 圖像編輯公開契約。計費以上游回傳的 usage token 明細為準，包含圖像輸入 token，以及 partial images 產生的額外圖像輸出 token。
</Note>

## 請求主體

<ParamField body="image" type="file" required>
  要編輯的圖像。必須是有效的 PNG 檔案，小於 50MB，且為正方形。圖像應在需要編輯的位置具有透明度。
</ParamField>
