...
<Warning>
此端點目前文件化的是 DALL-E 風格的 multipart 編輯流程。LemonData 上其他受支援的圖像到圖像模型會使用 `POST /v1/images/generations`，並在支援時透過 `operation: "image-to-image"` 與像 `image_url` 這類 JSON 輸入呼叫。
</Warning>
