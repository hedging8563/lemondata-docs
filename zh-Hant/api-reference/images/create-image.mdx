...
- `gemini-3-pro-image-preview` 和 `nano-banana-pro` 支援 `aspect_ratio` 以及 `resolution`（`1k`、`2k`、`4k`）。
- `gemini-2.5-flash-image`、`gemini-3.1-flash-image-preview`、`nano-banana` 和 `nano-banana-edit` 支援 `aspect_ratio`，但不提供公開的 `resolution` 選項。
...
<ParamField body="resolution" type="string">
  ...
  支援於 `gemini-3-pro-image-preview`、`nano-banana-pro`、`nano-banana-2` 以及類似的高解析度系列。常見值為 `1k`、`2k`、`4k`。除非模型明確說明，否則不要傳給 Gemini Flash 圖片系列。xAI Grok Imagine 圖片模型請使用 `1k` 或 `2k`。
</ParamField>
...
Flash 系列範例：對於 `gemini-2.5-flash-image` 或 `gemini-3.1-flash-image-preview`，請傳送 `aspect_ratio` 但省略 `resolution`：
