...
<ParamField body="model" type="string" required>
  要使用的模型（例如 `gpt-image-2`、`flux-pro`、`qwen-image` 或 `nano-banana-pro`）。使用 `GET /v1/models?recommended_for=image` 取得目前推薦清單。
</ParamField>
...
| `qwen-image` | 通常為 task-based | 文字渲染與 prompt 遵循較強 |
