...
LemonData 的公開契約接受 `audio-to-video` 與 `video-extension` 這兩個操作值，用於模型特定流程；但在目前這版文件對應的「通用啟用」公開影片模型清單中，並沒有一個廣泛啟用的公開模型明確對外提供這兩項能力。接入前請先透過 [Models API](/zh-Hant/api-reference/models/list-models) 或 [模型頁面](https://lemondata.cc/zh-Hant/models) 確認即時可用性。
