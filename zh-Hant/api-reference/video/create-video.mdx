...
<Info>
對於 Seedance 2.0 模型，TokenLab 會在生成前自動將圖片欄位（`image`、`image_url`、`image_urls`、`reference_images`、`start_image`、`end_image`）準備為可重複使用的虛擬數位人素材。如果 60 秒內仍未準備完成，API 會回傳 `409 seedance_material_preparing` 與 `auto_material_asset_ids`；請在這些素材變為 `ACTIVE` 後重試。
</Info>
...
<ParamField body="draft" type="boolean">
  Seedance 1.5 Pro 草稿工作流開關。僅在支援草稿的 Seedance 路由上使用 `draft=true` 建立草稿任務；不要和 `draft_task_id` 同時傳。
</ParamField>
