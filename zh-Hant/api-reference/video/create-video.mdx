...
<ParamField body="material_asset_id" type="string">
  [建立素材](/zh-Hant/api-reference/video/create-material-asset)回傳的 TokenLab Seedance 素材 ID。素材變為 `ACTIVE` 後，可在 Seedance 2.0 生成模型中使用。
</ParamField>

<ParamField body="material_asset_ids" type="array">
  多個 TokenLab Seedance 素材 ID。它們與 `reference_images` 共用 Seedance 圖片參考數量限制；Seedance 2.0 最多傳 9 個圖片參考。
</ParamField>
...
<ParamField body="output_audio" type="boolean">
  模型相關的音訊輸出開關。在 TokenLab 中，Veo 3 家族在省略此欄位時預設按 `true` 處理；`kling-3.0-video` 僅在非元素引用請求中接受此 selector，並會使用該模型相容的 sound 控制；Kling 請求省略時預設無聲。不要將 `output_audio=true` 與 `kling_elements` 組合使用。其他影片模型依各自文件說明的預設行為執行。為了相容歷史請求，也接受 camelCase 別名 `outputAudio`。
</ParamField>
