...
<ParamField body="output_audio" type="boolean">
  模型相關的音訊輸出開關。在 LemonData 中，Veo 3 家族在省略此欄位時預設按 `true` 處理；`kling-3.0-video` 僅在非元素引用請求中接受此 selector，並會映射到相容 upstream 的 sound 控制，Kling 請求省略時預設無聲。不要將 `output_audio=true` 與 `kling_elements` 組合使用。其他公開影片模型依各自治理後的預設行為執行。為了相容歷史請求，也接受 camelCase 別名 `outputAudio`。
</ParamField>
