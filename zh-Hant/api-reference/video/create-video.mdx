...
- 為了相容既有呼叫，TokenLab 也接受 `ratio`、`generate_audio`、`outputAudio`、`referenceImages` 和 `referenceImageType`。
...
`duration=-1` 表示讓 Seedance 在模型支援範圍內自動選擇輸出時長。TokenLab 會在任務完成前按保守方式預估費用，並在可取得完成任務 usage 時按實際結果結算。`service_tier=default` 對 Seedance 2.0 作為相容 no-op 接受；`service_tier=flex`、`frames`、`camera_fixed` 會在所選模型不支援時被拒絕。
