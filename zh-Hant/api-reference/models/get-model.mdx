...
<ResponseField name="lemondata" type="object">
  LemonData 專屬的公開中繼資料。詳情路由可包含 `category`、`pricing_unit`、`capability_flags`、`supported_operations`、`pricing_provenance`、`public_contract_summary`、完整的 `public_contract`，以及非聊天推薦快照的場景級 `agent_preferences`。
</ResponseField>
...
對於非聊天模型，詳情路由還會回傳 `lemondata.agent_preferences.<scene>`。
