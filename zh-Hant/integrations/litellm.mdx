---
title: "LiteLLM"
description: "將 TokenLab 與 LiteLLM 結合使用，可作為上游 provider，也可作為內部 gateway 層"
---
...
- 把 TokenLab 當作 LiteLLM 背後的 **OpenAI 相容 上游**
- 在 TokenLab 前面再放一層 LiteLLM，統一做虛擬 key、路由策略或額外觀測
...
<AccordionGroup>
  <Accordion title="對 TokenLab 優先使用 custom_openai">
    除非你有非常明確的理由去建立更複雜的 provider 映射，否則請把 TokenLab 視為 OpenAI 相容 上游。
  </Accordion>

  <Accordion title="只在確實需要額外 gateway 層時再用 LiteLLM">
    如果你的平台自己還需要 virtual keys、額外路由策略或集中日誌，LiteLLM 放在 TokenLab 前面是有價值的。
  </Accordion>
