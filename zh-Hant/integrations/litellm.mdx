---
title: "LiteLLM"
description: "將 LemonData 與 LiteLLM 結合使用，可作為上游 provider，也可作為內部 gateway 層"
---
...
- 把 LemonData 當作 LiteLLM 背後的 **OpenAI 相容 上游**
- 在 LemonData 前面再放一層 LiteLLM，統一做虛擬 key、路由策略或額外觀測

對 LemonData 而言，最乾淨的預設路徑是使用 LiteLLM 的 **custom OpenAI / OpenAI 相容** 設定，並把 `api_base` 指向 `https://api.lemondata.cc/v1`。

<Note>
如果你明確需要 Claude-native 或 Gemini-native 請求形狀，優先使用 LemonData 自己的原生整合頁，而不是強行經由 LiteLLM 的 OpenAI 相容 抽象。
</Note>
...
<AccordionGroup>
  <Accordion title="對 LemonData 優先使用 custom_openai">
    除非你有非常明確的理由去建立更複雜的 provider 映射，否則請把 LemonData 視為 OpenAI 相容 上游。
  </Accordion>
