---
title: "OpenCode"
description: "設定 OpenCode 以使用 LemonData API"
---

## 概覽

OpenCode 是一個在終端機中執行的開源 AI 程式設計助理。它支援多種 LLM provider，並可設定為使用 LemonData 的 API，以存取 300+ 個模型。
...
OpenCode 目前的文件建議透過 `opencode.json` 或 `opencode.jsonc` 設定 provider。對於 LemonData，請使用自訂 provider，而不是覆寫內建的 `openai` provider：
...
<Note>
使用 `@ai-sdk/openai-compatible` 以相容於 chat-completions 風格。如果你明確需要建立在 `/v1/responses` 上的 provider path，OpenCode 的 provider 文件建議將套件切換為 `@ai-sdk/openai`。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="找不到模型">
    - 檢查設定中的模型名稱是否完全一致
    - 在 [lemondata.cc/en/models](https://lemondata.cc/zh-Hant/models) 確認模型是否可用
  </Accordion>
</AccordionGroup>
