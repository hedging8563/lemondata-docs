...
LemonData 兼容任何支持 OpenAI 的聊天应用。本指南涵盖了热门桌面端和移动端聊天应用的配置方法。
...
<AccordionGroup>
  ...
  <Accordion title="404 模型不可用">
    - 验证模型名称拼写是否正确
    - 在 [lemondata.cc/en/models](https://lemondata.cc/zh-Hant/models) 检查模型可用性
    - 尝试使用其他模型
  </Accordion>

  <Accordion title="响应缓慢">
    - 尝试更快的模型（例如使用 `gpt-4o-mini` 代替 `gpt-4o`）
    - 如果可用，减小 `max_tokens` 参数
    - 检查您的网络连接
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="监控使用情况">
    在 LemonData 控制面板中检查您的使用情况，以跟踪成本并管理预算。
  </Accordion>
</AccordionGroup>