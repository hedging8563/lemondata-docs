---
title: "Dify"
description: "将 LemonData 与 Dify 集成以构建 LLM 应用"
---

## 概览

Dify 是一个开源的 LLM 应用开发平台。它提供可视化 Prompt 编排、RAG 管道、Agent 框架和 LLMOps 能力。LemonData 可以作为自定义模型供应商在 Dify 中进行配置。
...
<Steps>
  <Step title="打开设置">
    在 Dify 中，前往 **设置** → **模型供应商**
  </Step>
  <Step title="添加 OpenAI 兼容供应商">
    点击 **添加模型供应商** 并选择 **OpenAI-API-compatible**
  </Step>
  <Step title="配置供应商">
    输入以下设置：
...
| 视觉 | GPT-4o (支持图像), Claude Sonnet (支持图像) |
...
<AccordionGroup>
  <Accordion title="从经济型模型开始">
    在测试和开发阶段使用 GPT-4o-mini，在生产环境中切换到更强大的模型。
  </Accordion>
...
<AccordionGroup>
  <Accordion title="连接错误">
    - 验证 API Base URL 是否准确为 `https://api.lemondata.cc/v1`
    - 检查末尾是否有斜杠
    - 验证网络连接性
  </Accordion>
  ...
  <Accordion title="未找到模型">
    - 验证模型名称是否完全匹配
    - 在 [lemondata.cc/en/models](https://lemondata.cc/zh-Hant/models) 查看模型可用性
  </Accordion>
</AccordionGroup>