...
description: "為任何編碼代理安裝共用的 LemonData API 整合技能，並具備 Agent-First 錯誤恢復能力。"
---

<Note>
這個技能會教你的編碼代理只靠描述需求，就把 LemonData 的 **300+ AI API** 整合到程式裡。它被封裝成一個共用的 agent-first 技能，適用於任何支援 skill 或 rules 檔案的編碼代理。

這個 API 採用 **[Agent-First](/guides/agent-first-api)** 設計 —— 即使代理猜錯模型名稱，錯誤回應也會清楚告訴它如何修正。
</Note>
...
這會從 LemonData skills 倉庫安裝共用的 `lemondata-api-integration` 技能。

如果你的工具不支援安裝器，請把倉庫中的 `skills/lemondata-api-integration/` 目錄複製到該工具的共用 skills 或 rules 目錄。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="支援哪些編碼代理？">
    任何支援共用 skill 或 rules 目錄的編碼代理都可以使用它。若工具支援安裝器，`npx skills add` 會自動把共用的 `lemondata-api-integration` 技能放到偵測到的位置。
  </Accordion>
...
<Info>
**有任何疑問嗎？** 請查看我們的 [GitHub Issues](https://github.com/hedging8563/lemondata-skills/issues) 或聯繫 [support@lemondata.cc](mailto:support@lemondata.cc)
</Info>