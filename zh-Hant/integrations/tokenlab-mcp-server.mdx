...
description: "將 MCP 客戶端連接至 TokenLab 模型探索、定價、OpenAI 相容聊天與原生推論端點。"
---
...
| `create_response` | 呼叫 `/v1/responses` 端點。需要 `TOKENLAB_API_KEY`。 |
| `create_anthropic_message` | 呼叫 Anthropic Messages `/v1/messages` 端點。需要 `TOKENLAB_API_KEY`。 |
| `create_gemini_content` | 呼叫 Gemini 原生 `generateContent` 端點。需要 `TOKENLAB_API_KEY`。 |
