...
description: "將 LemonData 與 Pi 結合使用。Pi 是一個極簡終端編碼代理工具，同時支援 OpenAI、Anthropic 與 Google 風格的接入設定"
---
...
[Pi](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) 是一個極簡終端編碼代理工具。在 LemonData 的文件 IA 裡，它更適合歸類到 **Coding Tools**，而不是原生 SDK。
...
<AccordionGroup>
  <Accordion title="預設先走 OpenAI 相容">
    除非你的 Pi 工作流程明確依賴原生協議，否則優先從 LemonData 的 `https://api.tokenlab.sh/v1` 路徑開始。
  </Accordion>
