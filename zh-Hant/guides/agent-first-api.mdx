...
LemonData 的 Agent-First API 在錯誤回應中加入了可結構化的提示，AI 代理可以立即解析並採取行動——不需網路搜尋、不需查文件、也不需猜測。
...
| `cache_pricing` | object or `null` | 上游提示快取價格 + 平台語意快取折抵 |
| `agent_preferences.<scene>` | object | 該情境的非 chat 推薦快照 |

`agent_preferences` 來源於快取的 24 小時成功率快照：
