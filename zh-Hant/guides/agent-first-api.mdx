...
description: "為 AI Agent 提供的結構化錯誤提示，使其能在第一次重試時自我修正"
---

## 概覽

LemonData 的 Agent-First API 在錯誤回應中豐富了結構化提示，AI Agent 可以立即解析並採取行動——無需網頁搜尋、無需查閱文件、無需猜測。

每個錯誤回應在標準的 `error` 物件中包含選填欄位，如 `did_you_mean`、`suggestions`、`hint`、`retryable` 和 `retry_after`。這些欄位是向後相容的——不使用它們的用戶端不會看到任何差異。
...
| `retryable` | `boolean` | 重試相同的請求是否可能成功 |
...
<Note>
當原因為 `no_channels`（未為此模型配置通道）時，`retryable` 為 `false`。僅在發生斷路器觸發或配額耗盡等瞬時故障時，它才為 `true`。
</Note>
...
| 模型提供商 | 建議端點 | 效益 |
|---------------|-------------------|---------|
| Anthropic (Claude) | `/v1/messages` | 無格式轉換、擴展思考、提示詞快取 |
| Google (Gemini) | `/v1beta/gemini` | 無格式轉換、實證 (grounding)、安全設定 |
...
<CardGroup cols={2}>
  <Card title="快速失敗，提供資訊" icon="bolt">
