...
TokenLab 支援使用單一 API 金鑰的 **三種原生 API 格式**。選擇最適合您使用情境的格式 — 無需變更任何設定。
...
如果 TokenLab 無法將帶有原生工具的請求路由到支援原生格式的供應商路徑，會回傳明確的 unsupported-field 錯誤，而不是靜默丟棄工具或偽裝成 Chat Completions 函式。使用者自訂函式工具仍然是最可攜的工具路徑。
...
TokenLab 的魔力：使用 **任何 SDK** 搭配 **任何模型**。網關會自動處理格式轉換。
