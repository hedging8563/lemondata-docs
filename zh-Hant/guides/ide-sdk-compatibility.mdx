...
| **Gemini CLI** | 盡力支援 / 實驗性 | Gemini | 自訂的 LemonData base URL 流程並非穩定的上游契約 |
...
| **LlamaIndex** | Python | 透過 `OpenAILike` 支援 | 對於第三方 gateway（例如 LemonData）請使用 `OpenAILike`，而非內建的 OpenAI 類別 |
