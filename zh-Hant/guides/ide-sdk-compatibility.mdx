...
TokenLab 揭露多種 API 格式，以便常見的開發工具、SDK 與框架透過最少的黏合程式碼完成整合。

本頁面刻意比行銷矩陣範圍更窄：

- **Supported** 表示我們記錄了具體的設定路徑，且 TokenLab 揭露該路徑所期待的協定形態。
...
| **Gemini CLI** | 盡力支援 / 實驗性 | Gemini | 自訂的 TokenLab base URL 流程不是官方 Gemini CLI 整合 |
...
<Tabs>
  <Tab title="Cursor">
    ...
    ```
    Cursor 在內部使用 Anthropic 風格的工具格式。TokenLab 支援兩種格式：
...
| 參數 | 類型 | 描述 |
|-----------|------|-------------|
| `stream_options` | object | 僅 Chat Completions：`{ include_usage: true }` 用於 token 計數 |
...
支援 `stream: true` 的生成端點（包括 Chat Completions 和 Responses）使用 Server-Sent Events (SSE)：
...
<AccordionGroup>
  ...
  <Accordion title="僅在 Chat Completions 使用 stream_options.include_usage">
    Chat Completions 串流請啟用 `stream_options.include_usage` 以獲得準確的 token 計數。Responses 有自己的串流契約，不公開這個 Chat 專用選項。
  </Accordion>
