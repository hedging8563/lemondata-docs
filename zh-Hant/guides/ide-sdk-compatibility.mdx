---
title: "IDE 與 SDK 相容性"
description: "針對 AI 程式設計工具、SDK 與框架的實證相容性指南"
---

## 概覽

TokenLab 提供多種 API 格式，讓常見的程式設計工具、SDK 與框架能以最少的膠水程式碼（glue code）進行整合。

本頁面刻意比行銷矩陣更為精簡：

- **支援 (Supported)**：代表我們記錄了具體的設定路徑，且 TokenLab 提供了該路徑所預期的協定格式。
- **強原生路徑 (Strong native path)**：代表儲存庫（repo）針對該協定系列提供了直接的轉接器（adapter）或請求格式驗證。
...
| **Claude Code CLI** | 強原生路徑 | Anthropic | 原生 `/v1/messages` 路由，涵蓋思考（thinking）與 `tool_choice` 的轉接器支援 |
| **Codex CLI** | 支援（模型/路徑限制） | OpenAI Responses | 將 `/v1/responses` 視為 Codex 專用工作流程的進階路徑；並非所有模型與路由路徑皆保證支援僅限 Responses 的欄位 |
...
| SDK / 框架 | 語言 | 支援層級 | 備註 |
|-----------------|----------|---------------|-------|
| **OpenAI SDK** | Python/JS/Go | 支援核心路徑 | `/v1` Chat Completions 與 Embeddings 的相容路徑；針對提供者特定行為請使用原生提供者路由，且不要假設僅限 Responses 的欄位在每個模型上皆有效 |
| **Anthropic SDK** | Python/JS | 強原生路徑 | 原生 Messages 路由，直接支援工具、思考與提示詞快取（prompt caching） |
...
| **Langflow** | Python / Web | 支援（範圍限制） | 在 OpenAI 元件的 **OpenAI API Base** 欄位中使用 TokenLab 的 `/v1` 端點 |
| **Dify** | - | 支援（範圍限制） | OpenAI 提供者與 chat-completions 導向流程為預期路徑；不適用於 Codex 專用的 Responses 或 WebSocket 行為 |
| **Flowise** | Web / Node | 支援（範圍限制） | 若可用請使用 TokenLab 聊天節點，或指向 `https://api.tokenlab.sh/v1` 的 ChatOpenAI 相容節點 |
| **Agno** | Python | 支援路徑 | 若可用請使用 TokenLab 模型封裝器，或使用 OpenAI 相容的基礎 URL 路徑 |
| **Promptfoo** | CLI / Node | 支援路徑 | 使用 OpenAI 聊天或 Responses 提供者，並設定 `apiBaseUrl: https://api.tokenlab.sh/v1` |
...
| **Open WebUI** | Web Chat | 支援聊天應用路徑 | 將 TokenLab 新增為 OpenAI 相容 API 連線並重新整理模型列表 |
| **Chatbox / DeepChat / Jan / LibreChat / Cherry Studio** | Desktop / Web | 支援聊天應用路徑 | 使用各應用程式的 OpenAI 相容自訂提供者流程；這些應用程式通常以 chat-completions 為導向 |
...
| OpenAI 物件 | `{ "type": "function", "function": { "name": "fn" } }` | 強制使用特定函式 |
...
包含 Chat Completions 與 Responses 在內，所有支援 `stream: true` 的生成端點皆使用伺服器發送事件 (SSE)：
...
<AccordionGroup>
  ...
  <Accordion title="使用適當的 tool_choice 格式">
    請符合您 SDK 的預期格式。TokenLab 同時接受 OpenAI 與 Anthropic 格式。
  </Accordion>
</AccordionGroup>