---
title: "IDE 與 SDK 相容性"
description: "針對 AI 程式開發工具、SDK 與框架的實證相容性指南"
---

## 概覽

TokenLab 提供多種 API 格式，讓常見的程式開發工具、SDK 和框架能以最少的膠水程式碼（glue code）進行整合。

本頁面的內容刻意比行銷矩陣更為精簡：

- **支援 (Supported)**：代表我們已記錄具體的設定路徑，且 TokenLab 會公開該路徑所預期的協定格式。
- **強原生路徑 (Strong native path)**：代表儲存庫中針對該協定系列擁有直接的轉接器（adapter）或請求格式驗證。
- **盡力而為 (Best-effort)**：代表整合可能運作，但上游客戶端並未將此自訂閘道工作流程視為穩定的合約。

<Note>
  不支援的欄位處理方式並不統一。在相容性路由上，某些欄位會被忽略或標準化。在 `/v1/responses` 上，當該路由無法保證所請求的行為時，不支援的欄位可能會回傳明確的 `400` 或 `503` 錯誤。
</Note>
...
| **Claude Code CLI** | 強原生路徑 | Anthropic | 原生 `/v1/messages` 路由，具備針對思考（thinking）與 `tool_choice` 的轉接器覆蓋 |
| **Codex CLI** | 支援（模型/路徑限制） | OpenAI Responses | 將 `/v1/responses` 視為 Codex 專用工作流程的進階路徑；並非所有模型與路由路徑都能保證支援僅限 Responses 的欄位 |
| **Gemini CLI** | 盡力而為 / 實驗性 | Gemini | 自訂 TokenLab 基礎 URL 流程並非官方的 Gemini CLI 整合 |
| **OpenCode** | 支援 | OpenAI 相容 | 預設使用 OpenAI 相容提供者；僅在明確需要時才切換至基於 Responses 的提供者 |
| **OpenHands** | 支援 | OpenAI 相容 | 使用 LiteLLM/OpenAI 相容路徑，設定 `openai/<model>` 與 TokenLab 的 `/v1` 基礎 URL |

<Note>
  其他 OpenAI 相容的編輯器與代理工具通常也適用相同的基礎 URL 模式；在生產環境使用前，請先檢查該工具本身的自訂提供者支援情況。
</Note>
...
| SDK / 框架 | 語言 | 支援層級 | 備註 |
|-----------------|----------|---------------|-------|
| **OpenAI SDK** | Python/JS/Go | 支援核心路徑 | `/v1` Chat Completions 與 Embeddings 的相容路徑；針對提供者特定行為請使用原生提供者路由，且不要假設僅限 Responses 的欄位適用於所有模型 |
| **Anthropic SDK** | Python/JS | 強原生路徑 | 原生 Messages 路由，直接支援工具、思考與提示快取（prompt caching） |
| **Vercel AI SDK** | TypeScript | 推薦整合模式 | 優先使用 `@ai-sdk/openai-compatible`；僅在明確需要 Responses 原生行為時才使用 `@ai-sdk/openai` |
| **LangChain** | Python/JS | 支援標準介面 | `ChatOpenAI` 與 `OpenAIEmbeddings` 為預期範圍；廠商原生擴充功能不在範圍內 |
| **LlamaIndex** | Python | 透過 `OpenAILike` 支援 | 針對 TokenLab 等第三方閘道，請使用 `OpenAILike` 而非內建的 OpenAI 類別 |
...
| **Dify** | - | 支援（有限制） | OpenAI 提供者與 chat-completions 導向流程為預期路徑；不適用於 Codex 專用的 Responses 或 WebSocket 行為 |
...
| **Promptfoo** | CLI / Node | 支援路徑 | 使用 OpenAI 聊天或 Responses 提供者並設定 `apiBaseUrl: https://api.tokenlab.sh/v1` |
| **Portkey Gateway** | Gateway | 支援路徑 | 將 TokenLab 作為 OpenAI 相容聊天與 Responses 流量的上游提供者 |
| **Helicone** | Gateway / 可觀測性 | 支援可觀測性路徑 | 透過 Helicone Gateway 路由 OpenAI 相容請求，並設定 `Helicone-Target-Url: https://api.tokenlab.sh` |
| **Phoenix** | 可觀測性 | 支援追蹤路徑 | 自動檢測（Auto-instrument）配置了 TokenLab `/v1` 基礎 URL 的 OpenAI SDK 客戶端 |
| **Opik** | 可觀測性 | 支援追蹤路徑 | 封裝配置了 TokenLab `/v1` 基礎 URL 的 OpenAI SDK 客戶端 |
| **LangBot** | Bot 平台 | 支援聊天路徑 | 若可用請使用 TokenLab 請求器，或配置一個指向 TokenLab `/v1` 端點的 OpenAI 相容請求器 |
...
| 參數 | 類型 | 說明 |
|-----------|------|-------------|
| `stream_options` | object | 僅限 Chat Completions：`{ include_usage: true }` 以取得 token 計算 |
| `reasoning_effort` | string | 針對具備推理能力的 GPT-5 模型：`"low"`, `"medium"`, `"high"` |
...
包含 Chat Completions 與 Responses 在內，所有公開 `stream: true` 的生成端點皆使用 Server-Sent Events (SSE)：
...
詳細資訊請參閱 [錯誤處理指南](/guides/error-handling)。
...
<AccordionGroup>
  ...
  <Accordion title="僅在 Chat Completions 使用 stream_options.include_usage">
    針對 Chat Completions 串流，請啟用 `stream_options.include_usage` 以取得準確的 token 計算。Responses 擁有自己的串流合約，並不公開此 Chat 專用選項。
  </Accordion>
