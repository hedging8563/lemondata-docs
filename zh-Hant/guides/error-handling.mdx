...
| 403 | Forbidden - 拒絕存取或不允許使用該模型 |
| 404 | Not Found - 找不到模型或資源 |
| 413 | Payload Too Large - 超過輸入或檔案大小限制 |
| 429 | Too Many Requests - 超過速率限制 |
| 500 | Internal Server Error |
| 502 | Bad Gateway - 上游供應商錯誤 |
| 503 | Service Unavailable - 所有通道均失敗 |
...
| `expired_api_key` | `expired_api_key` | API key 已撤銷 |
...
| 類型 | 代碼 | 描述 |
|------|------|-------------|
| `access_denied` | `access_denied` | 拒絕存取資源 |
