...
OpenAI 相容 endpoints 使用 LemonData 的穩定 gateway 錯誤類型。Anthropic-compatible 與 Gemini-compatible endpoints 則使用其原生的錯誤分類與回應格式。
...
| 類型 | 代碼 | 說明 |
|------|------|-------------|
| `insufficient_balance` | `insufficient_balance` | 帳戶餘額過低（OpenAI 相容 endpoints） |
