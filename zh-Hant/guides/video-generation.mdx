...
模型庫存會持續變化。若要取得最新的公開影片模型可用性，請使用 [Models API](/zh-Hant/api-reference/models/list-models) 或造訪[模型頁面](https://lemondata.cc/zh-TW/models)。
...
影片計費會依模型而異。有些公開影片模型實際上偏向按次計費，有些則偏向按秒計費。請以[模型頁面](https://lemondata.cc/zh-TW/models)或 [Pricing API](/zh-Hant/api-reference/pricing/get-pricing) 的即時公開價格為準。
