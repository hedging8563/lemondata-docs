...
當前的公共示例使用 `suno_music` 進行音樂生成，並在 `mv` 中傳入 `chirp-v4` 等官方 Suno 模型版本。對於僅歌詞的流程，發送 `action: "LYRICS"` 與公開合同文件中包含歌詞生成的模型，並省略 `mv`。將模型 ID 視為公共 TokenLab ID，而不是保證供應商特定字段是公共合同字段。
