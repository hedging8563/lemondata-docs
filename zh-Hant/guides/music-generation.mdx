...
音樂生成是異步的。 `POST /v1/music/generations` 創建一個公共的 TokenLab 任務並返回 `id` / `task_id`、`status`，通常還有 `poll_url`。您的應用程序應該存儲該任務身份，顯示任務狀態，並輪詢直到達到終端狀態。
...
當前的公共示例使用 `suno_music` 進行音樂生成，並在 `mv` 中傳入 `chirp-v4` 等官方 Suno 模型版本。對於僅歌詞的流程，發送 `action: "LYRICS"` 與模型詳情文件中包含歌詞生成的模型，並省略 `mv`。將模型 ID 視為公共 TokenLab ID，而不是保證供應商特定字段是支持说明字段。
...
最終媒體欄位只會在 `status` 為 `completed` 後出現。失敗任務會返回 `status: "failed"`，並帶有 `error`。
