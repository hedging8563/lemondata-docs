...
使用請求體中的 `cache_control` 參數控制每個請求的快取行為：
...
| 類型 | 效果 |
|------|------|
| `no_cache` | 跳過快取查詢，始終獲取全新回應 |
| `no_store` | 不將此回應存入快取 |
| `response_only` | 僅使用精確匹配快取（跳過語義快取） |
| `semantic_only` | 僅使用語義快取（跳過精確匹配） |
...
```
X-Cache-Status: HIT    # 回應來自快取
X-Cache-Status: MISS   # 來自模型的全新回應
```
...
| 模式 | 描述 |
|------|-------------|
| **共享 (Shared)** | 啟用快取，回應可能跨使用者共享（個人帳戶預設） |
| **隔離 (Isolated)** | 啟用快取，但回應僅限您的組織私有（組織帳戶預設） |
| **停用 (Disabled)** | 完全不使用快取 |

其他可配置項：
- **相似度閾值**：調整語義匹配靈敏度（預設：92%）
- **自訂 TTL**：覆蓋快取過期時間
- **排除模型**：為特定模型停用快取

### 請求層級

使用 `cache_control` 參數覆蓋單個請求的設定：
