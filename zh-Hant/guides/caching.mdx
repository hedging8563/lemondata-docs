...
LemonData 提供智慧快取系統，可顯著降低您的 API 成本並減少回應延遲。我們的快取不僅僅是簡單的請求匹配，它還能理解您提示詞（prompts）的**語義**。
...
- **階段 1（僅查詢）**：使用者查詢相似度 ≥95%
- **階段 2（完整上下文）**：包含對話上下文的相似度 ≥85%
- **適用於**：常見問題集（FAQ）類型的查詢、常見問題
...
在儀表板中為每個 API Key 配置快取行為：

| 模式 | 描述 |
|------|-------------|
| **預設 (Default)** | 啟用快取，可能與相似請求共享 |
| **不共享 (No Share)** | 啟用快取，但回應僅限您的帳戶私有 |
| **已停用 (Disabled)** | 完全不進行快取 |
...
<AccordionGroup>
  ...
  <Accordion title="對具時效性的查詢使用 no-cache">
    時事、即時數據應跳過快取。
  </Accordion>
