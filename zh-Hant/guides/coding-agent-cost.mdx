...
LemonData 的[語意快取](/zh-Hant/guides/caching)按語意匹配請求，而非精確文字。這對 Coding Agent 特別有效：
...
### Prompt Cache（Provider 級別）

[上游 Prompt Cache](/zh-Hant/guides/prompt-cache) 透過 LemonData 自動生效。長系統提示——Coding Agent 每次都會包含——在 Provider 層被快取：

| Provider | 快取折扣 | 最低 token 數 |
|----------|---------|-------------|
