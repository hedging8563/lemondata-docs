...
1. 使用您的 `callback_url` 呼叫 [建立真人驗證會話](/zh-Hant/api-reference/video/create-real-person-verification-session)。
...
4. 當 `verification_status` 為 `succeeded` 時，呼叫 [綁定真人驗證結果](/zh-Hant/api-reference/video/bind-real-person-verification-result)。
5. 儲存回傳的 `group_id`；在建立 `liveness_face` 素材資產時傳入此 ID。
6. 使用 [列出真人驗證歷史記錄](/zh-Hant/api-reference/video/list-real-person-verification-history) 查看先前的驗證紀錄。紀錄會長期保存，但不會延長 120 秒的綁定時效。
...
對於 `aigc_avatar`，`group_id` 為選填；TokenLab 會使用或建立組織預設的虛擬人像群組。對於 `liveness_face`，`group_id` 為必填，且必須是 [綁定真人驗證結果](/zh-Hant/api-reference/video/bind-real-person-verification-result) 所回傳的群組。
