...
<Steps>
  <Step title="계정 만들기">
    이메일, Gmail 또는 GitHub 계정으로 [lemondata.cc](https://lemondata.cc)에 가입하세요.
  </Step>
  <Step title="크레딧 추가">
    대시보드를 열어 계정에 크레딧을 추가하세요. LemonData는 최소 약정 없이 사용한 만큼 지불하는 요금제를 사용합니다.
  </Step>
  <Step title="API 키 생성">
    **대시보드 → API 키**로 이동하여 새 키를 생성하세요. 이 키는 한 번만 표시되므로 안전하게 복사해 두세요.
  </Step>
</Steps>
...
LemonData는 수백 개의 모델을 지원합니다. 변경할 것은 `model` 필드뿐입니다:
