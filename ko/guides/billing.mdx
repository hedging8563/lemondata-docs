---
title: "결제 및 요금"
description: "LemonData의 종량제 요금 체계를 이해합니다"
---

## 개요

LemonData는 **종량제 요금제(pay-as-you-go)**를 사용합니다. 구독이나 최소 약정 없이 사용한 만큼만 비용을 지불하면 됩니다.
...
<Note>
  표시된 가격은 할인된 플랫폼 가격입니다. 실제 가격은 변동될 수 있습니다. 현재 요율은 [pricing page](https://lemondata.cc/ko/pricing)에서 확인하세요.
</Note>
...
<Warning>
  **중요**: 비디오, 음악 및 3D 생성 작업은 완료 시점이 아닌 **제출 시점에 결제**됩니다. 일부 이미지 모델 또한 작업 기반 응답을 반환할 수 있으며, 이 경우 동일하게 처리됩니다.
</Warning>

작업 기반 생성 흐름(비디오, 음악, 3D 및 일부 이미지 모델)의 경우:

1. 작업을 제출할 때 **즉시 결제가 발생**합니다
2. 상태 확인을 위해 반환된 `poll_url`을 따르고, `task_id`를 비동기 작업 식별자로 사용하세요
3. 작업이 실패하더라도 **자동 환불되지 않습니다**

이는 업스트림 제공업체가 결과에 관계없이 작업이 제출될 때 비용을 청구하기 때문입니다. 작업 실패가 발생하는 경우 support@lemondata.cc로 문의하여 도움을 받으세요.
...
# ⚠️ 비디오가 준비될 때가 아니라 지금 결제됩니다

poll_url = response.json()["poll_url"]
task_id = response.json()["task_id"]
# 상태 확인을 위해 poll_url 폴링...

### 청구 거래 ID

과금 대상 비스트리밍 JSON 응답은 응답이 확정되기 전에 정산이 완료된 경우 `billing_transaction_id` 를 포함합니다. 같은 값은 `X-Billing-Transaction-ID` 응답 헤더로도 노출되어 브라우저와 서버 통합에서 읽을 수 있습니다. 비동기 미디어 작업은 생성 응답의 `poll_url` 또는 `GET /v1/tasks/{id}` 를 폴링하세요. 정산이 완료되면 작업 응답에 `billing_transaction_id` 가 포함됩니다. 스트리밍 응답은 스트림 전송 후에 정산될 수 있으므로 헤더가 없으면 dashboard 사용 로그로 대조하세요.
