...
1. `callback_url`과 함께 [실제 인물 인증 세션 생성](/ko/api-reference/video/create-real-person-verification-session)을 호출합니다.
...
4. `verification_status`가 `succeeded`이면 [실제 인물 인증 결과 바인딩](/ko/api-reference/video/bind-real-person-verification-result)를 호출합니다.
5. 반환된 `group_id`를 저장하고, `liveness_face` 소재 에셋을 생성할 때 전달합니다.
6. [실제 인물 인증 기록 목록 조회](/ko/api-reference/video/list-real-person-verification-history)를 사용하여 이전 세션을 검토합니다. 기록은 장기간 보관되지만 120초의 바인딩 창을 연장하지는 않습니다.
...
`aigc_avatar`의 경우 `group_id`는 선택 사항이며, TokenLab은 조직 기본 가상 아바타 그룹을 사용하거나 생성합니다. `liveness_face`의 경우 `group_id`가 필수이며, [실제 인물 인증 결과 바인딩](/ko/api-reference/video/bind-real-person-verification-result)에서 반환된 그룹이어야 합니다.
