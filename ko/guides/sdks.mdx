...
지원 경계는 각 integration 페이지를 기준으로 보세요. 이 문서 세트에서 `recommended` 또는 `supported` 는 문서화된 설정 경로가 존재한다는 뜻이지, repo 안의 모든 framework helper 또는 provider-specific feature에 대해 end-to-end 회귀가 있다는 뜻은 아닙니다.
