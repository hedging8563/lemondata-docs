---
title: "✨ 에이전트 우선 API (Agent-First API)"
description: "AI 에이전트가 첫 번째 재시도에서 스스로 수정할 수 있도록 돕는 구조화된 오류 힌트"
---

## 개요

LemonData의 Agent-First API는 AI 에이전트가 웹 검색, 문서 조회, 추측 없이 즉시 파싱하고 조치를 취할 수 있는 구조화된 힌트로 오류 응답을 풍부하게 합니다.
...
<Note>
`retryable`은 원인이 `no_channels`(이 모델에 대해 구성된 채널이 없음)인 경우 `false`입니다. 서킷 브레이커 작동이나 할당량 소진과 같은 일시적인 실패의 경우에만 `true`가 됩니다.
</Note>
...
| 모델 제공자 | 권장 엔드포인트 | 이점 |
|---------------|-------------------|---------|
| Anthropic (Claude) | `/v1/messages` | 형식 변환 없음, 확장 생각(extended thinking), 프롬프트 캐싱 |
...
기계가 읽을 수 있는 API 개요를 다음에서 확인할 수 있습니다:
...
<CardGroup cols={2}>
  ...
  <Card title="데이터 기반 제안" icon="database">
    모든 권장 사항은 하드코딩된 목록이 아닌 실제 프로덕션 데이터를 기반으로 합니다.
  </Card>
