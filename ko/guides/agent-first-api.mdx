...
LemonData의 에이전트 우선(Agent-First) API는 AI 에이전트가 즉시 파싱하고 조치를 취할 수 있는 구조화된 힌트로 오류 응답을 풍부하게 제공합니다. 웹 검색, 문서 조회, 추측이 필요 없습니다.

모든 오류 응답에는 표준 `error` 객체 내에 `did_you_mean`, `suggestions`, `hint`, `retryable`, `retry_after`와 같은 선택적 필드가 포함됩니다. 이 필드들은 하위 호환성을 유지하므로, 이를 사용하지 않는 클라이언트에게는 아무런 차이가 없습니다.
...
`did_you_mean` 해결에는 다음이 사용됩니다:
1. 정적 별칭 매핑 (프로덕션 오류 데이터 기반)
2. 정규화된 문자열 일치 (하이픈 제거, 대소문자 구분 없음)
3. 편집 거리(Edit distance) 일치 (임계값 ≤ 3)
...
<Note>
`retryable`은 원인이 `no_channels`(이 모델에 대해 구성된 채널 없음)인 경우 `false`입니다. 서킷 브레이커 트리거 또는 할당량 소진과 같은 일시적인 실패의 경우에만 `true`가 됩니다.
</Note>
...
`retry_after` 값은 실제 rate limit 윈도우 재설정 시간을 기준으로 계산됩니다.

<Note>
OpenAI 호환 엔드포인트는 `rate_limit_exceeded`, `upstream_error`, `all_channels_failed`와 같은 LemonData의 안정적인 공용 오류 유형을 사용합니다. Anthropic 호환 및 Gemini 호환 엔드포인트는 해당 서비스 고유의 응답 형식을 사용합니다.
</Note>

### context_length_exceeded (400)

입력이 모델의 컨텍스트 창을 초과할 때 (업스트림 오류에 힌트가 추가됨):
...
| 모델 제공자 | 추천 엔드포인트 | 장점 |
|---------------|-------------------|---------|
| Anthropic (Claude) | `/v1/messages` | 형식 변환 없음, 확장된 사고(extended thinking), 프롬프트 캐싱 |
| Google (Gemini) | `/v1beta/gemini` | 형식 변환 없음, 그라운딩(grounding), 안전 설정 |
| OpenAI | — | Chat completions가 이미 네이티브 형식임 |
...
```bash
GET /v1/models?category=chat          # 채팅 모델만 조회
GET /v1/models?category=image         # 이미지 생성 모델만 조회
GET /v1/models?tag=coding&category=chat  # 코딩에 최적화된 채팅 모델 조회
```

## llms.txt

기계 읽기 가능한 API 개요를 다음 주소에서 확인할 수 있습니다:
...
<CardGroup cols={2}>
  <Card title="빠른 실패, 정보 제공형 실패" icon="bolt">
    에이전트가 스스로 수정하는 데 필요한 모든 데이터와 함께 오류가 즉시 반환됩니다.
  </Card>
  <Card title="자동 라우팅 없음" icon="route">
    API는 다른 모델로 자동으로 대체하지 않습니다. 결정은 에이전트가 내립니다.
  </Card>
  <Card title="데이터 기반 제안" icon="database">
    모든 추천은 하드코딩된 목록이 아닌 실제 프로덕션 데이터를 기반으로 합니다.
  </Card>
  <Card title="하위 호환성" icon="plug">
    모든 힌트 필드는 선택 사항입니다. 기존 클라이언트는 아무런 차이를 느끼지 못합니다.
  </Card>
</CardGroup>