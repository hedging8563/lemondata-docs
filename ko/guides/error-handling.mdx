---
title: "오류 처리"
description: "API 오류를 원활하게 처리하세요"
---
...
| `quota_exceeded` | `quota_exceeded` | API 키 사용 한도 도달 |
...
| 유형 | 코드 | 설명 |
|------|------|-------------|
| `access_denied` | `access_denied` | 리소스 액세스 거부 |
...
| `context_length_exceeded` | 모델에 비해 입력이 너무 긺 |
...
<AccordionGroup>
  <Accordion title="지수 백오프 구현">
    속도 제한이 발생하면 재시도 간격을 점진적으로 늘리세요:
    ```python
  ...
  <Accordion title="타임아웃 설정">
    요청이 중단되는 것을 방지하기 위해 항상 적절한 타임아웃을 설정하세요:
    ```python
  ...
  <Accordion title="디버깅을 위한 오류 로깅">
    지원을 위해 요청 ID를 포함한 전체 오류 응답을 로깅하세요:
    ```python
    except APIError as e:
