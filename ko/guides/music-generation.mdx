...
현재 공개 예제는 음악 생성을 위해 `suno_music`을 사용하고 `chirp-v4` 같은 공식 Suno 모델 버전을 `mv`에 포함합니다. 가사 전용 흐름의 경우, 가사 생성을 문서화한 모델과 함께 `action: "LYRICS"`를 전송하고 `mv`는 생략하세요. 모델 ID는 공개 TokenLab ID로 취급하며, 제공자 특정 필드가 공개 계약 필드라는 보장은 아닙니다.
