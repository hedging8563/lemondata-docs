...
현재 공개 예제는 음악 생성을 위해 `suno_music`을 사용하고 `chirp-v4` 같은 공식 Suno 모델 버전을 `mv`에 포함합니다. 가사 전용 흐름의 경우, 가사 생성을 문서화한 모델과 함께 `action: "LYRICS"`를 전송하고 `mv`는 생략하세요. 모델 ID는 공개 TokenLab ID로 취급하며, 제공자 특정 필드가 모델 세부정보 필드라는 보장은 아닙니다.
...
생성 응답은 최종 오디오가 아니라 폴링 가능한 작업 레코드입니다.
...
완료된 폴링 응답에는 최종 미디어 필드가 포함될 수 있습니다.
...
최종 미디어 필드는 `status`가 `completed`가 되기 전에는 없습니다. 실패한 작업은 `status: "failed"`와 `error`를 반환합니다.
