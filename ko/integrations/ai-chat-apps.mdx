...
<Tabs>
  ...
  <Tab title="CherryStudio">
    ...
    5. 모델을 추가합니다: `gpt-4o`, `claude-sonnet-4-5` 등
    6. 저장하고 테스트합니다.
  </Tab>
  <Tab title="Chatbox">
...
<AccordionGroup>
  <Accordion title="연결 오류">
    ...
    - 끝에 슬래시(/)가 있는지 확인하세요 (있는 경우 제거).
    - 인터넷 연결 및 방화벽 설정을 확인하세요.
  </Accordion>

  <Accordion title="401 Unauthorized">
    - API Key를 다시 확인하세요.
    - [LemonData 대시보드](https://lemondata.cc/dashboard/api)에서 키가 활성 상태인지 확인하세요.
    - 키에 공백이 포함되어 있지 않은지 확인하세요.
  </Accordion>
  ...
  <Accordion title="느린 응답 속도">
    - 더 빠른 모델을 사용해 보세요 (예: `gpt-4o` 대신 `gpt-4o-mini`).
    - 가능한 경우 `max_tokens` 파라미터 값을 줄이세요.
    - 네트워크 연결 상태를 확인하세요.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="적절한 모델 사용">
    작업에 맞는 모델을 선택하세요. 간단한 채팅에는 GPT-4o-mini를, 복잡한 작업에는 Claude나 GPT-4o를 사용하세요.
  </Accordion>

  <Accordion title="자주 사용하는 프롬프트 저장">
    대부분의 앱은 프롬프트나 페르소나 저장을 지원합니다. 일반적인 작업을 위한 템플릿을 만들어 보세요.
  </Accordion>
