...
<Note>
채팅 보완 호환성에는 `@ai-sdk/openai-compatible`을 사용하세요. `/v1/responses`가 꼭 필요하다면 provider 패키지를 `@ai-sdk/openai`로 바꾸세요.
</Note>
...
<AccordionGroup>
  <Accordion title="연결 오류">
    - `options.baseURL`가 `https://api.lemondata.cc/v1`으로 설정되어 있는지 확인하세요
    - 네트워크 연결 상태를 확인하세요
