...
<Note>
이 스킬은 코딩 에이전트에게 필요한 내용을 설명하는 것만으로 LemonData의 **300개 이상의 AI API**를 통합하는 방법을 알려줍니다. 하나의 공유 Agent-First 스킬로 패키징되어 있으며, 스킬 또는 rules 파일을 지원하는 모든 코딩 에이전트에서 작동합니다.
