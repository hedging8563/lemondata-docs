...
description: "어떤 코딩 에이전트에도 공용 LemonData API 통합 스킬을 설치할 수 있으며, Agent-First 오류 복구도 함께 제공합니다."
---

<Note>
이 스킬은 필요한 내용을 설명하기만 하면 코딩 에이전트가 LemonData의 **300+ AI API** 를 코드에 통합하도록 도와줍니다. 하나의 공용 agent-first 스킬로 패키징되어 있으며, skill 또는 rules 파일을 지원하는 모든 코딩 에이전트에서 사용할 수 있습니다.

이 API 는 **[Agent-First](/guides/agent-first-api)** 방식으로 설계되었습니다. 에이전트가 모델 이름을 잘못 추측해도, 오류 응답이 어떻게 수정해야 하는지 정확히 알려줍니다.
</Note>
...
<Note>
표준 비대화형 설치 명령을 사용하세요:

```bash
npx skills add https://github.com/hedging8563/lemondata-skills --skill lemondata-api-integration -y
```

이 명령은 LemonData skills 저장소에서 공용 `lemondata-api-integration` 스킬을 설치합니다.

도구가 설치 프로그램을 지원하지 않으면 저장소의 `skills/lemondata-api-integration/` 폴더를 해당 도구의 공용 skills 또는 rules 디렉터리로 복사하세요.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="어떤 코딩 에이전트가 지원되나요?">
    공용 skill 또는 rules 디렉터리를 지원하는 모든 코딩 에이전트에서 사용할 수 있습니다. 설치 프로그램을 지원하는 경우 `npx skills add` 가 공용 `lemondata-api-integration` 스킬을 감지된 위치에 자동으로 배치합니다.
  </Accordion>
...
<Info>
**궁금한 점이 있으신가요?** [GitHub Issues](https://github.com/hedging8563/lemondata-skills/issues)를 확인하거나 [support@lemondata.cc](mailto:support@lemondata.cc)로 문의해 주세요.
</Info>