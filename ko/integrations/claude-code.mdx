...
Anthropic은 현재 네이티브 설치 프로그램 사용을 권장합니다. npm 설치 방식은 호환성을 위해 남아 있지만 업스트림에서는 deprecated 상태입니다.

```bash
curl -fsSL https://claude.ai/install.sh | bash
```

npm 패키지가 꼭 필요하다면 다음을 사용하세요:
...
<Warning>
Claude Code에서 `sudo npm install -g`를 사용하지 마세요.
</Warning>
