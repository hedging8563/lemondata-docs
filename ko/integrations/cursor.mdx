...
[Cursor](https://cursor.sh)는 AI 기반 코드 에디터입니다. LemonData를 커스텀 API 제공자로 사용하여 300개 이상의 모델에 액세스할 수 있습니다.

## 설정

1. Cursor 설정을 엽니다 (`Cmd/Ctrl + ,`)
2. 왼쪽 사이드바에서 **Models**로 이동합니다
3. **OpenAI API Key** 섹션으로 스크롤을 내립니다
...
Composer 기능(`Cmd/Ctrl + I`)은 LemonData 모델과 함께 다음 작업에 사용됩니다:
...
<AccordionGroup>
  <Accordion title="모델이 표시되지 않음">
    Cursor는 모델 목록을 캐시합니다. API 설정 후 Cursor를 다시 시작해 보세요.
  </Accordion>

  <Accordion title="응답 속도 느림">
    모델 선택을 확인하세요. 빠른 작업에는 `gpt-4o-mini`와 같은 더 빠른 모델을 사용하세요.
  </Accordion>

  <Accordion title="인증 오류">
    API 키가 정확하고 잔액이 충분한지 확인하세요.
  </Accordion>
</AccordionGroup>
