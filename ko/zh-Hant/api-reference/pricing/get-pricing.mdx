---
title: "가격 정보 조회"
openapi: "GET /v1/pricing"
description: "모델의 가격 정보를 조회합니다."
---

## 쿼리 파라미터

<ParamField query="model" type="string">
  가격 정보를 조회할 모델 ID입니다. 지정하지 않으면 모든 모델의 가격 정보를 반환합니다.
</ParamField>

## 응답

<ResponseField name="object" type="string">
  항상 `list`입니다.
</ResponseField>

<ResponseField name="data" type="array">
  모델 가격 정보 객체의 배열입니다.

  각 객체는 다음을 포함합니다:
  - `model` (string): 모델 식별자
  - `pricing` (object): 가격 정보
    - `input_per_1m` (string | null): 1M input tokens당 USD 가격
    - `output_per_1m` (string | null): 1M output tokens당 USD 가격
    - `per_request` (string | null): 요청당 USD 가격 (고정 가격 모델에 적용)
    - `is_lock_price` (boolean): 가격이 요청당 고정 가격인지 여부
    - `currency` (string): 항상 `USD`
  - `provider` (string): 모델 제공자
</ResponseField>

<RequestExample>
```bash cURL
curl "https://api.lemondata.cc/v1/pricing?model=gpt-4o" \
  -H "Authorization: Bearer sk-your-api-key"
```

```python Python
import requests

response = requests.get(
    "https://api.lemondata.cc/v1/pricing",
    headers={"Authorization": "Bearer sk-your-api-key"},
    params={"model": "gpt-4o"}
)

print(response.json())
```

```javascript JavaScript
const response = await fetch(
  'https://api.lemondata.cc/v1/pricing?model=gpt-4o',
  {
    headers: { 'Authorization': 'Bearer sk-your-api-key' }
  }
);

const data = await response.json();
console.log(data);
```

```go Go
package main

import (
    "encoding/json"
    "fmt"
    "net/http"
)

func main() {
    req, _ := http.NewRequest("GET", "https://api.lemondata.cc/v1/pricing?model=gpt-4o", nil)
    req.Header.Set("Authorization", "Bearer sk-your-api-key")

    client := &http.Client{}
    resp, _ := client.Do(req)
    defer resp.Body.Close()

    var result map[string]interface{}
    json.NewDecoder(resp.Body).Decode(&result)
    fmt.Println(result)
}
```

```php PHP
<?php
$ch = curl_init('https://api.lemondata.cc/v1/pricing?model=gpt-4o');

curl_setopt_array($ch, [
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_HTTPHEADER => [
        'Authorization: Bearer sk-your-api-key'
    ]
]);

$response = curl_exec($ch);
curl_close($ch);

$data = json_decode($response, true);
print_r($data);
```
</RequestExample>

<ResponseExample>
```json Response
{
  "object": "list",
  "data": [
    {
      "model": "gpt-4o",
      "pricing": {
        "input_per_1m": "2.50",
        "output_per_1m": "10.00",
        "per_request": null,
        "is_lock_price": false,
        "currency": "USD"
      },
      "provider": "openai"
    }
  ]
}
```
</ResponseExample>

## 가격 유형

| 유형 | 설명 | 예시 모델 |
|------|-------------|----------------|
| **Token 기반** | 입력/출력 tokens에 따라 과금 | GPT-4o, Claude, Gemini |
| **요청당 과금** | 요청당 고정 가격 | DALL-E 3, Sora, Suno |