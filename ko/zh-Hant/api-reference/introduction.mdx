---
title: 'API 레퍼런스'
description: 'LemonData API의 전체 레퍼런스 가이드'
---

## 개요

LemonData API는 **OpenAI와 호환**됩니다. 즉, Base URL만 변경하면 공식 OpenAI SDK를 그대로 사용할 수 있습니다. 또한 네이티브 **Anthropic** 및 **Gemini** 요청 형식도 지원합니다.

## Base URL

```
https://api.lemondata.cc
```

## 인증

모든 API 엔드포인트는 Bearer token을 사용한 인증이 필요합니다:

```bash
Authorization: Bearer sk-your-api-key
```

[콘솔](https://lemondata.cc/dashboard)에서 API 키를 발급받으세요.

<Warning>
**인터랙티브 Playground 안내**: 본 문서 사이트의 Playground는 데모용이며 API 키 입력을 지원하지 않습니다. API를 테스트하려면 다음을 사용하세요:
- **cURL** - 예제 명령어를 복사하고 `sk-your-api-key`를 실제 키로 교체하세요.
- **Postman** - [OpenAPI 사양](/openapi.json)을 가져오기 하세요.
- **SDK** - OpenAI/Anthropic SDK를 당사의 Base URL과 함께 사용하세요.
</Warning>

## 지원되는 엔드포인트

### 채팅 및 텍스트 생성

| 엔드포인트 | 메서드 | 설명 |
|----------|--------|-------------|
| `/v1/chat/completions` | POST | OpenAI 호환 채팅 완성 |
| `/v1/messages` | POST | Anthropic 호환 메시지 API |
| `/v1/responses` | POST | OpenAI Responses API |

### 임베딩 및 리랭크

| 엔드포인트 | 메서드 | 설명 |
|----------|--------|-------------|
| `/v1/embeddings` | POST | 텍스트 임베딩 생성 |
| `/v1/rerank` | POST | 문서 리랭크(Rerank) |

### 이미지

| 엔드포인트 | 메서드 | 설명 |
|----------|--------|-------------|
| `/v1/images/generations` | POST | 텍스트에서 이미지 생성 |
| `/v1/images/edits` | POST | 이미지 편집 |

### 오디오

| 엔드포인트 | 메서드 | 설명 |
|----------|--------|-------------|
| `/v1/audio/speech` | POST | 텍스트 음성 변환 (TTS) |
| `/v1/audio/transcriptions` | POST | 음성 텍스트 변환 (STT) |

### 비디오

| 엔드포인트 | 메서드 | 설명 |
|----------|--------|-------------|
| `/v1/videos/generations` | POST | 비디오 생성 작업 생성 |
| `/v1/videos/generations/{id}` | GET | 비디오 작업 상태 조회 |

### 음악

| 엔드포인트 | 메서드 | 설명 |
|----------|--------|-------------|
| `/v1/music/generations` | POST | 음악 생성 작업 생성 |
| `/v1/music/generations/{id}` | GET | 음악 작업 상태 조회 |

### 3D 생성

| 엔드포인트 | 메서드 | 설명 |
|----------|--------|-------------|
| `/v1/3d/generations` | POST | 3D 모델 생성 작업 생성 |
| `/v1/3d/generations/{id}` | GET | 3D 작업 상태 조회 |

### 모델

| 엔드포인트 | 메서드 | 설명 |
|----------|--------|-------------|
| `/v1/models` | GET | 사용 가능한 모든 모델 목록 조회 |
| `/v1/models/{model}` | GET | 특정 모델 정보 조회 |

### Gemini (v1beta)

네이티브 Google Gemini API 형식 지원:

| 엔드포인트 | 메서드 | 설명 |
|----------|--------|-------------|
| `/v1beta/models/{model}:generateContent` | POST | 콘텐츠 생성 (Gemini 형식) |
| `/v1beta/models/{model}:streamGenerateContent` | POST | 스트리밍 콘텐츠 생성 (Gemini 형식) |

<Note>
Gemini 엔드포인트는 표준 Bearer token 외에도 `?key=` 쿼리 파라미터 인증을 지원합니다.
</Note>

## 응답 형식

모든 응답은 일관된 형식을 따릅니다:

### 성공 응답

```json
{
  "id": "chatcmpl-abc123",
  "object": "chat.completion",
  "created": 1234567890,
  "model": "gpt-4o",
  "choices": [...],
  "usage": {
    "prompt_tokens": 10,
    "completion_tokens": 20,
    "total_tokens": 30
  }
}
```

### 라우팅 투명성

모든 응답에는 채널 정보가 포함된 `_routing` 필드가 포함됩니다:

```json
{
  "id": "chatcmpl-abc123",
  ...,
  "_routing": {
    "channel": {
      "id": "ch_xxx",
      "name": "channel-name",
      "provider": "openai",
      "channelType": "PLATFORM"
    },
    "cached": false,
    "retryCount": 0
  }
}
```

| 필드 | 설명 |
|-------|-------------|
| `channel.id` | 사용된 채널 식별자 |
| `channel.provider` | 업스트림 제공자 (openai, anthropic 등) |
| `channel.channelType` | `PLATFORM` (LemonData) 또는 `PRIVATE` (BYOK) |
| `cached` | 응답의 캐시 여부 |
| `retryCount` | 재시도 횟수 (있는 경우) |

### 오류 응답

```json
{
  "error": {
    "message": "Invalid API key provided",
    "type": "invalid_api_key",
    "code": "invalid_api_key"
  }
}
```

## 속도 제한

속도 제한은 역할을 기반으로 하며 관리자가 구성할 수 있습니다. 기본값:

| 역할 | 분당 요청 수(RPM) |
|------|-------------|
| 사용자 | 60 |
| 파트너 | 300 |
| VIP | 1,000 |

<Note>
사용자 지정 속도 제한을 원하시면 지원 팀에 문의하세요. 정확한 수치는 계정 설정에 따라 다를 수 있습니다.
</Note>

모든 응답에는 속도 제한 헤더가 포함됩니다:

```
X-RateLimit-Limit: 60
X-RateLimit-Remaining: 55
X-RateLimit-Reset: 1234567890
```

## OpenAPI 사양

<Card
  title="OpenAPI 사양"
  icon="file-code"
  href="/openapi.json"
>
  전체 OpenAPI 3.0 사양 다운로드
</Card>