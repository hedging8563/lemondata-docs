...
LemonData는 Gemini 모델의 네이티브 Google Gemini API 형식을 지원합니다. 이를 통해 Google AI SDK와 직접적인 호환성을 제공합니다.
...
<ParamField body="contents" type="array" required>
  ...
  - `parts` (array): 콘텐츠 부분 (텍스트 또는 인라인 데이터)
</ParamField>
...
<ParamField body="generationConfig" type="object">
  ...
  - `topP` (number): 핵 샘플링 확률
  - `topK` (integer): Top-K 샘플링
  - `maxOutputTokens` (integer): 최대 출력 token 수
  - `stopSequences` (array): 중지 시퀀스
</ParamField>
