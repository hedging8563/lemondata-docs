...
<ResponseField name="lemondata" type="object">
  LemonData 전용 공개 메타데이터입니다. 상세 경로는 `category`, `pricing_unit`, `capability_flags`, `supported_operations`, `pricing_provenance`, `public_contract_summary`, 전체 `public_contract`, 그리고 비채팅 추천 스냅샷용 scene 수준의 `agent_preferences` 를 포함할 수 있습니다.
</ResponseField>
...
비채팅 모델의 경우 상세 경로는 `lemondata.agent_preferences.<scene>` 도 반환합니다.
