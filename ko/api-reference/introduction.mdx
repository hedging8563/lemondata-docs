...
LemonData는 **네이티브 우선이며 OpenAI 호환입니다**. 네이티브 동작이 필요할 때는 Anthropic에는 `POST /v1/messages`, Gemini에는 `/v1beta/models/...:generateContent`와 같은 공급자 네이티브 경로를 사용하고, 기존 OpenAI 스타일 SDK나 도구를 마이그레이션할 때는 OpenAI 호환 `/v1` 엔드포인트를 사용하세요. `POST /v1/responses`는 Responses 전용 동작을 위한 고급 선택 경로로 남아 있습니다.
