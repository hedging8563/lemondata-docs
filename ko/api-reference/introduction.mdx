...
| `/v1/tasks/{id}` | GET | 비디오 작업의 비동기 상태 조회 |
| `/v1/videos/generations/{id}` | GET | 하위 호환용 비디오 작업 상태 경로 |

<Info>
새 클라이언트에서는 `/v1/tasks/{id}`를 우선 사용하고, create 응답이 돌려주는 `poll_url`을 그대로 따르세요. `/v1/videos/generations/{id}`는 backward compatibility 용도로만 남겨두세요.
</Info>
