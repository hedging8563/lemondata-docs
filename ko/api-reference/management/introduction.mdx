...
Management API 는 일반 추론용 API 키를 쓰지 않고도 조직 API 키를 관리하고 특정 키의 사용량과 과금을 조회할 수 있게 해줍니다.
...
- 기본적으로 요청의 `limitAmount` 는 USD로 해석됩니다. `limitCurrency: "CNY"` 를 설정하면 RMB 한도를 보낼 수 있고, 서버는 저장 전에 현재 환율로 USD로 변환합니다.
- 응답의 금액 필드는 원래 USD 값을 유지하면서 `_cny` 접미사가 붙은 RMB 표시값을 함께 제공합니다.
- `exchange_rate` 는 API 키 메타데이터와 함께 반환되며, 해당 응답의 각 `*_cny` 값에 사용된 USD→CNY 환율입니다.
