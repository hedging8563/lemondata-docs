...
Gemini `generateContent`의 네이티브 SSE streaming 변형입니다. TokenLab는 upstream 이벤트, 순서, 필드를 보존합니다. metadata-only 이벤트, `finishReason`이 없는 중간 chunk, 자연스러운 EOF가 모두 유효합니다. Chat Completions `[DONE]`을 추가하지 않으며 Chat으로 fallback하지 않습니다.
## 경로 파라미터
...
`generationConfig.candidateCount`, 도구 조합, 향후 필드는 변경 없이 upstream으로 전달하며 TokenLab 자체 streaming 제한을 두지 않습니다.
