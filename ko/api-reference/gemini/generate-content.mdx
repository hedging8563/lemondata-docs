...
<ParamField body="contents" type="array" required>
  ...
  `user`와 `model` 역할 값은 대소문자를 구분하지 않고 정규화됩니다. `application/octet-stream`의 `inlineData` / `inline_data`는 LemonData가 지원되는 이미지 또는 비디오 바이트로 식별할 수 있을 때만 허용되며, 그렇지 않으면 라우팅 전에 실패합니다. 네이티브 이미지 출력 요청에서는 Google search/maps 도구 계열만 허용되고, 지원되지 않는 도구 조합은 upstream 재시도 전에 실패합니다.
</ParamField>
