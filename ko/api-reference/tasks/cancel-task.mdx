...
현재 취소는 대기 중인 Volcengine Seedance 동영상 작업에만 지원되며 모델은 `seedance-1.5-pro`, `seedance-2.0`, `seedance-2.0-fast`입니다. 이미 `processing`/running 상태인 작업은 취소할 수 없습니다. 완료, 실패, 만료 또는 미지원 작업은 상태와 과금 정산을 위해 계속 조회 가능하게 유지하며 업스트림 레코드를 삭제하지 않습니다.
