...
<Note>
`gpt-image-2`는 이 엔드포인트에서 지원됩니다. Send multipart `image` uploads, JSON `image_url`, JSON `image_urls`, or the official JSON `images` array with `image_url` references; GPT Image edits accept up to 16 source images. Optional parameters are `mask`, `n`, `size`, `quality`, `response_format`, `output_format`, `output_compression` or `compression`, `moderation`, `partial_images`, and `user`. `background` 는 이미지 생성 경로에만 남아 있으며 현재 `gpt-image-2` 이미지 편집 공개 계약에는 포함되지 않습니다. Billing follows the upstream usage token details, including image input tokens and any image output tokens added for partial images.
</Note>
