...
<ParamField body="image" type="file">
  multipart 원본 이미지. 여러 GPT Image 원본을 제공하려면 `image`를 반복해서 보내세요. 파일은 PNG, JPEG, WebP여야 하며 최대 16개 원본 이미지, 각 `50MB`까지 지원됩니다. 기존 DALL-E 2 마스크 편집은 여전히 투명 영역이 있는 PNG 입력 또는 별도 `mask`를 기대합니다.
</ParamField>
