...
<ParamField body="size" type="string" default="1024x1024">
  이미지 크기입니다. OpenAI 스타일 이미지 패밀리와 정확한 픽셀 크기를 받는 다른 모델에 사용합니다.

  Google Gemini image-family 모델의 경우 `size` 는 모델의 공개 `aspect_ratio` 와, 지원되는 경우 `resolution` 계약에 맞는 호환성 별칭으로 취급됩니다. 해당 모델에는 `aspect_ratio` 를 직접 보내는 편이 좋습니다.
</ParamField>
