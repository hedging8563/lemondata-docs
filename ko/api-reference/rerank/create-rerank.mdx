...
<ParamField body="top_n" type="integer">
  반환할 상위 결과의 개수입니다. 기본값은 모든 문서입니다. 최소 `1`이어야 하며 `documents.length`보다 클 수 없습니다. 현재 TokenLab에는 관리 중인 제공자별 더 낮은 하드 캡이 없습니다. 제공자가 나중에 이를 공개하면 문서화하거나 강제하기 전에 rerank request-shape truth에 먼저 추가해야 합니다.
</ParamField>
