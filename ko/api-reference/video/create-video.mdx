...
<ParamField body="reference_images" type="array">
  reference-to-video 흐름에서 사용하는 참조 이미지 입력입니다. 허용 개수는 모델마다 다릅니다. `seedance-2.0` 및 `seedance-2.0-fast` 에서 LemonData 는 현재 최대 9장의 참조 이미지와 추가로 최대 3개의 참조 비디오, 3개의 참조 오디오를 지원합니다. 공개 `https` URL을 권장하며, 호환 모델은 `data:` URL도 지원합니다.
</ParamField>
...
<ParamField body="video_urls" type="array">
  멀티모달 참조 조건부 생성을 지원하는 모델용 추가 참조 비디오 입력입니다. 허용 개수는 모델마다 다릅니다. `seedance-2.0` 및 `seedance-2.0-fast` 에서 LemonData 는 현재 최대 3개의 참조 비디오를 지원합니다.
</ParamField>
...
<ParamField body="audio_urls" type="array">
  멀티모달 참조 조건부 생성을 지원하는 모델용 추가 참조 오디오 입력입니다. 허용 개수는 모델마다 다릅니다. `seedance-2.0` 및 `seedance-2.0-fast` 에서 LemonData 는 현재 최대 3개의 참조 오디오를 지원합니다.
</ParamField>
...
<ParamField body="output_audio" type="boolean">
  모델별 오디오 출력 토글입니다. LemonData에서 Veo 3 계열 요청은 이 필드를 생략하면 기본값이 `true`입니다. `kling-3.0-video`는 요소 참조를 사용하지 않는 요청에서 이 selector를 허용하며 호환되는 upstream sound 제어로 매핑합니다. 생략된 Kling 요청은 기본적으로 무음입니다. `output_audio=true`와 `kling_elements`를 함께 사용하지 마세요. 다른 공개 비디오 모델은 관리된 기본 동작을 따릅니다. 호환성을 위해 camelCase 별칭 `outputAudio`도 허용됩니다.
</ParamField>
...
<ResponseField name="billing_transaction_id" type="string">
  정산이 이미 완료된 경우 반환되는 LemonData 청구 거래 ID입니다. dashboard / 정산에 사용하는 거래 식별자이며 비동기 `id` / `task_id` 와는 별개입니다.
</ResponseField>
...
모델이 전용 참조 조건부 생성을 지원하면 `operation=reference-to-video` 를 사용하세요. LemonData 공개 계약에서는 이미지 참조는 `reference_images`, 멀티모달 참조 비디오와 오디오는 `video_urls` 와 `audio_urls` 를 사용합니다. `seedance-2.0` 및 `seedance-2.0-fast` 에서 LemonData 는 현재 최대 9장의 참조 이미지와 추가로 최대 3개의 참조 비디오, 3개의 참조 오디오를 지원합니다. `duration` 은 생성 출력 길이만 제어하며, 참조 비디오 입력 길이의 별도 제한을 설정하지 않습니다.
...
LemonData 의 공개 계약은 모델별 흐름을 위해 `audio-to-video` 와 `video-extension` 을 받아들입니다. 다만 이 문서 빌드 기준으로 공개적으로 널리 활성화된 모델 목록에는 이 두 기능을 공개적으로 내세우는 모델이 없습니다. 실제 연동 전에는 [Models API](/ko/api-reference/models/list-models) 또는 [모델 페이지](https://lemondata.cc/ko/models)에서 현재 지원 상태를 확인하세요.
