...
<ParamField body="reference_images" type="array">
  reference-to-video 흐름에서 사용하는 참조 이미지 입력입니다. 허용 개수는 모델마다 다릅니다. `seedance-2.0` 및 `seedance-2.0-fast` 에서 TokenLab 는 현재 최대 9장의 참조 이미지와 추가로 최대 3개의 참조 비디오, 3개의 참조 오디오를 지원합니다. 공개 `https` URL을 권장하며, 호환 모델은 `data:` URL도 지원합니다.
</ParamField>
