...
오디오 출력은 모델마다 다릅니다. TokenLab 에서는 `output_audio` 를 생략하면 Veo 3 계열이 기본적으로 오디오 켜짐으로 처리됩니다. 모델이 오디오 제어를 지원한다면 `output_audio` 로 명시적으로 켜거나 끄세요. camelCase 별칭 `outputAudio` 도 호환성을 위해 허용됩니다.
...
<ParamField body="output_audio" type="boolean">
  모델별 오디오 출력 토글입니다. TokenLab에서 Veo 3 계열 요청은 이 필드를 생략하면 기본값이 `true`입니다. `kling-3.0-video`는 요소 참조를 사용하지 않는 요청에서 이 selector를 허용하며 해당 모델의 호환되는 sound 제어를 사용합니다. 생략된 Kling 요청은 기본적으로 무음입니다. `output_audio=true`와 `kling_elements`를 함께 사용하지 마세요. 다른 비디오 모델은 문서화된 기본값을 사용합니다. 호환성을 위해 camelCase 별칭 `outputAudio`도 허용됩니다.
</ParamField>
...
- 호환성을 위해 TokenLab 는 camelCase 별칭 `referenceImages`, `referenceImageType`, `outputAudio` 도 허용합니다.
- `operation` 을 생략하면 TokenLab 가 입력을 보고 추론합니다. 그래도 운영 환경에서는 명시적으로 보내는 편이 좋습니다.
...
모델이 피사체 이미지와 모션 참조 비디오를 모두 필요로 한다면 `operation=motion-control` 을 사용하세요. TokenLab 는 공개 입력 형태인 `image_url` + `video_url` 을 upstream 계약에 맞게 정규화합니다.
