...
Responses API는 OpenAI의 최신 상태 유지 대화 API입니다. TokenLab는 호환 모델에 대해 고급 선택 경로로 이 형식을 지원합니다. Responses 고유의 동작이 명시적으로 필요한 경우가 아니면 기본적으로 OpenAI 호환 경로인 `POST /v1/chat/completions`를 사용하세요.
