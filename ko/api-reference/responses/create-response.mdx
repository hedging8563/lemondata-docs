...
description: "OpenAI Responses API 형식을 사용하여 response를 생성합니다."
---
...
<ParamField body="input" type="array" required>
  ...
  각 항목은 다음과 같을 수 있습니다:
  - `message`: role과 content를 포함한 대화 메시지
  - `function_call`: 함수 호출 요청
  - `function_call_output`: 함수 호출 결과
</ParamField>

<ParamField body="instructions" type="string">
  모델을 위한 시스템 지침입니다 (system message와 동일).
</ParamField>
...
<ParamField body="previous_response_id" type="string">
  대화를 이어가기 위한 이전 response의 ID입니다.
</ParamField>

<ParamField body="store" type="boolean" default="true">
  나중에 조회할 수 있도록 response를 저장할지 여부입니다.
</ParamField>

<ParamField body="metadata" type="object">
  추적 목적으로 response에 첨부할 메타데이터입니다.
</ParamField>
...
<ResponseField name="created_at" type="integer">
  response가 생성된 시점의 Unix 타임스탬프입니다.
</ResponseField>

<ResponseField name="output" type="array">
  모델에 의해 생성된 출력 항목 목록입니다.
</ResponseField>
